...
当前开发版本：`v0.8.0-dev.39`（2026-07-21）；最新稳定版：`v0.7.29`。核心运行形态正在从托盘常驻 + Web 管理面板迁移为 Windows 桌面壳 + 托盘常驻。
...
- Windows 桌面总览页使用固定 `1536x1024` 设计画布整体缩放，覆盖 `1080x720` 最小窗口到宽屏窗口；移除全局底栏后，工作台按可用视口自适应延伸到底部，集中展示同步健康、今日活动、运行任务、7 条最近同步、优先处理、任务状态和今日传输。
