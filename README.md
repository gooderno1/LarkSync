...
当前版本：`v0.7.19-dev.1`（2026-05-24），核心运行形态为托盘常驻 + Web 管理面板。
...
- macOS CI 现已进一步补齐安装/启动级 smoke：构建出 DMG 后会自动挂载镜像、复制 `.app` 到临时安装目录，并直接启动 bundle 内 `LarkSync --backend` 做 `/health` 检查，避免只验证“能打包”而不验证“能装、能起”。
