...
当前开发版本：`v0.8.0-dev.27`（2026-07-15）；最新稳定版：`v0.7.29`。核心运行形态正在从托盘常驻 + Web 管理面板迁移为 Windows 桌面壳 + 托盘常驻。
...
- 独立任务详情页恢复原始浅色设计：固定使用“同步任务 / 任务名称”面包屑和“任务详情”标题，主列完整展示任务身份、本地与云端关系、当前或最近运行、5 条运行历史；右侧 300px 依次保留问题摘要、任务操作、策略摘要、忽略目录和常驻危险操作五张独立卡片。同步关系区使用“电脑端点—代码内双环同步符号—云端端点”，左右连接线等宽且不引用品牌 Logo；任务表中的项目名称与右侧文件夹按钮均可进入详情。详情页“编辑策略”复用任务表三点按钮的居中设置弹窗，本地目录通过后端接口调用系统文件管理器打开；开发演示任务可直接进入详情，`?ui-data=live` 切换真实数据。
...
- 桌面工作区底部与 78px 状态栏统一使用纯白收口，只保留一条浅蓝分割线；四页关键卡片已在 `1536x1024`、`1440x1024`、`1280x1024` 三档窗口执行边界检测，首屏越界为 0。
