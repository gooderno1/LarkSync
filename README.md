...
当前稳定版本：`v0.8.18`（2026-07-31）。核心运行形态为 Windows 桌面壳 + 托盘常驻。
当前代码基线：`v0.8.19-dev.1`。
...
- v0.8.19-dev.1 补齐 v0.8.18 的历史问题迁移：未解决或已忽略的问题只要最新证据严格匹配`完成: total=N ok=N failed=N skipped=N`运行汇总，就自动以`workflow_summary_not_problem`结案并保留原始活动证据；扫描同时兼容已被旧迁移改成`task_run`且分类器已是 v3 的记录，真实下载、认证和对象错误不会被误结案。
