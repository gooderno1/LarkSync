...
当前版本：`v0.7.3`（2026-05-12），核心运行形态为托盘常驻 + Web 管理面板。
...
- Markdown 上行遇到超限表格时，会在块级上传前按飞书建表限制自动拆分为多个原生表格，补齐可读列宽，并为表格单元格文本写入居中样式；既有云端文档缺少当前表格渲染修复标记时，即使本地内容 hash 未变化，也会跳过局部 diff 并在原 doc token 内全量重建，避免历史坏表格块继续残留为代码块、窄表格或未居中表格。
