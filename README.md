...
当前稳定版本：`v0.9.10`（2026-09-03）。核心运行形态为 Windows / macOS 桌面壳 + 托盘或菜单栏常驻。
当前代码基线：`v0.9.11-dev.2`。
...
- v0.9.11-dev.2 修复 macOS 安装体验：PyInstaller 符号链接下的根目录 Logo 以正确 PNG 类型返回；菜单栏父进程不再出现在 Dock/应用切换器，只有桌面窗口保留一个 LarkSync；重复启动会唤醒现有窗口；全屏关闭改为隐藏 Cocoa 应用，避免黑色全屏 Space，并可从 Dock 或菜单栏恢复。
