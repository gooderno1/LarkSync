...
当前稳定版本：`v0.9.5`（2026-09-02）。核心运行形态为 Windows / macOS 桌面壳 + 托盘或菜单栏常驻。
当前代码基线：`v0.9.6-dev.1`。
...
- v0.9.6-dev.1 将组织名称改为零额外权限的本地账号标识：新账号授权成功页可直接编辑，未修改时保存“飞书组织 N / Lark 组织 N”稳定预设；schema v13 自动为缺少组织名的旧账号补齐预设并按账号发送一次可改名通知。用户信息接口返回的 `tenant_key` 会保存为内部组织标识，但数据隔离仍以 `account_id` 为边界。
- v0.9.6-dev.1 不再为组织名称要求用户扫码开通 `tenant:tenant:readonly` 或进入开发者后台；已有权限时仍可静默补全官方名称和 Logo，缺权只表示增强信息不可用，不影响登录、同步和账号状态。设置页只保留“修改组织名称”，组织兜底不再暴露账号 UUID 前缀。
