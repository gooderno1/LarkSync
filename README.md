...
当前版本：`v0.6.2-dev.4`（2026-04-29），核心运行形态为托盘常驻 + Web 管理面板。
...
- Windows 自动更新支持静默安装链路：更新请求默认走 NSIS `/S`，托盘会等待安装器退出后自动重启新版本，并把安装器 PID、退出码和重启动作写入 `update-install.log`；如安装到 `Program Files`，Windows 仍可能弹出 UAC 权限确认。
