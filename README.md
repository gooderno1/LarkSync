...
当前开发版本：`v0.8.0-dev.34`（2026-07-15）；最新稳定版：`v0.7.29`。核心运行形态正在从托盘常驻 + Web 管理面板迁移为 Windows 桌面壳 + 托盘常驻。
...
- 活动、冲突、设置和维护页在开发环境默认使用带“前端演示数据”标识的完整数据态，占满桌面工作区并覆盖典型问题、冲突、规则和更新场景；首屏列表按完整卡片容量收敛并提供剩余数量提示，设置与维护内容重新分区，避免卡片在状态栏上方被截断；使用 `?ui-data=live` 可切换到真实后端数据，生产构建始终使用真实数据。
- 桌面壳应用内顶栏由 88px 收敛为 56px，并以轻冷色和 Windows 原生白色标题栏分层；底部状态栏由 78px 收敛为 48px，继续使用纯白背景与单条浅蓝分割线。当前保留原生标题栏，以维持系统边缘缩放、贴边布局、窗口菜单和无障碍能力；四页关键卡片已在 `1536x1024`、`1440x1024`、`1280x1024` 三档窗口执行边界检测，首屏越界为 0。
