...
当前开发版本：`v0.8.0-dev.20`（2026-07-14）；最新稳定版：`v0.7.29`。核心运行形态正在从托盘常驻 + Web 管理面板迁移为 Windows 桌面壳 + 托盘常驻。
...
- 同步任务页将搜索、状态、模式、健康筛选与新建操作合并为同一工具栏；表格模式与操作列保持单行，任务启停使用可识别的开关，更多设置使用三点菜单语义，任务统计固定在表格底部；从未执行的任务明确显示“尚未运行”。
- 同步任务、任务详情、活动与问题、冲突处理、设置、更新维护和五列新建任务向导已统一到浅色科技风设计；`npm run dev:test` 使用项目内 `data/dev-test`，可显示 8 条隔离测试任务验证完整数据密度。
...
- 同步任务页新增独立任务详情入口；任务详情页固定采用主列 + 右侧 320px 检查器，路径关系、当前运行指标、问题摘要、任务操作、策略摘要、忽略目录和危险操作不再按 viewport 断点切换结构。
- 同步任务页、任务卡片、新建任务向导和云端目录树已统一为浅色科技风，任务列表不再混入旧暗色卡片；任务创建仍沿用本地目录、云端目录、同步策略和确认摘要的分步流程。
