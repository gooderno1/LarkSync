...
当前代码基线：`v0.8.16-dev.2` 已实装设置与更新维护页自然高度双栏，正在进入正式发布验证。
...
- v0.8.16-dev.2 实装自然高度双栏：设置页主栏依次放置账号与设备、默认同步策略和折叠 OAuth，辅助栏放置忽略规则与数据保护；更新与维护页按`3:2`承载版本安装与本机维护。两页取消强制满高、等高、内部滚动和底部锚定，正常桌面窗口保持双栏，逻辑宽度低于`900px`时才回退单栏。
