...
当前版本：`v0.6.18`（2026-05-06），核心运行形态为托盘常驻 + Web 管理面板。
...
- 冲突管理支持连续为多条冲突选择“使用本地 / 使用云端”；前端会明确显示“已排队 / 处理中 / 等待任务空闲 / 已完成 / 处理失败”，并对“任务运行中”的冲突处理自动重试，避免点完后看不出是否生效。
- 日志中心切换任务时，默认先回到 `概览`，任务概览和运行摘要优先读取 `sync_runs` 摘要表；只有真正打开事件/问题明细时才读取 `sync-events.jsonl`，避免一切换任务就自动拉取大日志时间线。
