...
当前稳定版本：`v0.9.11`（2026-09-05）。核心运行形态为 Windows / macOS 桌面壳 + 托盘或菜单栏常驻。
当前代码基线：`v0.9.12-dev.2`。
...
- v0.9.12-dev.2 修复 macOS 钥匙串重复授权：后台读取需要系统授权时停止请求，由页面“重试钥匙串访问”主动恢复；Token 改为每账号单条目，更新保留原条目身份，App Secret 读取加入缓存，已迁移账号不再反复访问全局旧凭据。兼容现有钥匙串格式，账号、任务和文件保持保留；仅删除并重装 `.app` 通常不会清除钥匙串与用户数据。
