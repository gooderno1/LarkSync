...
当前稳定版本：`v0.9.13`（2026-09-16）。核心运行形态为 Windows / macOS 桌面壳 + 托盘或菜单栏常驻。
当前代码基线：`v0.9.14-dev.1`。
...
3. 授权成功后可直接确认或修改本地组织名称；该名称只用于区分 LarkSync 中的账号，不要求开通组织信息权限，也不会更改飞书组织。
4. 按 [快速开始](docs/QUICK_START.md) 创建 `download_only` 任务，把少量文档同步到本地。若组织禁止自动创建个人应用，可从高级入口手动填写 App ID / App Secret 后扫码。
5. 在「活动管理」确认同步事件，并在「问题中心」查看需要处理的问题，再决定是否扩大目录或启用双向同步。
...
- v0.9.14-dev.1 修复通知“全部标为已读”只处理前 500 条的问题：按当前账号一次性标记全部未读通知，保留通知记录与原已读时间；界面显示实际处理数量、失败提示及每条通知的已读状态。

- v0.9.13 默认排除任意层级的 `node_modules`，在本地遍历和云端目录查询前跳过整个依赖目录，减少海量依赖上传触发的目录容量错误。已有本地与云端文件保留；关闭默认过滤可恢复纳入同步，任务自定义忽略仍独立生效。
- v0.9.12 修复 macOS 钥匙串重复授权：后台读取需要系统授权时停止请求，由页面“重试钥匙串访问”主动恢复；Token 改为每账号单条目，更新保留原条目身份，App Secret 读取加入缓存，已迁移账号不再反复访问全局旧凭据。兼容现有钥匙串格式，账号、任务和文件保持保留；仅删除并重装 `.app` 通常不会清除钥匙串与用户数据。
- v0.9.12-dev.1 为 macOS 测试版增加独立应用图标：语义版本包含预发布后缀时，Finder 与 Dock 使用右下角带橙色 `DEV` 徽标的 `LarkSync-Dev.icns`；稳定版继续使用原 `LarkSync.icns`。安装冒烟会核对 Release Channel 与图标文件，避免测试包误带正式图标。
- v0.9.11 修复 macOS 安装体验：PyInstaller 符号链接下的根目录 Logo 以正确 PNG 类型返回；菜单栏父进程不再出现在 Dock/应用切换器，只有桌面窗口保留一个 LarkSync；重复启动会唤醒现有窗口；全屏关闭改为隐藏 Cocoa 应用，避免黑色全屏 Space，并可从 Dock 或菜单栏恢复。
- v0.9.11-dev.1 完成最新版 macOS 运行与交付适配：Cocoa 激活统一调度到主线程；红色关闭按钮继续隐藏窗口，`⌘Q`、应用菜单退出或托盘退出会清理桌面窗口、后端和开发服务；DMG 在缺少 Homebrew `create-dmg` 时自动使用系统 `hdiutil`；安装冒烟改为验证当前 Device Flow 账号连接首屏，并在 headless WebKit 回退前确认原生 App PID 仍存活。
- v0.9.10 将设置页从两套独立纵向流改为共享行轨道：账号与设备卡首行上下边对齐，默认同步策略跨越应用配置和忽略规则两行，左右最终底边一致；更新维护页两张主面板同步拉伸到同一自然内容高度。两个页面均不使用固定视口高度或栏内滚动，低于 900px 时回退单列。
- v0.9.9-dev.1 将设置页按“账号、应用与文件范围｜本机与同步行为”重新配重：左栏依次放置账号管理、默认收起的应用配置和忽略规则，右栏放置当前设备、开机自启动与默认同步策略；移除没有操作项的数据保护静态卡，两栏继续自然高度且不增加内部滚动。
- v0.9.8 将设置页应用配置改为默认收起的统计摘要，展开后以紧凑列表提供本地改名和对应 App ID 的飞书/Lark 开发者后台“应用管理”入口；桌面双栏调整为接近等宽，默认首屏的信息密度和高度更平衡。
- v0.9.7 将账号管理改为默认收起的摘要卡，展开后再显示授权、同步与风险操作；移除账号使用应用内确认弹窗，设置页按约 `7:5` 分配账号/策略与设备/应用配置，减少左栏拥挤和右栏空置。
- v0.9.7 为每个本地应用配置增加可编辑别名、脱敏 App ID、创建时间、已绑定账号数和可恢复账号数；自动创建应用可在第一次扫码前命名，重新绑定已软移除的同一身份会恢复原账号及其任务、状态与历史。飞书开发者后台中的官方应用名称不受本地别名影响。
- v0.9.7 将新授权默认权限收敛为 `drive:drive`、`docx:document` 和 `docx:document.block:convert`。已有应用已开通的冗余权限不会被客户端远程撤销，但 LarkSync 不再主动请求只读重复权限、云盘元数据只读权限或通讯录基础信息权限。
- v0.9.6 将组织名称改为零额外权限的本地账号标识：新账号授权成功页可直接编辑，未修改时保存“飞书组织 N / Lark 组织 N”稳定预设；schema v13 自动为缺少组织名的旧账号补齐预设并按账号发送一次可改名通知。用户信息接口返回的 `tenant_key` 会保存为内部组织标识，但数据隔离仍以 `account_id` 为边界。
- v0.9.6 不再为组织名称要求用户扫码开通 `tenant:tenant:readonly` 或进入开发者后台；已有权限时仍可静默补全官方名称和 Logo，缺权只表示增强信息不可用，不影响登录、同步和账号状态。设置页只保留“修改组织名称”，组织兜底不再暴露账号 UUID 前缀。
- v0.9.4-dev.1 将自动注册拆为“第 1 次扫码创建应用 → 明确成功检查点 → 用户点击继续 → 第 2 次扫码授权账号”，已有应用、手动应用和重新授权保持单次扫码；账号切换采用串行事务、账号化缓存键、显式请求头和 WebSocket 订阅，任务、日志、目录、问题、冲突、通知与诊断不再复用其他账号数据。
- v0.9.5-dev.1 将侧边栏组织切换器改为与应用导航一致的扁平活动态，移除白色卡片嵌套；账号管理卡把组织权限、账号凭据和本机风险操作固定分层，断开与移除收入口菜单，避免按钮随机换行。
- v0.9.5-dev.1 正确识别飞书组织接口真实缺权码 `99991672`，只对当前 App ID、官方域名和 `tenant:tenant:readonly` 均校验通过的权限地址生成二维码；扫码进入官方权限页后每 5 秒自动复检，3 分钟后暂停。`1184001` 单独显示为当前应用类型不可用，既不要求账号重新授权，也不影响同步。
- v0.9.4-dev.1 在侧边栏顶部提供栏内展开的组织账号列表，以组织名和组织 Logo 为主身份、成员姓名为次身份，并显示每个账号独立的未读与同步错误；设置页支持组织信息刷新和本地显示名编辑，开机自启动紧邻当前账号。
- v0.9.4-dev.1 自动执行 schema v11 迁移和旧账号组织信息后台回填；组织元数据与头像缓存按账号隔离，无 `tenant:tenant:readonly` 权限时继续登录和同步，并保留本地组织显示名降级方式。官方快速注册接口没有公开应用改名字段，飞书后台仍可能显示“CLI应用”，LarkSync 内部统一显示为“LarkSync”。
- v0.9.3-dev.1 在 Device Flow 返回成功后保留明确的“授权成功”确认页，展示账号名称、V2 协议和本机凭据安全保存结果，点击完成后才刷新账号并关闭窗口；设置页把开机自启动移动到当前飞书账号下方，当前设备收敛为右侧紧凑卡，账号管理按身份、同步状态、授权方式、凭据有效期、主要操作和维护操作重新分层。
...
- 默认 OAuth 权限已收敛为 `drive:drive`、`docx:document` 与 `docx:document.block:convert`；历史 `docs:doc` 会在运行时迁移为所需 Docx scopes，旧配置中的重复或非必要默认权限会自动忽略。
...
- 设置页支持“默认忽略隐藏/缓存路径”开关：默认会跳过所有以 `.` 开头的文件或目录，以及任意层级的 `__pycache__`、`node_modules`。本地初始扫描、补齐和目录移动在进入忽略目录前剪枝；云端扫描在请求忽略目录的子项前剪枝，含文件夹快捷方式。上传、下载、删除联动和历史问题核验共用过滤规则；已同步的依赖文件保留，不因排除目录而删除。关闭该开关后可显式同步依赖目录，但任务级忽略目录仍生效；仍可按任务排除其他自定义子目录。
