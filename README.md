...
当前稳定版本：`v0.9.14`（2026-09-16）。核心运行形态为 Windows / macOS 桌面壳 + 托盘或菜单栏常驻。
当前代码基线：`v0.9.15-dev.1`。
...
- v0.9.15 将 Windows 托盘和 macOS 菜单栏的“需要处理”统一到当前账号的问题中心：已结案、已忽略或路径已排除的历史冲突不再误报；权限、上传等其他未解决问题也会提示。历史运行错误保留在诊断中，状态查询失败保持异常提示；冲突详情和文件内容不变。
- v0.9.14 修复问题中心已清空但总览仍显示冲突：优先处理、健康状态与顶部待处理统一读取当前账号的未解决问题，已解决/已忽略问题和历史排队事件不再重复计数；真实问题卡片跳转问题中心，读取失败时显示状态暂不可用。
- v0.9.14 修复通知“全部标为已读”只处理前 500 条的问题：按当前账号一次性标记全部未读通知，保留通知记录与原已读时间；界面显示实际处理数量、失败提示及每条通知的已读状态。
