...
- macOS 打包 spec 已默认把 PyInstaller `target_arch` 收口到 `universal2`（可通过 `LARKSYNC_MACOS_TARGET_ARCH` 覆盖），并在 Intel `macos-13` 与 Apple Silicon `macos-14` runner 上做日常 smoke，持续朝 `Intel/M-series` 兼容收口。
