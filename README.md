...
当前开发版本：`v0.8.0-dev.40`（2026-07-21）；最新稳定版：`v0.7.29`。核心运行形态正在从托盘常驻 + Web 管理面板迁移为 Windows 桌面壳 + 托盘常驻。
...
- 桌面版启动后先保留 1 秒 API 渲染窗口，并在 React 接管前直接显示“正在启动本地服务”；后台同步的运行摘要写入会串行执行，首次本地扫描使用单次映射查询并在线程中遍历目录，避免多任务同时启动时因大型 SQLite 数据库竞争而出现长时间空白页。
- Windows 桌面壳默认窗口与设计画布调整为 `1360x900`，常规尺寸保持 1:1 渲染，大窗口不再继续放大，小于设计画布时才按比例缩放到 `1080x720` 下限；高频 `10/11/12px` 辅助字号分别提升为 `11/12/13px`，减少高 DPI 屏幕中的整体偏小感。
- 飞书授权状态区分“权限正常、飞书明确返回缺权、暂时无法检查”三种结果；网络超时不会再显示权限不足或引导重新授权，而会保留当前登录、5 秒后自动重试，单次云空间权限探测最长等待 5 秒。
...
- 活动与问题页按 `07-activity-issues-light` 恢复原始诊断结构：页头提供任务选择与状态，主工作台使用 276px 运行历史、568px 问题摘要与连续时间线、416px 详细事件诊断三栏；运行历史包含搜索与 5 次完整记录，时间线包含四类筛选和 5 条关键事件，所有彩色事件点与竖线共用同一几何中轴；右栏固定展示基础信息、错误码、原始错误、原因、建议步骤及复制/重试操作。开发演示态和 `?ui-data=live` 真实数据态共用同一列宽与页头结构。冲突处理在零冲突时展示完整健康空状态，有冲突时切换为连续版本决策工作台。
...
- 桌面壳按统一信息架构重组：220px 侧栏负责导航、后端/飞书状态、最近同步、版本和维护入口；52px 顶部命令栏只显示任务范围、立即同步、任务管理和账号入口；职责重复的全局底部状态栏已删除。Windows 原生标题栏继续保留系统缩放、贴边布局、窗口菜单和无障碍能力，并使用 `#EAF2F8` 标题背景、`#24364F` 文字和 `#B9CBE0` 边线形成克制的冷色层次；六个核心页面已完成三档边界与控制台复核。
...
- 桌面壳内的页面适配参考 codex-companion 的固定画布缩放方法：左侧栏固定 220px、顶部命令栏固定 52px，总览、任务、任务详情、活动与问题、冲突处理、设置、更新维护和首次授权页均保持同一设计结构，再由外层画布统一缩放；大窗口不再居中露出背景边。
