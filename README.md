...
当前稳定版本：`v0.8.23`（2026-08-14）。核心运行形态为 Windows / macOS 桌面壳 + 托盘或菜单栏常驻。
当前代码基线：`v0.8.24-dev.1`。
...
- 当前首次授权页二维码承载本机 OAuth URL；当回调地址是`localhost`时，只保证在同一台电脑的浏览器中完成授权，手机扫码不能回调到桌面端。真正跨设备扫码登录需按 [v0.8.24 方案](docs/design/v0.8.24-settings-tray-qr-login-plan.md) 部署 HTTPS 中继后实现。
