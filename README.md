...
当前版本：`v0.6.17-dev.3`（2026-05-05），核心运行形态为托盘常驻 + Web 管理面板。
...
- 冲突管理支持连续为多条冲突选择“使用本地 / 使用云端”；前端会即时显示“已排队 / 处理中”，并按顺序等待后端确认，失败时保留冲突状态并提示错误。
- 日志中心切换任务时，任务概览和运行摘要优先读取 `sync_runs` 摘要表；只有真正打开事件/问题明细时才读取 `sync-events.jsonl`，大日志场景下切换明显更快。
