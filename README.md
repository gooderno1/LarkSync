...
当前稳定版本：`v0.8.15`（2026-07-30）。核心运行形态为 Windows 桌面壳 + 托盘常驻。
当前代码基线：`v0.8.16-dev.1` 仅完成设置与更新维护页自然高度双栏设计，业务页面仍为`v0.8.15`实装状态。
...
- v0.8.16-dev.1 仅更新设计：设置页收敛为“主栏配置 + 380–420px 辅助栏”，OAuth 位于主栏底部并默认折叠；更新与维护页取消强制等高、满高和底部锚定。正常桌面窗口统一使用连续尺寸响应，不再维护重复的标准/紧凑信息架构。两张最新版 imagegen 设计图已归档，等待确认后再修改业务页面。
