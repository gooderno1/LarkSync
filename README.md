...
当前版本：`v0.7.5`（2026-05-15），核心运行形态为托盘常驻 + Web 管理面板。
...
- Markdown 上行遇到超限表格时，会在块级上传前按飞书建表限制自动拆分为多个原生表格并补齐可读列宽；表格文本保持飞书默认水平左对齐，填充单元格前会清理飞书默认空段落，避免内容被追加到第二段后视觉下沉；既有云端文档缺少当前表格渲染修复标记时，即使本地内容 hash 未变化，也会跳过局部 diff 并在原 doc token 内全量重建，避免历史坏表格块继续残留为代码块、窄表格、旧水平居中样式或单元格空段落。
