...
当前版本：`v0.5.59-dev.3`（2026-04-19），核心运行形态为托盘常驻 + Web 管理面板。
...
- Markdown 上行支持 HTML 内嵌 `data:image/...` 图片：会优先复用本地 `figures/`、`插图/`、`assets/` 中的对应图片资源并上传为飞书图片块。
- 同步器会忽略 `figures/` 与 `插图/` 这类嵌入源图目录，避免源图被当作独立附件重复上传。
- Markdown 上行检测到飞书块级建表超限时，会自动改走飞书原生 Markdown 导入重建，保留大表格与图片块而不是降级成代码块。
