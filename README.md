...
当前稳定版本：`v0.9.9`（2026-09-03）。核心运行形态为 Windows / macOS 桌面壳 + 托盘或菜单栏常驻。
当前代码基线：`v0.9.10-dev.1`。
...
- v0.9.10-dev.1 将设置页从两套独立纵向流改为共享行轨道：账号与设备卡首行上下边对齐，默认同步策略跨越应用配置和忽略规则两行，左右最终底边一致；更新维护页两张主面板同步拉伸到同一自然内容高度。两个页面均不使用固定视口高度或栏内滚动，低于 900px 时回退单列。
