...
当前版本：`v0.7.28-dev.2`（2026-07-06）；最新稳定版：`v0.7.27`。核心运行形态为托盘常驻 + Web 管理面板。
...
4. 在日志中心确认最近一次同步结果，再决定是否扩大目录或启用双向同步。
...
- 事件管理统一展示待删除、失败、取消和冲突事件；冲突仍支持“使用本地 / 使用云端”定向解决。
- 日志中心按任务和运行记录查看上传、下载、删除、跳过、失败、待删除和冲突事件。
...
- 仪表盘任务概览会限制展示数量和最大高度，避免任务卡片把页面拉长；右侧“需要关注”不再被左侧高度拉伸出大块留白。
...
- 事件管理去掉重复的独立标题横条，改为单个紧凑工作区，并支持按“问题类型”或“任务”分组查看待删除、失败、删除失败、冲突和取消事件。
- 事件管理配色收敛为中性卡片 + 状态胶囊，只有失败、冲突和待删除等状态使用小面积提示色，避免整页被警告/成功色块占满。
- 日志中心的“实际变更”、任务运行摘要和任务卡片现在会明确展示 `删除 / 待删除 / 删除失败` 等删除链路状态，不再只显示上传/下载。
- 日志中心任务诊断默认只显示有上传、下载、删除、待删除、失败、冲突或正在运行的任务；全 0 无动作任务会折叠隐藏，并可在任务选择区切换显示全部。
