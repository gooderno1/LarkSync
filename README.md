...
当前版本：`v0.5.57-dev.2`（2026-04-09），核心运行形态为托盘常驻 + Web 管理面板。
...
- 更新包下载完成后支持“确认安装”安全流程：用户确认后由托盘延迟接管安装请求，避免前端请求被中断；Windows 端进一步强化安装器拉起链路，兼容含中文/特殊字符路径，降低“程序退出但安装器未启动”的风险。
