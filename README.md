...
## 公开 Beta 试用路径
第一次试用建议从低风险的 `download_only` 模式开始：先把一个小型飞书测试目录同步到本地，确认 OAuth、目录映射和转码效果正常后，再扩展到更大的目录或启用双向同步。

1. 从 [GitHub Releases](https://github.com/gooderno1/LarkSync/releases) 下载安装包。
2. 按 [快速开始](docs/QUICK_START.md) 完成安装、飞书 OAuth 配置和第一个同步任务。
3. 优先创建 `download_only` 任务，选择少量文档做首次验证。
4. 在日志中心确认最近一次同步结果，再决定是否切换到双向同步。

推广/试用前建议先阅读：

- [快速开始](docs/QUICK_START.md)
- [OAuth 配置指南](docs/OAUTH_GUIDE.md)
- [安全与隐私说明](docs/SECURITY_AND_PRIVACY.md)
- [反馈与排障指南](docs/FEEDBACK.md)
- [FAQ](docs/FAQ.md)
- [OpenClaw / AI Agent 本地飞书缓存教程](docs/OPENCLAW_LOCAL_CACHE_GUIDE.md)
- [使用教程](docs/USAGE.md)
