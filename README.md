...
当前版本：`v0.7.19-dev.5`（2026-05-24），核心运行形态为托盘常驻 + Web 管理面板。
...
- 设置页已按 `OAuth / 同步策略 / 自动更新 / 忽略目录 / 维护工具` 等面板完成组件化拆分，便于继续增强设置项与回归测试。
- 任务页与“新建任务”向导已拆成独立卡片/步骤组件，并将路径摘要、任务健康、手动云端目录解析和创建 payload 组装下沉为可测试 helper，便于继续治理任务管理流程。
...
- macOS CI 现已进一步补齐安装/启动级 smoke：构建出 DMG 后会自动挂载镜像、复制 `.app` 到临时安装目录，并直接启动 bundle 内 `LarkSync --backend` 做 `/health` 检查；若启动超时或提前退出，会回抛 bundle stdout/stderr 与 `larksync.log` 尾部，并将默认等待时间提高到 60 秒，避免 GitHub runner 上再次出现“只知道 Connection refused、不知道为什么没起来”的黑盒失败。
- macOS / Windows 安装包现在都会显式打入 `greenlet`，避免打包产物在 `sqlalchemy.ext.asyncio` 初始化数据库时因为缺少该运行时依赖而直接崩溃，导致安装后启动 smoke 卡在 `/health` 之前。
- macOS 打包现默认按当前 runner 原生架构出包，并在 Intel `macos-13` (`x86_64`) 与 Apple Silicon `macos-14` (`arm64`) runner 上分别做日常 smoke；正式版 tag 会上传双架构 DMG，更新服务会优先选择与当前机器架构匹配的安装包。
...
- 日志中心诊断 query 的 `include_problems` 判断、URL 参数组装和轮询间隔判断现已沉到 `taskDiagnosticsQuery` helper，并补齐独立测试；主诊断 hook 不再直接拼装诊断请求参数字符串。
- 日志中心概览排序、`runAlert` 和展示派生状态现已下沉到 `taskDiagnosticsState` helper，并补齐独立测试；`useLogCenterTaskDiagnostics.ts` 进一步收口为 query 接线与少量状态编排。
