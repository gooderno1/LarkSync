...
- 活动与问题页按 `07-activity-issues-light` 恢复原始诊断结构：页头提供任务选择与状态，主工作台使用 276px 运行历史、568px 问题摘要与连续时间线、416px 详细事件诊断三栏；运行历史包含搜索与 5 次完整记录，时间线包含四类筛选和 5 条关键事件，右栏固定展示基础信息、错误码、原始错误、原因、建议步骤及复制/重试操作。开发演示态和 `?ui-data=live` 真实数据态共用同一列宽与页头结构。冲突处理在零冲突时展示完整健康空状态，有冲突时切换为连续版本决策工作台。
