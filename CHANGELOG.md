...
[2026-09-03] v0.9.8-dev.1 feat(settings): 应用配置默认收起为统计摘要，展开后使用紧凑列表并提供对应飞书/Lark 开发者后台应用管理入口；设置页双栏调整为接近等宽
[2026-09-02] v0.9.7 release: 账号折叠、应用别名、软移除恢复、最小授权与设置页平衡双栏
[2026-09-02] v0.9.7-dev.1 feat(settings): 账号管理默认收起并统一应用内确认/编辑弹窗；应用配置增加本地别名、脱敏 ID、创建时间及关联/可恢复账号统计；自动注册支持扫码前命名，同应用同身份重新绑定恢复原账号；默认授权收敛为 drive/docx/convert 三项
[2026-09-02] v0.9.6 release: v0.9.6 zero-permission organization naming
[2026-09-02] v0.9.6-dev.1 feat(accounts): 新账号授权成功页可编辑本地组织名称；schema v13 自动为旧账号补齐稳定预设并发送一次通知；保存无额外权限的 tenant_key，移除组织名称权限扫码入口和账号 UUID 兜底
...
[2026-09-01] v0.9.2 release: v0.9.2 Windows 长凭据原子存储修复
[2026-09-01] v0.9.2-dev.1 fix(auth): Windows 系统凭据改为长 Token 分片、SHA-256 回读校验和活动 generation 原子切换；失败时回退旧授权，重新授权界面明确区分飞书成功与本机保存失败
[2026-09-01] v0.9.1 release: v0.9.1 认证兼容与账号连接修复
[2026-09-01] v0.9.1-dev.1 fix(auth): 按账号协议保留 OAuth V1 刷新兼容并支持手动刷新、原账号 Device Flow V2 重新授权、升级通知直达、连续两步扫码及统一开关边界
[2026-09-01] v0.9.0 release: 多账号扫码登录与并行同步
[2026-09-01] v0.9.0-dev.1 feat(accounts): 原生接入官方 lark-cli Device Flow 与个人应用注册；支持多账号同时登录、任务和凭据隔离、账户切换未读通知、单账号暂停/断开/移除及 v0.8 数据自动备份迁移
[2026-08-31] v0.8.24-dev.1 feat(desktop): 设置页接入经系统回读校验的开机自启动；重构原生托盘菜单并移除未实际暂停后端的伪暂停入口；完成基于 HTTPS 回调中继与 PKCE 的跨设备扫码登录方案
[2026-08-14] v0.8.23 release: 修复手动重试方向恢复
[2026-08-14] v0.8.22 release: 修复问题中心分方向恢复事实
[2026-08-14] v0.8.21 release: 修复问题中心恢复判断与本地移动上传竞态
...
[2026-07-31] v0.8.18 release: 修复飞书刷新凭据 20026 的跨进程重复失败、认证证据缺失和空汇总误报；更新安装前确保旧后端完全退出
[2026-07-31] v0.8.18-dev.1 fix(auth): 串行化跨进程 OAuth 凭据轮换并在刷新前强制重读；修复 20026 重复发生、任务真实错误证据缺失、空失败汇总误报下载问题及更新安装残留旧后端
[2026-07-31] v0.8.17 release: 修复 Windows 托盘唤醒已有窗口后仍被其他应用遮挡；增加跨进程前台授权、原生激活和非永久 TopMost 回退
[2026-07-31] v0.8.17-dev.1 fix(desktop): 托盘唤醒已有窗口时授权 WebView 子进程取得前台权限；Windows 激活失败时临时置顶后立即解除，修复窗口只恢复可见但仍被其他窗口遮挡
[2026-07-31] v0.8.16 release: 正式发布设置与更新维护页自然高度双栏、连续响应和默认窗口无额外滚动布局
[2026-07-31] v0.8.16-dev.2 feat(frontend): 实装设置与更新维护页自然高度双栏；移除强制满高、等高和底部锚定，补充 900px 单栏安全回退与页面结构回归测试
[2026-07-30] v0.8.16-dev.1 docs(design): 将设置与更新维护页收敛为自然高度双栏和连续响应；归档两张最新版 imagegen 设计图，业务页面尚未修改
[2026-07-30] v0.8.15 release: 修正设置页默认窗口滚动、双栏比例与 OAuth 展开逻辑；更新与维护双面板占满剩余工作区，消除底部大块留白；提高 Windows Runner 对 PowerShell 启动延迟的测试容差
...
[2026-07-22] v0.8.3-dev.4 feat(desktop): 实装活动管理与统一问题中心；新增物理窗口三档响应式布局、全部任务与事件分页审计、跨页定位、统一问题/出现/动作持久化、稳定去重、冲突映射、后端动作能力与解决验证，并以后台分批方式回溯全部历史问题，避免阻塞桌面启动

[2026-07-22] v0.8.3-dev.3 docs(design): 逐项展开活动管理与问题中心紧凑、标准、宽屏三档布局和交互响应；补齐断点滞回、低高度降级、状态迁移、实时更新、异常与无障碍契约，复核并重绘活动紧凑、活动宽屏、问题标准、问题紧凑列表和问题宽屏设计，共归档六张 v3 图片，暂不修改业务页面

[2026-07-22] v0.8.3-dev.2 docs(design): 重构活动管理与问题中心产品、数据、交互和响应式设计基线；v1 图片降级为概念稿，新增严格复用真实桌面壳的标准/紧凑窗口 v2 四图，明确统一问题模型、稳定去重、状态验证、现有动作能力矩阵及窗口切换规则，暂不修改业务页面

[2026-07-22] v0.8.3-dev.1 docs(design): 使用 imagegen 生成并归档活动管理与问题中心 v1 设计图，建立最新版设计与图片更新记录；实机审计自动更新下载中断、错误未持久化和安装流程快照错位，形成统一 UpdateOperation、断点续传、自动重试、真实进度与启动对账升级方案，暂不修改业务页面和更新代码

[2026-07-22] v0.8.2 fix(desktop): 桌面窗口关闭后隐藏到托盘并由回环控制通道快速恢复、导航和置前，桌面宿主使用轻量入口且取消固定 0.4 秒等待；正式后端默认端口迁移到 18765 并兼容环境变量覆盖、旧 OAuth 回调迁移和过渡期运行安全检查；归档活动管理与问题中心详细改版方案但暂不修改页面

[2026-07-21] v0.8.1 fix(frontend): 逐页完成桌面窗口配适；总览摘要卡避免截断，任务表格在 1080×720 保留统计与操作列，活动摘要改为 2×2 且筛选标签不再竖排，引导页三栏和任务详情检查器支持独立滚动，顶栏账号名称完整显示

[2026-07-21] v0.8.0-dev.41 fix(desktop): 授权状态收敛为本地已授权/未授权二态并删除启动远端权限探测；OAuth 回调不再同步等待用户资料，Windows 凭据增加进程缓存；20 秒后在线程维护 1.09 GB JSONL，自动同步延后 12 秒、每任务错峰 0.75 秒且最多 2 路并发；统一系统字体与 11/12/13/14px 字阶、行高、228px 侧栏、56px 顶栏和正文留白

[2026-07-21] v0.8.0-dev.40 fix(desktop): 修复云空间权限探测超时被误判为授权缺权并诱导重新扫码；授权状态改为正常/明确缺权/暂不可用三态，暂不可用时保留登录并每 5 秒重试；默认窗口和设计画布按 Codex Companion 调整为 1360x900、常规尺寸 1:1 渲染，并提升高频 10/11/12px 辅助字号

[2026-07-21] v0.8.0-dev.39 fix(desktop): 修复升级后首次打开可能长时间空白；调度器启动后保留 1 秒界面优先窗口，跨任务运行摘要写入串行化，双向任务运行中跳过重复上行扫描，首次扫描由逐文件 SQLite 查询改为单次批量映射查询并在线程中遍历目录，静态 HTML 在 React 接管前显示明确启动状态

[2026-07-15] v0.8.0-dev.38 fix(sync): 真实专用目录回归发现新建双向任务会在无删除补齐中回传刚下载的 Markdown 及其镜像资源；补齐前现冻结本地文件清单，云端优先下行后仅上传冻结清单，并禁止无删除下行刷新云端 Markdown 镜像，避免未修改内容重复转换、镜像刷新和文件 Token 替换

[2026-07-15] v0.8.0-dev.37 feat(safety): 新增任务状态恢复、五类运行配置档、快照只读与真实云端写保护、全局飞书令牌桶和脱敏 SQLite 快照/校验/基准工具；修复窗口版 Pydantic/FastAPI 收集与 Uvicorn 无控制台启动崩溃，桌面壳显示非生产环境，前端构建拆分为 5 个低于 500 kB 的 JS chunk

[2026-07-15] v0.8.0-dev.36 fix(frontend): 优化移除全局底栏后的总览页；工作台自适应填满剩余视口，将重复的快速操作与连接状态替换为任务状态和今日传输，最近同步扩展为 7 条，并保持主区与右栏三层严格对齐

[2026-07-15] v0.8.0-dev.35 fix(frontend): 统一桌面壳信息架构；删除与顶栏重复的全局底部状态栏，将运行状态、最近同步、系统入口和版本归入常驻侧栏，将顶部收敛为 52px 任务范围与操作栏，并为 Windows 原生标题栏应用克制的冷蓝灰 DWM 配色

[2026-07-15] v0.8.0-dev.34 fix(frontend): 校正活动页时间线圆点与竖线中轴；将应用内顶栏从 88px 压缩至 56px、底部状态栏从 78px 压缩至 48px，并用轻冷色顶栏区分 Windows 原生标题栏，在保留系统缩放与贴边能力的前提下释放 62px 工作区高度

[2026-07-15] v0.8.0-dev.33 fix(frontend): 按原始 07 号浅色设计稿重做活动与问题页；移除工作台顶部任务卡片条，恢复页头任务选择、276/568/416px 三栏、运行搜索、四类问题摘要、连续事件时间线和详细错误诊断，并完成三档完整可见性与交互验收

[2026-07-15] v0.8.0-dev.32 fix(frontend): 修正任务详情品牌 Logo 仍偏右及右缘裁切；将裁切视口由 205×97 扩为完整有效像素 214×97，并按两端文字、图标和按钮的真实渲染边界将同步关系整体左移 17px，三档左右间距误差降至 0.4px 以内

[2026-07-15] v0.8.0-dev.31 fix(frontend): 恢复任务详情正确的 LarkSync 品牌 Logo，保留已确认的对称箭头；将本地和云端内容收口为等宽 300px 容器并分别贴合左右外缘，使 Logo 到两个可见端点的三档间距误差均为 0px

[2026-07-15] v0.8.0-dev.30 fix(frontend): 纠正任务详情同步关系区误用品牌 Logo 的问题；按原始稿改为电脑端点、代码内双环同步符号、等宽双向连接线和实心映射健康状态，移除飞鸟与重复方向图标，并完成三档中心误差和连接线等宽验收

[2026-07-15] v0.8.0-dev.29 fix(frontend): 修正任务详情同步关系中央品牌图比例，以横版品牌源图的 205×97 视口等比裁出标识；同步任务表的项目名称新增可访问的详情入口，并完成 1536/1440/1280 三档点击链路、容器边界和横向溢出验收

[2026-07-15] v0.8.0-dev.28 fix(frontend): 按原始 `06-task-detail-light-v3` 恢复同步任务后的独立任务详情页；补齐演示任务详情数据、本地与云端关系、68% 当前运行、5 条历史和右侧五张独立卡片，详情“编辑策略”复用居中任务设置弹窗，并新增真实本地目录打开接口与三档内容溢出验收

[2026-07-15] v0.8.0-dev.27 fix(frontend): 以原始浅色稿重新完成同步任务页；恢复 8 行代表性任务、三种模式色彩、状态健康双层信息和固定操作顺序，将刷新与测试任务开关降级到表尾，开发环境默认演示且可用 `?ui-data=live` 切回真实数据，并消除 1440/1280 窗口中的内部空白与遮挡

[2026-07-15] v0.8.0-dev.26 fix(frontend): 纠正四页固定高度列中的卡片裁切与底部异色带；活动首屏按 5 次运行和 6 条事件完整排布，冲突、设置、维护重组内部容器，工作区底部与 78px 状态栏统一纯白，并新增三档窗口逐卡片边界验收

[2026-07-14] v0.8.0-dev.25 fix(frontend): 按满画布标准重做活动、冲突、设置和维护页；新增开发环境默认演示数据态与 `?ui-data=live` 真实数据开关，补齐典型运行、事件、冲突、规则和更新内容，四页工作台在 1536/1440/1280 下占满剩余高度且底边距不超过 23px
