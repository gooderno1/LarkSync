...
- 验证方式：
  ...
  - 第五轮 arm64 runner 证明安装版 App 已进入`webview_starting`且原生事件循环保持运行，但托管 runner 无交互式 WindowServer，60 秒内不会触发`loaded`；CI 降级仅在原生 PID 存活且停留该阶段时启用 headless Playwright WebKit，并继续断言 1080×720 首屏、二维码 ready、可见尺寸和 PNG data URL。
- 遗留问题：
