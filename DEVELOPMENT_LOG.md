...
## v0.9.15 正式发布（2026-09-16）

- 发布原因：
  - 交付托盘与问题中心的待处理统计统一修复，消除已结案历史冲突持续提示需要处理的问题。
- 实现方式：
  - 纳入 v0.9.15-dev.1；根项目、后端、前端及两个 lockfile 统一为 v0.9.15。
  - 通过正式标签构建 Windows NSIS、macOS Apple Silicon 与 Intel 安装包；发布说明依据本日志中的具体行为生成。
- 当前结果：
  - Windows 托盘与 macOS 菜单栏使用当前账号问题摘要；已解决、已忽略和路径排除问题退出待处理，其他未解决问题正常提示。
  - 冲突版本证据保留；历史运行错误仅作诊断，不再单独维持托盘错误状态；查询失败保持异常提示。
- 验证方式：
  - 本地完整后端 806 项通过、2 项平台跳过；前端 143 项、ESLint、TypeScript 与生产构建通过。
  - 已完成隔离 `npm run dev`、用户目录打包启动与实际接口 0→1→0 状态验证，详见 dev.1 记录。
  - 正式标签 v0.9.15 指向 `6afe8381f0e91e00d8b04238e04c9ba13604cce5`；标签流水线 `35088867807` 全部成功，后端 807 项通过、1 项平台跳过，前端 143 项通过。
  - 相同发布提交的 main 流水线 `35088862802` 全部成功，包含 macOS 双架构定向测试、打包及安装启动检查。
  - 正式 arm64 与 x86_64 DMG 均完成挂载、用户目录安装复制、版本与签名核验、后端健康检查；原生窗口检查返回 `stage=ui_verified`，Logo 解码和账号连接按钮可用。
  - 两种正式 Mac 包的实际 Keychain 长凭据写入、读取、刷新与清除均成功；发布沿用 ad-hoc 签名，首次安装的系统放行说明已写入 Release。
  - 三个安装包与三个独立 `.sha256` 文件共 6 个资产已上传；实际下载后的文件大小、SHA256、校验文件内容与 GitHub 资产 digest 全部一致。
  - Windows 安装包 58,858,240 bytes；SHA256：`650910e06ca5190e2c99bab83c6d7248e4f8d82011197140c845fd025f31f192`。
  - Apple Silicon DMG 66,320,140 bytes；SHA256：`d73460dd675df33dd15913c63b52e4f913db040236fe4f5f0d02eefc4e64ff38`。
  - Intel DMG 67,233,225 bytes；SHA256：`04999f8d1d3b2501c49b34b691a9861f1aaa908b285653b73819a4a6e4294ad4`。
  - Release 已公开并设为 Latest，`isDraft=false`、`isPrerelease=false`；回读正文与本地生成的完整变更、安装选择和三个校验值一致。
  - 发布地址：https://github.com/gooderno1/LarkSync/releases/tag/v0.9.15 。
  - 本机原 v0.9.14 安装保持运行；测试使用独立目录、端口与合成数据，测试进程已清理。

## v0.9.15-dev.1（2026-09-16）

- 开发原因：
  - v0.9.14 实机问题中心未解决数为 0，托盘仍读取 2 条原始未解决冲突，持续显示需要处理。
  - 两条冲突对应的依赖目录已被排除，统一问题记录按 `path_excluded` 结案；原始冲突保留版本证据及未执行覆盖的状态。
- 实现方式：
  - 保留问题主表与冲突详情分工；当前待处理统计统一复用 `ProblemService.get_summary()`，不修改历史冲突的覆盖结果。
  - 沿用请求账号上下文，只统计 `open,in_progress,waiting`；排除 `resolved,ignored`，未指定账号的托盘请求使用当前活动账号。
  - 桌面聚合接口新增 `problems.unresolved` 与 `problems.by_category`；原 `conflicts.unresolved` 改读相同摘要中的冲突分类数。
  - 托盘接口新增 `unresolved_problems`；该值大于 0 显示需要处理，为 0 按任务运行情况显示同步中或空闲。
  - `last_error` 保留为运行诊断信息，不参与当前待处理判断；冲突通知只使用统一摘要的未解决冲突数。
  - 摘要读取失败返回 HTTP 503；托盘接口不可达、缺少计数字段或轮询异常时保持异常提示，避免误报正常。
  - 状态读取只查询已持久化的问题摘要，不触发历史回填或问题结案写入；5 秒托盘轮询和 10 秒应用轮询沿用现有间隔。
- 当前结果：
  - Windows 托盘和 macOS 菜单栏的需要处理状态与当前账号问题中心一致，已结案、已忽略和路径已排除的历史冲突不再持续触发提醒。
  - 权限、上传、本地读写等非冲突问题也按当前未解决状态提示，切换账号后使用对应账号的问题统计。
  - 当前未解决问题为 0 时，历史失败文本不再使托盘持续报错；问题仍存在时，运行中的任务不会盖过待处理提示。
  - 原始冲突的版本、哈希、内容预览和实际覆盖结果保留；升级不会删除历史冲突，也不会执行本地或云端文件覆盖。
  - 问题状态查询失败时保持异常提示；后端状态接口不再遍历旧冲突队列，复用与问题中心相同的摘要。
- 验证方式：
  - 测试账号 A 的 2 条已结案冲突和 1 条已忽略冲突均不计入待处理，原始冲突与内容预览保留；账号 B 的 1 条冲突、1 条上传问题及 1 条本地读写问题统计为 3，账号 A 保持 0。
  - TDD：新增测试在旧实现下出现 8 项失败，覆盖旧冲突计数、历史错误、非冲突问题、查询失败及缺少统计字段；实现后定向回归通过。
  - 后端完整回归 808 项：806 项通过、2 项按平台条件跳过；托盘相关 21 项通过。
  - 前端全部 143 项测试、ESLint、TypeScript 与生产构建通过；Windows 静默安装异常路径 smoke 通过。
  - `npm run dev` 在独立端口 18135/13735、合成数据和禁止云端访问的配置下启动托盘、热重载后端与 Vite；前端入口正常。
  - 开发联调实际完成账号 A→B→B 忽略问题→A，问题摘要、桌面接口和托盘轮询标签分别为 0/空闲→1/需要处理→0/空闲→0/空闲；A 的 2 条旧冲突及预览全程保留。
  - Python 3.14.2 / Node 25.2.1 下执行 `python scripts/build_installer.py` 成功；独立用户目录安装复制后，托盘、打包后端及桌面窗口进程存活，静态页面正常。
  - 打包后端使用独立 18136 端口；同一组实际 API 与托盘轮询验证通过，运行状态确认 `packaged=true`、禁止云端写入、调度和监听已关闭。
  - 开发、打包与验证证据保存在本地 `data/diagnostics/tray-problems-20260916/`，不提交到 Git；未修改真实账号、任务、冲突记录或同步文件。

## v0.9.14 正式发布（2026-09-16）

- 发布原因：
  - 将通知全部已读和总览遗留冲突计数修复交付稳定渠道。
- 实现方式：
  - 汇总 v0.9.14-dev.1 与 v0.9.14-dev.2；统一根项目、后端、前端与两个 lockfile 的正式版本。
  - 通过 `v0.9.14` 标签触发 Windows NSIS 与 macOS Intel/Apple Silicon 构建及安装启动检查。
- 当前结果：
  - 通知全部已读不再受 500 条上限约束；总览和顶部待处理与当前账号的问题中心保持一致。
- 验证方式：
...
- 开发原因：
  - 问题中心已根据路径排除或恢复证据结案，总览仍读取旧 `conflicts` 表，并累加最近 200 条日志中的失败、创建和待删除事件。
  - 顶栏将旧冲突数量与失败任务数相加，也可能显示已处理事项。
- 实现方式：
  - 总览复用问题中心的账号范围、缓存键、失效通知和 10 秒轮询；只查询 `open,in_progress,waiting`，按严重程度与最近发生时间预览 1 条。
  - 总数读取 `summary.unresolved`，冲突读取未解决范围的 `by_category.conflict`，其他问题为两者之差；计数不受预览分页限制。
  - 顶栏同步读取相同问题摘要。历史事件保留在最近同步中，正常删除宽限队列不计入人工待处理问题。
  - 加载和请求失败显示未知状态，不将接口错误展示为健康或无问题；点击真实问题卡片进入问题中心。
- 当前结果：
  - 已解决、已忽略和路径排除后的旧冲突不再出现在总览优先处理；总览、顶栏和问题中心采用相同的当前账号未解决口径。
  - 测试账号 A 保留 4 条未更新终态的旧冲突、对应问题已结案时，总览显示 0；账号 B 的 1 条未解决问题正常显示 1。
- 验证方式：
  - 先新增回归测试，旧实现出现 4 项失败；修复后前端全部 143 项测试通过，ESLint 和 TypeScript 检查通过。
  - `npm run dev` 使用独立数据、文件凭据、端口 18118/13718 与 `synthetic_test`；实际切换账号显示 0 → 1，在问题中心忽略测试问题后顶栏立即归零，返回总览也显示 0。
  - 后端问题服务、问题接口、账号通知、多账号隔离和发布说明相关 53 项测试通过。
  - `python scripts/build_installer.py` 构建 Windows 独立目录产物成功；复制至测试安装目录运行，健康检查正常，4 条已排除旧冲突对应的总览、顶栏与问题摘要均为 0。
  - 未修改真实冲突记录、同步文件、任务或云端内容。

## v0.9.14-dev.1（2026-09-16）

- 开发原因：
  - 通知“全部标为已读”复用了分页查询，一次最多处理 500 条；超过上限时未读计数无法一次归零。
  - 通知抽屉没有处理数量和失败提示，已读状态仅靠浅色样式区分，容易被误认为操作未生效。
- 实现方式：
  - 批量已读改为单条 SQL UPDATE，仅更新 `read_at IS NULL` 的通知，不再应用通知列表的 500 条读取上限。
  - 传入 `account_id` 时仅更新该账号；未传账号的内部接口保持原有全账号语义。返回实际更新行数。
  - 保留通知内容与原有已读时间；重复执行返回 0；操作后新产生的通知仍为未读。
  - 前端固定本次操作的账号，完成后刷新该账号通知和账号摘要，显示处理数量、失败原因与逐条已读状态。
- 当前结果：
  - 当前账号超过 500 条未读通知也可一次全部标记已读；其他账号未读、已读时间及随后新通知均保留，界面明确显示处理数量和失败信息。
  - 一次操作可处理 1,125 条未读；重复操作返回 0，并保持其他账号未读数不变。
- 验证方式：
  - 修复前对 v0.9.13 本机积压 1,125 条通过原接口分批处理，返回 500、500、125、0；未删除通知。
  - 先以 1,001 条同账号未读和 2 条其他账号未读复现失败：旧实现仅返回 500。
  - 后端账号服务、账号接口及多账号隔离相关 13 项测试通过；覆盖账号范围、无账号范围、幂等、保留旧已读时间和后续新通知。
  - 前端通知抽屉、账号切换和侧边栏相关 5 项测试通过；ESLint、TypeScript 与生产构建通过。
  - 通过 `npm run dev` 在独立端口和 `synthetic_test` 数据目录启动，真实点击一次处理 1,125 条；账号 A 未读归零，账号 B 的 3 条未读保留。
...
- 开发原因：
  - 将依赖目录误同步修复交付给稳定版用户，减少 node_modules 引起的海量请求和目录容量错误。
- 实现方式：
...
## v0.9.13-dev.1（2026-09-16）

- 开发原因：
  - 实机问题中心的 2,087 条未解决项中，2,086 条来自同一项目的 node_modules；其中 2,081 条上传错误返回 1062507（parent node out of sibling num.）。
  - 另一任务也将含目录链接的 node_modules 纳入同步，本轮达到 25,113 个文件；原默认过滤仅包括点号路径和 __pycache__。
- 实现方式：
  - ignore_hidden_cache_paths=true 时，按不区分大小写的路径段匹配任意层级 node_modules；不按字符串前缀匹配，node_modules.md、my_node_modules、源码、包清单和锁文件仍参与同步。
  - 关闭默认过滤时允许同步该目录；任务 ignored_subpaths 仍独立生效，不自动清空或覆盖用户自定义规则。
  - 本地初始扫描、无删除补齐和目录移动使用 os.walk 提前剪枝，避免进入被排除的依赖目录及目录链接。
  - DriveService 接受单次扫描的目录过滤回调；同步下行和 upload_only 云端映射预填在查询子目录前过滤，快捷方式使用当前逻辑路径判断，保留未排除的同目标入口。
  - 下载候选先过滤再查询表格等导出子对象，避免被排除内容继续发起外部请求。
  - 上传、下载、删除联动与历史问题核验复用同一过滤规则；保留既有本地/云端依赖文件，历史问题按 path_excluded 证据结案。
  - 设置页文案和 README 明确默认规则的覆盖范围、开关与例外。
- 当前结果：
  - 默认排除任意层级的 node_modules，避免项目依赖被批量上传并反复触发飞书目录容量错误；Windows 与 macOS 使用相同规则。
  - 规则按完整路径段、不区分大小写匹配；node_modules.md、my_node_modules、源码、package.json 和锁文件继续参与同步。
  - 本地扫描在进入排除目录前停止递归；云端扫描在查询其子目录前跳过，下载候选也先过滤再查询导出子对象，减少无效遍历和 API 请求。
  - 上传、下载、删除联动与历史问题核验统一遵守过滤规则；已有本地和云端依赖文件保留，符合当前排除规则的历史问题自动结案。
  - 默认过滤沿用“忽略隐藏与缓存目录”开关；关闭后可重新同步 node_modules，任务中已设置的精确忽略项仍然独立生效。
  - 设置页与 README 已写明默认范围。此修复不改变飞书对其他目录的容量限制，也不包含 SQLite 写锁或历史数据库清理。
- 验证方式：
  - 已为用户授权的两个任务追加精确 node_modules 相对路径，配置原值和变更记录保存在本地 data/diagnostics/20260916-node-modules/，不提交用户路径和任务标识。
  - 安装版 API 连续超时且未落库后，停止卡住的应用，仅修改对应任务的 ignored_subpaths/updated_at 并重启；通过 API 回读确认两个任务仍启用且规则生效。
  - 应用自行将依赖目录问题结案；实机问题中心从 2,087 条未解决降至 1 条，剩余为历史测试任务的授权记录。
  - 本机已运行 v0.9.13-dev.1；健康与静态前端正常，登录保持连接，2 个账号和 9 个同步任务保留且 9 个任务均启用。
  - 部署后实际同步事件中 node_modules 路径为 0 条，正常业务文件继续上传；两个精确忽略项经运行接口回读确认。
  - TDD：默认目录过滤和云端剪枝的 6 项用例先失败，再实现修复；同时验证大小写、嵌套目录、显式忽略、关闭开关、普通文档保留和不进入排除目录。
  - 额外覆盖初始上传队列、下行候选、同目标快捷方式、剪枝后不触发删除、历史问题按当前规则结案。
  - 完整后端回归 792 项通过、2 项平台条件跳过；首次全量运行暴露两条新增用例继承前序全局配置，显式隔离默认开关后重跑全量通过。
  - 前端 ESLint、设置面板 3 项测试、TypeScript 和 Vite 生产构建通过。
  - npm run dev 在独立 synthetic_test 数据目录启动托盘、热重载后端与 Vite；后端健康、运行配置和前端入口均验证成功。
  - Python 3.14.2 / Node 25.2.1 下完成 PyInstaller 与 NSIS 构建；安装包大小 72,483,422 bytes，SHA256 为 da74b469dcec530831399e4c24d6764f5470f1325e3cfb0053f8eb1c22967b25。
  - 打包产物复制部署到独立用户目录，真实托盘、后端和桌面窗口进程存活；健康接口、静态页面、版本 v0.9.13-dev.1 和默认过滤开关均通过，测试环境禁止云端写入并禁用调度与监听。
  - 本机更新使用已验证的 onedir 程序文件覆盖；提前备份旧程序和完整 SQLite 状态库，构建产物不含运行 data 目录，不执行卸载或云端文件清理。
- 遗留问题：
  - 排除规则只处理依赖目录引起的同步膨胀，不改变飞书对其他正常目录的容量限制；本次不清理已上传文件或历史数据库。
  - 之前观察到的 SQLite 写锁和冗余日志回填仍需要独立排查，不能据此宣称所有同步错误均已消除。

## v0.9.12 正式发布（2026-09-07）

- 开发原因：
  - 交付 macOS 钥匙串反复弹窗修复；用户删除应用后重装仍受影响，不能将重新安装当作凭据和授权已清空的证据。
- 实现方式：
  - 合并 `v0.9.12-dev.2` 的原生钥匙串非交互访问、失败暂停、主动恢复、Token 单条目原位更新和旧格式兼容迁移。
...
## v0.9.12-dev.2（2026-09-07）

- 开发原因：
  - macOS 用户反馈最新版在“始终允许”后仍反复显示 `larksync` 钥匙串访问提示。
  - 删除并重装应用只替换 `.app`；钥匙串条目和用户数据不随应用包删除，不能据此排除历史授权影响。
  - 旧实现跨平台共用 900 字符 Token 分片；模拟 4,200/4,600 字符凭据产生 15 个同服务条目。
  - keyring 的 macOS 更新会删除后新建条目；读取失败没有暂停状态，账号摘要每 10 秒查询会继续触发系统访问。
- 实现方式：
  - 新增 macOS Security 原生适配器；后台使用禁止交互查询，原位更新精确 service/account 对应的值，不存在时才新增。
  - 针对文件型登录钥匙串忽略 SecItem 交互参数的限制，在进程锁内同时临时禁止系统交互；成功、失败和用户取消都恢复原值，不改变系统或其他进程的安全策略。
  - 系统访问失败后按进程暂停；只有用户点击“重试钥匙串访问”才在工作线程恢复系统交互。
  - 新增 `/auth/keychain/status` 与 `/auth/keychain/retry`；状态接口只读内存，前端每 3 秒观察 macOS 状态，Windows 首次确认不支持后停止轮询。
  - macOS Token 单条目保存，兼容旧分片、拆分和合并格式；新格式损坏时拒绝回退到旧 Token。
  - App Secret 首次读取后缓存，显式重试时重载；账号迁移不再读取已被账号凭据替代的全局旧条目，也不重复保存未变化 Token。
  - 注销先清除旧格式再删除新格式，避免重启后旧凭据恢复登录；Windows 保留原分片存储。
  - 安装冒烟使用实际 macOS TokenStore 验证长凭据、更新、重载和清除，Security 桥接依赖纳入平台依赖与打包。
- 当前结果：
  - 后台授权失败不会继续唤起系统弹窗；页面提供明确恢复操作，并保留账号、任务、文件和数据库。
  - 每账号的长 Token 以单条钥匙串记录保存，刷新时原位更新；已有 App Secret 不再随着账号摘要查询重复读取。
  - 旧格式可自动复制迁移；已存在账号凭据时不再访问全局旧 Token，减少重装和升级后的多次授权请求。
  - 主动恢复只在点击按钮后触发；取消授权后维持暂停，允许用户再次重试，不自动重复弹窗。
  - 现有 ad-hoc 安装包跨构建升级可能仍需要一次主动授权；当前开发环境为 Windows，真实 Mac 系统弹窗需现场复核。
  - 既有 Windows 符号链接测试在当前普通用户下触发 WinError 1314；仅此权限不足场景跳过，macOS 仍强制执行。
- 验证方式：
  - TDD：原生接口与单条目存储测试先失败，再实现并验证缓存、迁移、失败暂停和恢复行为。
  - 前端 ESLint、43 个测试文件共 137 项测试、TypeScript 和生产构建通过。
  - 后端完整回归收集 774 项：772 项通过、1 项按既有平台条件跳过；唯一失败是当前 Windows 用户无符号链接权限。修正该测试的精确跳过条件后，相关 50 项回归为 49 项通过、1 项跳过。
  - Python editable 安装检查通过；使用 Python 3.14.2、Node 25.2.1 构建 Windows PyInstaller 与 NSIS `v0.9.12-dev.2` 安装包成功。
  - Windows 打包后端在独立 `18143` 端口、隔离 synthetic_test 数据目录启动；健康检查、空账号摘要和非 macOS 钥匙串状态接口通过。
  - 隔离 `synthetic_test` 配置通过 `npm run dev` 启动托盘、热重载后端与 Vite；首次连接页正常。
  - 浏览器使用生产构建与模拟状态接口验证：初始暂停、取消授权保持暂停、再次主动重试后恢复页面；未接触真实飞书凭据。
  - 最终修复提交 `3b32f1e` 的 `main` 流水线 `34095887805` 全部通过；Windows 后端 775 项通过、1 项平台跳过，macOS 双架构完成实际 Keychain 长凭据保存、刷新、重载和清除验证。
- 遗留问题：
  - 当前工作机为 Windows；真实 macOS 弹窗、跨版本 ACL 与用户现场行为需在 Mac 验收，不能用模拟测试替代。
  - 当前发布凭据未配置 Developer ID；ad-hoc 构建升级仍可能需要一次用户主动重新授权。
...
- 开发原因：
  - 将 `v0.9.11-dev.1` 与 `v0.9.11-dev.2` 完成的 macOS 桌面适配正式交付，解决绑定后 Logo 缺失、Dock/应用切换器出现两个 LarkSync、重复启动新增窗口以及全屏关闭留下黑色 Space 的问题。
- 实现方式：
  - 正式版冻结已验证的静态资源符号链接解析、父进程 Accessory 与窗口子进程 Regular 激活策略、跨进程单窗口控制、Cocoa 全屏隐藏与重新打开恢复逻辑。
  - 保留 `v0.9.11-dev.1` 的 Cocoa 主线程调度、完整退出清理、系统 `hdiutil` DMG 回退和当前 Device Flow 安装冒烟。
  - Release 正文生成器兼容开发日志现行的中文全角日期括号，确保稳定版归档包含每个开发版本的升级重点和详细结果。
  - 依赖 `/bin/bash` 的 `hdiutil` 回退脚本集成测试标记为仅在 macOS 执行；Windows 继续通过平台无关单元测试覆盖构建调度，并由 macOS 双架构任务验证 DMG shell 链路。
...
- 开发原因：
  - 将已完成自动化、双视口视觉和 Windows 打包验证的设置页与更新维护页对齐方案正式交付给自动更新用户。
- 实现方式：
  - 正式版冻结 `v0.9.10-dev.1` 的共享网格布局：设置页账号与设备共享首行，默认同步策略跨越左侧应用配置和忽略规则两行；更新维护页双主面板按自然内容高度拉伸对齐。
  - 保留 900px 单栏断点和页面整体自然滚动，不引入固定视口高度、栏内滚动、底部锚定或无意义填充卡片。
...
## v0.9.10-dev.1（2026-09-03）

- 开发原因：
  - 设置页左右栏虽然总高度接近，但两栏各自排版导致“账号管理”与“当前设备”高度不同，第二排起始线相差约 78px。
  - 更新维护页外层使用 `items-start`，1360×900 真实数据下左右主面板底边相差约 55px。
- 实现方式：
  - 设置页改为两列三行的共享网格：账号管理与当前设备共享第一行；应用配置、忽略规则位于左侧第二、三行；默认同步策略在右侧跨越第二、三行。
  - 更新维护页保留 3:2 列宽和自然页面高度，改为拉伸同一网格行中的两个主面板。
  - 两页继续在逻辑宽度低于 900px 时回退单列；未增加固定视口高度、独立滚动区、底部锚定或填充型静态卡片。
- 当前结果：
  - 1360×900 下设置页账号与设备卡均为 325px，顶边和底边误差均为 0；两侧最终底边均为 766px。
  - 1080×720 下设置页账号与设备卡均为 258px，两侧最终底边均为 602px；页面横向溢出为 0。
  - 1360×900 下更新页左右主面板均为 641px；1080×720 下均为 509px，两个视口的顶边、底边误差均为 0，横向溢出为 0。
  - 本次只调整前端布局和展示顺序，不变更账号、凭据、授权、任务、更新服务、数据库 schema 或持久化数据；无需迁移和重新授权。
- 验证方式：
  - 测试先行：页面结构断言在实现前按预期失败，实现共享网格和拉伸规则后通过。
  - 前端 42 个测试文件共 134 项通过；TypeScript、ESLint 和 Vite 生产构建通过。
  - 后端全量 738 项 pytest 通过。
  - 使用本机真实数据在 1360×900 与 1080×720 测量关键区域矩形和横向溢出；800×720 单列布局按账号、设备、应用、策略、忽略规则的顺序展示，横向溢出为 0。
  - `python scripts/build_installer.py --nsis` 通过；生成 `LarkSync-Setup-v0.9.10-dev.1.exe`，大小 72,485,764 bytes，SHA256 为 `6C4F2818137C0463D93212C165B6C5249533D01AABBA2D5CF511EF863922A0AA`。
  - 打包后端使用独立 `synthetic_test` 数据目录、file Token Store、端口 18212 与锁端口 48932 启动；`/health` 返回 `ok`，静态前端 HTTP 200，桌面状态返回 `packaged=true`、版本 `v0.9.10-dev.1`，关闭接口返回 `shutting_down` 且端口正常释放。
- 遗留问题：
  - 本次为开发版本，不创建正式 Release；需用户明确要求后再执行正式发布。

## v0.9.9 正式发布（2026-09-03）

- 开发原因：
  - 将已完成自动化、视觉和 Windows 打包验证的设置页双栏重配正式交付给自动更新用户。
- 实现方式：
  - 正式版冻结 `v0.9.9-dev.1` 的设置页信息架构：左栏为账号管理、应用配置与忽略规则，右栏为当前设备、开机自启动与默认同步策略。
  - 删除无操作价值的数据保护静态卡；既有 Token 存储、冲突副本和云端事实来源机制保持不变。
  - 后端、前端、根项目及两个 lockfile 的版本元数据统一更新为 `v0.9.9`。
  - 正式 Tag 触发 `Release Build`，执行质量门、Windows 静默安装、WebKit 登录流程、Windows NSIS 和 macOS 双架构构建及安装启动冒烟。
- 当前结果：
  - 正式 Tag `v0.9.9` 指向提交 `555f76bef6c86e835d96341c62bab3f437956d84`，该提交同时位于 `main`。
  - GitHub Release 已公开并设为 Latest：`https://github.com/gooderno1/LarkSync/releases/tag/v0.9.9`；不是草稿或预发布。
  - Windows 安装包 `LarkSync-Setup-v0.9.9.exe` 大小为 58,620,823 bytes，SHA256 为 `3281473BFAE15D6BC75A44FFD8325AC9652B237AFF7573276F9A50F7A28D4995`。
  - Apple Silicon 安装包 `LarkSync-v0.9.9-arm64.dmg` 大小为 65,675,952 bytes，SHA256 为 `9545725979D7EB3DA31A273D736F411CECDBC09D5195760A19A53F37AB1B9A64`。
  - Intel 安装包 `LarkSync-v0.9.9-x86_64.dmg` 大小为 66,639,901 bytes，SHA256 为 `FCC4BF0D67EDC6D3247CF940A96A3B7330DB9ECB8B4375AF0B0506DE306DE77B`。
  - 三个平台安装包及三个独立 `.sha256` 文件共六个资产均为 `uploaded`；Release 正文已收口为三平台完整校验表。
- 验证方式：
  - 发布前 `v0.9.9-dev.1` 已通过前端 134 项测试、后端 738 项测试、TypeScript、ESLint、Vite 生产构建、双视口真实数据视觉检查、Windows NSIS 构建和打包后端启动冒烟。
  - Tag 流水线 `33719479768` 成功；质量门、Windows 静默安装、WebKit 两次扫码、Windows NSIS、macOS arm64/x86_64 构建、两类 macOS 安装启动冒烟及资产上传全部通过。
  - 同一提交的 `main` 流水线 `33719476620` 也成功；非 Tag 分支按条件跳过正式资产上传，但完成了质量门、WebKit 和 macOS 打包验证。
  - 三个 `.sha256` 文件已下载到隔离目录核对，内容分别与对应安装包的 GitHub 资产 digest 一致。
- 遗留问题：
  - 仓库未配置 Apple Developer ID 与公证凭据；macOS DMG 使用 ad-hoc 签名，首次打开可能需要在系统设置中手动放行。

## v0.9.9-dev.1（2026-09-03）

- 开发原因：
  - 应用配置默认收起后，右栏仍同时承载设备、应用、忽略规则和数据保护，左右栏在主题数量与操作密度上不平衡。
  - 数据保护卡只有三条静态说明，没有设置项或可执行操作，占用空间但不能帮助用户完成设置。
- 实现方式：
  - 设置页按“账号、应用与文件范围｜本机与同步行为”重组：左栏依次为账号管理、应用配置、忽略规则；右栏依次为当前设备及开机自启动、默认同步策略。
  - 应用配置保留 v0.9.8 的默认收起、按需展开、应用改名和官方后台入口；本次只改变所属栏与页面顺序。
  - 删除数据保护静态卡及未再使用的图标导入；Token 系统凭证库、云端事实来源和冲突副本等既有保护机制不变。
  - 双栏保持接近等宽、自然高度和 900px 单栏断点，不增加强制等高或内部滚动。
- 当前结果：
  - 默认设置页的左右栏主题职责更清楚；应用配置与账号归属相邻，设备和同步行为集中在右栏。
  - 页面没有为了补高度保留低价值卡片；应用列表展开时仅由左栏按内容自然增长。
  - 本次不变更 API、数据库 schema、账号、凭据、任务、授权或同步行为，无需数据迁移或重新授权。
- 验证方式：
  - 前端 42 个测试文件共 134 项通过；静态渲染测试覆盖账号、应用、忽略规则、设备、策略的顺序，并断言数据保护卡不再存在。
  - TypeScript、ESLint 和 Vite 生产构建通过。
  - 后端在生产行为下使用项目内隔离数据、临时文件和凭据目录完成全量 738 项 pytest，无失败。
  - 1360×900 真实数据默认收起态左栏约 526px、右栏约 565px，高度差约 39px；1080×720 左栏约 418px、右栏约 449px，高度差约 31px。两个视口横向溢出均为 0，浏览器控制台无 error。
  - `python scripts/build_installer.py --nsis` 通过；生成的 `LarkSync-Setup-v0.9.9-dev.1.exe` 大小为 72,484,754 bytes，SHA256 为 `CAE04FD44AC5BAC4B1CB4723F4384A489577D51A613DE8BAA0AD73A7B2D03737`。
  - 打包后端使用独立 `synthetic_test` 数据目录、file Token Store、端口 18211 与锁端口 48931 启动；`/health` 返回 `ok`，静态前端 HTTP 200，桌面状态返回 `packaged=true`、版本 `v0.9.9-dev.1`，关闭接口返回 `shutting_down` 且进程正常退出。
- 遗留问题：
  - 本次为开发版本，不创建正式 Release；需用户明确要求后再执行正式发布。

## v0.9.8 正式发布（2026-09-03）

- 开发原因：
  - 将已完成自动化、视觉和桌面打包验证的设置页应用配置收纳方案交付给自动更新用户。
- 实现方式：
  - 正式版冻结 `v0.9.8-dev.1` 的应用配置默认收起、紧凑展开列表、官方应用管理入口和接近等宽双栏。
  - 后端、前端、根项目和 lockfile 版本统一更新为 `v0.9.8`；README 的稳定版本与功能说明同步更新。
  - 正式 Tag 触发 `Release Build`，执行后端/前端质量门、Windows 静默安装、WebKit 登录流程、Windows NSIS 和 macOS arm64/x86_64 构建并上传校验文件。
- 当前结果：
...
## v0.9.8-dev.1 (2026-09-03)

- 开发原因：
  - 应用配置默认展开全部应用，应用数量直接拉长设置页右栏；账号卡默认收起后，左右栏高度和信息密度再次失衡。
  - 应用配置只有本地改名，用户无法从 LarkSync 直接进入对应应用的官方开发者后台。
- 实现方式：
  - 应用配置默认只显示应用总数、正在使用的应用数和可恢复账号总数；具体应用 DOM 仅在用户点击“展开应用”后渲染。
  - 展开区从嵌套卡片改为分隔行；保留别名、脱敏 App ID、来源、创建时间、绑定数和可恢复数，并把“应用管理”与“改名”作为两个明确操作。
  - 飞书应用跳转到 `https://open.feishu.cn/app/{app_id}`，Lark 应用跳转到 `https://open.larksuite.com/app/{app_id}`；仅传递编码后的 App ID，不传递 Secret、Token 或账号信息。
  - 设置页桌面网格从约 `7:5` 调整为接近 `1:1`；低于 900px 继续回退单栏，仍使用自然高度且不增加内部滚动。
- 当前结果：
  - 默认设置页不再被历史或未绑定应用占满，账号/策略与设备/应用/规则在首屏中的视觉重量更接近。
  - 用户可以按需展开应用列表，并通过对应行直接进入官方开发者后台继续管理该 App ID。
  - 账号、应用、凭据、授权、同步、数据隔离和数据库 schema 均未改变，不需要数据迁移。
- 验证方式：
  - 前端 42 个测试文件共 134 项通过；新增用例覆盖默认收起、展开/再次收起、空状态、本地改名回调，以及飞书和 Lark 两类官方应用管理地址。
  - TypeScript、ESLint 与 Vite 生产构建通过；后端全量 738 项 pytest 通过。
  - 使用本机 v0.9.7 三个应用配置的真实只读数据，在 1360×900 和 1080×720 完成默认收起视觉检查；1360×900 另验收展开态，三个应用管理链接均携带对应 App ID，页面横向溢出为 0。
  - ImageGen 布局基线及三张真实实现截图保存在 `docs/local_specs/design_artifacts/v0.9.8/`；该目录属于本地产品资料，不进入 Git。
  - `python scripts/build_installer.py --nsis` 通过，生成 `LarkSync-Setup-v0.9.8-dev.1.exe`，大小 `72,483,725 bytes`，SHA256 为 `EDD8591C7376DE23E78CC7665C86E004C43C40B79F57A6FC81F5C9D9AB5CA2CF`。
  - 打包后的 `LarkSync.exe --backend` 使用独立 `synthetic_test` 数据目录、file Token Store、端口 `18210` 与锁端口 `48930` 启动；`/health` 返回 `ok`，桌面状态返回 `runtime.packaged=true`、`update.current_version=v0.9.8-dev.1`、`profile=synthetic_test`，静态前端 HTTP 200，关闭接口返回 `shutting_down` 且进程和端口均已退出。
- 遗留问题：
  - “应用管理”只负责打开官方后台；权限审批、发布、删除与官方应用名称修改仍由用户在飞书/Lark 开发者后台完成。
  - 本次为开发版本，不创建正式 Release。

## v0.9.7 正式发布（2026-09-02）

- 开发原因：
  - 多账号默认展开导致设置页形成按钮墙，原生移除确认框与应用视觉不一致，左栏拥挤而右栏仍有空位。
  - 自动注册应用在 LarkSync 内都显示同名，用户重新绑定软移除账号时难以辨认原应用配置。
  - 历史默认授权同时请求可被读写权限覆盖的只读 scope 和仅用于身份展示的通讯录 scope，增加审批概率但不影响核心同步。
- 实现方式：
  - 账号卡改为默认全部收起、单卡展开；摘要直接显示组织、成员、同步状态、未读数、应用别名和 App ID 尾号，授权与风险操作仅在展开后出现。
  - 移除账号复用应用内确认弹窗并明确“软移除、可恢复、不删除任务与历史”；组织名和应用别名改为统一的应用内编辑弹窗，支持键盘与错误反馈。
  - 应用配置增加本地可编辑别名、来源、创建时间、脱敏 App ID、当前绑定账号数和可恢复账号数；自动注册第一次扫码前允许输入应用别名，未修改默认名时会在第二次授权后按本地组织名生成更易辨认的别名。
  - 同一 `app_profile_id + open_id` 再次授权会恢复被软移除的原账号，保留 `account_id`、任务、状态、映射和历史；不同应用或仅名称相同不会自动合并。
  - 新授权默认 scope 收敛为 `drive:drive`、`docx:document` 和 `docx:document.block:convert`；历史 `docs:doc` 继续兼容迁移，旧默认配置中的重复只读和通讯录 scope 会被忽略。
  - Device Flow 换取 Token 后按响应中的实际 scope 核验三项核心权限；权限不全时返回缺项列表，不保存账号，也不误显示授权成功。
  - 设置页使用约 `7:5` 双栏：账号管理与默认同步策略在左，当前设备/开机自启动、应用配置、忽略规则和数据保护在右；移除重复的当前账号大卡。
- 当前结果：
  - 多账号场景可以先扫描摘要再按需展开，当前账号、未读通知、同步状态与应用归属无需展开即可辨认。
...
- 开发原因：
  - 扫码登录的目标是避免用户进入飞书开发者后台；仅为显示组织名称要求 `tenant:tenant:readonly`、权限扫码或管理员审批，操作成本与实际收益不匹配。
  - 无官方组织信息的账号使用 `account.id` 前缀作为名称，出现“组织 · legacy”“组织 · 621fe8”，既不稳定也无法帮助同一成员区分不同组织。
  - 旧账号缺少本地组织名称；新账号授权成功后也没有就地命名入口。
- 实现方式：
  - 无额外权限的用户信息响应会把 `tenant_key` 保存为内部组织标识；账号、凭据、任务、状态、通知与同步数据仍严格以 `account_id` 隔离，显示名不参与鉴权或数据归属判断。
  - 新账号没有官方组织名时按品牌和现存名称分配持久化预设“飞书组织 N”或“Lark 组织 N”；重复登录、重新授权和账号排序不会自动重编号。
  - 新账号授权成功页新增组织名称输入框；用户可当场修改，点击“进入该组织”时先保存名称，再刷新账号并切换。保存失败时保留授权成功事实和当前页面，不回滚凭据。
  - schema v13 在升级前沿用统一 SQLite 自动备份；按创建顺序只为 `account_alias` 与 `tenant_name` 同时为空的未移除账号补齐预设名，并为每个受影响账号创建一次 `organization_name_migration` 通知。已有别名和官方名不覆盖，重复启动不重复通知。
  - 设置页不再显示“扫码开通权限”“组织信息与权限”或开发者后台引导，只保留“修改组织名称”；官方元数据服务和兼容字段保留，已有权限时可以静默补全，缺权不影响登录与同步。
  - 组织名称只读兜底改为“飞书组织 / Lark 组织”，不再暴露账号 UUID 片段。
- 当前结果：
  - 新用户完成原有 Device Flow 扫码即可获得可辨识、可编辑的组织名称，无需为此增加任何飞书权限配置。
  - v0.9.5 及更早账号在首次启动 v0.9.6-dev.1 时自动完成名称迁移，并在对应账号收到一次可改名通知。
  - 详细方案保存在 `docs/local_specs/LarkSync v0.9.6 零权限组织命名与升级通知详细修改方案.md`；创建时间 `2026-09-02 01:07:55 +08:00`，实施记录更新时间 `2026-09-02 01:29:22 +08:00`。
- 验证方式：
  - 后端全量 pytest 执行至 `100%` 并通过；覆盖新账号预设名、官方名保留、`tenant_key` 新登录与重新授权持久化、清空别名安全回退，以及 schema v13 自动迁移和通知幂等。
  - 前端 40 个测试文件共 130 项通过；TypeScript、ESLint 与 Vite 生产构建通过。
  - 使用本机 v0.9.5 双账号只读数据在 `1365×900` 与 `1080×720` 完成 Edge 渲染检查；设置页横向溢出为 0，权限扫码入口不可见，账号卡显示本地组织名称状态。模拟完整两次扫码后，授权成功页正确显示预设“飞书组织 3”的可编辑输入框且横向溢出为 0。
  - `python scripts/build_installer.py --nsis` 通过，生成 `LarkSync-Setup-v0.9.6-dev.1.exe`，大小 `72,455,141 bytes`，SHA256 为 `CA6E32C7FFC07ADC8F79CDE66639EDE41739355149CB9013A29B2E11054FC6B4`。
  - 打包后的 `LarkSync.exe --backend` 使用独立 `synthetic_test` 数据目录、file Token Store、端口 `18207` 与实例锁端口 `48903` 启动；`/health` 返回 `ok`，桌面状态返回 `packaged=true`、`current_version=v0.9.6-dev.1`，SQLite `schema_version=13`。测试进程已停止。
- 遗留问题：
  - 本次交付为开发版本提交，不创建正式 Release；需用户明确要求后再执行正式版本、Tag 和公开 Release 流程。
...
- 开发原因：
  - v0.9.4 对组织接口真实错误码 `99991672` 识别失败，两个 Device Flow V2 账号均被显示为“等待获取组织信息”，用户没有可执行的权限开通入口。
  - 侧边栏把当前组织和展开列表分别做成白色阴影卡片，与工作区导航的扁平活动态不一致；账号卡底部自由换行导致当前账号与非当前账号的操作位置不同。
- 实现方式：
  - 后端从 `error.permission_violations[].subject` 读取缺少的 scope；仅当错误码为 `99991672` 且缺少 `tenant:tenant:readonly` 时进入 `permission_required`。
  - 权限地址只接受 HTTPS、飞书/Lark 官方主机、当前 App ID 的 `/app/<app_id>/auth` 路径、组织只读 scope 和 `token_type=tenant` 五项同时匹配的 URL；未通过校验的地址不会生成二维码。
  - schema v12 新增 `tenant_metadata_error_code` 与 `tenant_permission_url`；升级前沿用统一 SQLite 自动备份，迁移由应用启动自动完成，无需手动操作。
  - 设置页使用经过后端校验的官方 URL 生成二维码；弹窗明确提示这不是账号重新登录，页面可见时每 5 秒复检，3 分钟后停止自动轮询，同时保留手动检查和浏览器入口。
  - 官方错误 `1184001` 单独归类为 `unavailable`；页面说明账号与同步仍可使用，不再把它误判为可扫码解决的缺权。
  - 侧边栏宽度调整为 `244px`，当前组织复用导航活动态和左侧强调线；展开列表使用同一浅色导航面，通知与当前组织同行，添加入口降低视觉权重。
  - 账号卡固定为“成员身份—组织权限—同步/授权事实—账号操作”四层；刷新、重新授权、暂停为固定三列，断开与移除进入“更多操作”。
- 当前结果：
  - 缺少组织只读权限时可扫码打开当前应用的官方权限页，权限生效后自动刷新组织名称与 Logo；账号 Token、任务和同步状态不发生变化。
  - 组织接口不可用或权限地址不可信时保持明确降级，不显示可误导用户的二维码。
  - 双账号真实数据在 `1365×900` 视口中渲染为 `244px` 扁平侧边栏；展开组织列表位于栏内，设置页两张账号卡结构一致且底部操作不换行。
  - 详细方案保存在 `docs/local_specs/LarkSync v0.9.5 组织权限扫码与账号界面一致性详细修改方案.md`；创建时间 `2026-09-01 23:09:26 +08:00`，实施记录更新时间 `2026-09-01 23:29:34 +08:00`。
- 验证方式：
  - 后端 pytest 全量执行至 `100%`并通过；覆盖真实缺权响应、安全 URL 拒绝、`1184001` 降级、成功后诊断清理与 schema v12 自动迁移。
  - 前端 40 个测试文件共 128 项通过；TypeScript、ESLint 和 Vite 生产构建全部通过。
  - 前端覆盖权限二维码、立即复检成功、侧边栏视觉契约和设置页账号卡结构；另以本机 v0.9.4 双账号只读数据在 `1080×720`、`1280×800`、`1440×900`、`1920×1080` 完成 Edge 截图检查，四档页面、侧边栏与账号卡横向溢出均为 0。
  - `python scripts/build_installer.py --nsis`通过，生成 `LarkSync-Setup-v0.9.5-dev.1.exe`，大小 `60,753,206 bytes`，SHA256 为 `F2331DB9F5B052AB7707A329E569B5F66916DE23DEE7707DC9A42ACC9EEECD20`。
  - 打包后的 `LarkSync.exe --backend`在独立 `synthetic_test` 数据目录与端口 `18206`启动；`/health`返回 `ok`，桌面状态返回 `packaged=true`、`current_version=v0.9.5-dev.1`，SQLite `schema_version=12`。
  - 打包 smoke 进程已停止；测试目录已移入系统回收站，可恢复。
- 遗留问题：
  - 扫码后的飞书官方页面是否允许当前用户直接开通，取决于应用管理权、租户管理员审批和应用版本状态；LarkSync 不会绕过官方审批。
  - 正式版发布、安装包哈希与完整质量门禁结果将在 v0.9.5 Release 完成后补记。
...
- 开发原因：
  - 自动创建应用与账号授权连续替换二维码，第一次扫码成功后缺少稳定检查点，用户难以确认两次扫码的用途与当前步骤。
  - 多账号前端查询依赖可变的`localStorage`活动账号；切换期间的迟到响应、共享 Query Key 和未过滤 WebSocket 可能把旧账号数据带入新账号页面。
  - 账号入口与设置卡以成员姓名为主，无法清晰区分同一成员所在的不同组织，也没有官方组织 Logo、组织信息降级和本地显示名能力。
- 实现方式：
  - 后端把 App Registration 与账号 Device Session 拆开；第一次扫码成功只返回并保存`AppProfile`，用户点击“继续第 2 次扫码”后才创建账号授权会话。
  - 已有应用、手动应用和重新授权保持单次扫码；最终授权成功页持续显示`Device Flow V2`、凭据保存状态和“进入该组织”，点击后才切换账号。
  - `AccountProvider`改为显式活动账号状态和串行切换队列；任务、状态、日志、云目录、问题、冲突、通知、诊断和写操作使用账号化 Query Key、显式请求头与取消信号。
  - WebSocket 增加`account_id`订阅；服务端只向匹配账号连接广播，旧客户端未指定账号时继续走兼容通道。
  - schema v11 为账号增加`tenant_key`、企业编号、组织类型、官方头像、账号隔离缓存路径、元数据状态、更新时间和本地别名；升级自动备份并迁移，无需用户手动处理。
  - 授权成功后、旧账号后台回填和用户手动刷新时，使用应用身份读取官方`tenant/v2/tenant/query`；缺少`tenant:tenant:readonly`时记为`permission_required`，不改变账号连接状态，不触发重复扫码。
  - 官方组织头像限制为 HTTPS、2 MiB 和 PNG/JPEG/WebP，缓存到`data/accounts/<account_id>/tenant/`；图片失败时使用 CSS 组织首字占位。
  - 侧边栏顶部改为栏内展开的组织列表，组织名为主、成员名为次，每行独立显示未读消息和同步错误；设置页增加组织信息刷新与本地显示名编辑，开机自启动保持在当前账号下方。
  - 官方快速注册协议没有公开应用改名字段；LarkSync 内部显示名统一为`LarkSync`，飞书后台可能显示“CLI应用”的事实提示保留，不调用私有接口或浏览器自动化改名。
- 当前结果：
  - 双账号页面数据和实时事件具备前后端双重隔离；跨账号任务与冲突主键读取返回不存在。
  - 快速执行 A→B→A 时切换请求按顺序完成，最终前端状态、后端活动账号与本地偏好均保持 A；切换失败时继续停留在原账号。
  - 组织读取失败不影响登录、同步或其他账号后台任务；用户可用本地组织显示名区分无权限账号。
  - 本地详细方案保存在`docs/local_specs/LarkSync v0.9.4 多账号体验、数据隔离与组织化账号导航详细修改方案.md`；方案创建时间`2026-09-01 19:45:35 +08:00`，实现完成时间`2026-09-01 20:54:56 +08:00`。
- 验证方式：
  - 后端全量`python -m pytest -q`通过，共 729 项；覆盖两阶段授权、schema v11、组织元数据成功/无权限降级、Logo 缓存、WebSocket 账号过滤和跨账号主键拒绝。
  - 前端 38 个测试文件共 125 项通过；覆盖两次扫码检查点、单扫码路径、授权成功页、显式账号请求头、快速切换竞态、切换失败和组织头像降级。
  - `npm run lint`、TypeScript 检查与 Vite 生产构建全部通过。
  - Edge Playwright 使用当前双账号只读数据完成`1080×720`、`1280×800`、`1440×900`和`1920×1080`验收；四档均无横向溢出，侧边栏账号列表完全位于侧边栏边界内，当前账号下方可见开机自启动，账号卡可见组织信息操作。
  - `python scripts/build_installer.py --nsis`通过，生成`LarkSync-Setup-v0.9.4-dev.1.exe`，大小`72,430,782 bytes`，SHA256 为`45EC704EA711A7C18D0B39F52D89DD48729BED721EFD048E5C26186303226E66`。
  - 打包后的`LarkSync.exe --backend`使用独立`synthetic_test`数据目录、端口`18204`和实例锁端口`48924`启动；`/health`返回`ok`，桌面状态返回`packaged=true`、`profile=synthetic_test`和`current_version=v0.9.4-dev.1`。
  - 打包 smoke 进程、监听端口和隔离数据目录均已清理；隔离目录与视觉截图采用可恢复方式移入系统回收站。
- 遗留问题：
  - 真实飞书两次扫码和组织元数据权限结果仍需用户在正式版自动更新后验收；自动化测试不代替本人扫码。
  - 官方快速注册应用在飞书后台仍可能显示“CLI应用”；当前公开协议没有改名字段，本版本按确认方案保留无配置流程。

## v0.9.3-dev.1 (2026-09-01)

- 开发原因：
  - Device Flow 重新授权完成后立即关闭弹窗，只显示短时 Toast，用户无法稳定确认授权是否成功以及是否已切换到 V2。
  - 设置页把当前账号与设备强制放入左右分栏；开机自启动位于设备详情底部，账号管理又把身份、有效期和六组操作挤在同一行。
- 实现方式：
  - Device Flow 返回`authorized`后进入独立成功终态，展示账号、`Device Flow V2`和“凭据已安全保存”；点击“完成”后才刷新账号列表并调用完成回调。
  - 系统凭据保存失败、二维码拒绝、过期和普通失败继续使用独立终态；成功页不会覆盖或弱化失败提示。
  - 设置主栏改为“当前飞书账号 → 账号管理 → 默认同步策略”；开机自启动直接放在当前账号状态下方，并保留立即生效语义。
  - 当前设备收敛为右侧紧凑卡，只显示设备名称、设备 ID、设备类型和隔离规则。
  - 新增账号管理卡：身份、同步状态、授权方式、访问凭据有效期、主要授权操作和维护操作分层展示；切换、刷新、重新授权与暂停、断开、移除按风险分组。
- 当前结果：
  - 运行中的 v0.9.2 正式版返回当前账号`auth_protocol=device_v2`、`state=connected`、`paused=false`、`last_auth_error=null`。
  - 账号摘要由`KeyringTokenStore`回读后生成；Access Token 与 Refresh Token 有效期均非空，因此本次确认不仅来自数据库协议字段，V2 凭据也能从 Windows 凭据管理器正常读取。
  - 授权成功提示不会再随弹窗自动关闭而消失；用户可以先核对成功事实，再主动完成流程。
  - 详细方案保存在本地开发资料`docs/local_specs/LarkSync v0.9.3 授权反馈与账号设置布局修改方案.md`；创建时间`2026-09-01 18:21:44 +08:00`，完成时间`2026-09-01 18:40:49 +08:00`。
- 验证方式：
  - 新增授权成功终态测试：成功页必须包含“授权成功”、`Device Flow V2`和“凭据已安全保存”；点击“完成”前不得刷新账号或关闭流程。
  - 前端 36 个测试文件共 119 项通过；TypeScript、ESLint 与生产构建全部通过。
  - 后端全量`python -m pytest -q`通过，共 723 项。
  - 新版前端连接当前正式版只读接口，在 Edge Playwright 的`1080×720`与`1360×900`视口完成真实数据验收；两档`bodyOverflow`和`mainOverflow`均为 0，开机自启动位于账号卡内，设备卡位于辅助栏，账号事实区稳定为三列。
  - `python scripts/build_installer.py --nsis`通过，生成`LarkSync-Setup-v0.9.3-dev.1.exe`，大小`72,403,496 bytes`，SHA256 为`69217CB2ACAC46DA2B48BC796E8039C368A180B3A4A796626BE92DC1070C49D0`。
  - 打包后的`LarkSync.exe --backend`使用独立`synthetic_test`数据目录、端口`18196`和实例锁端口`48916`启动；`/health`返回`ok`，桌面状态返回`packaged=true`、`profile=synthetic_test`和`current_version=v0.9.3-dev.1`。
  - 打包 smoke 进程、监听端口和两个隔离数据目录均已清理；隔离目录采用可恢复方式移入系统回收站。
- 遗留问题：
  - 本次没有再次触发用户真实账号扫码；当前 V2 与凭据回读结论来自运行中正式版，成功确认页的状态推进由自动化会话响应覆盖。
  - `v0.9.3-dev.1`已生成本地 NSIS 安装包，但尚未创建稳定标签或 GitHub Release。
...
- 实现方式：
  ...
  - 现有`Release Build`新增`validate_macos_credentials`手动模式；可通过`--ref`在当前发布分支运行，临时导入 P12 到隔离 Keychain，实际签名并验证测试 Mach-O，再用`notarytool history`验证 Apple ID、Team ID 与应用专用密码。
...
- 验证方式：
  ...
  - 后端全量`python -m pytest -q`：678 项通过；`python -m pip install --dry-run -e apps/backend`解析版本为`larksync-backend==0.8.20.dev3`。
...
- 开发原因：
  - “芯华 - 工作记录”任务最近多轮下载均显示`total=2477 downloaded=2 skipped=2475`，但两份目标 Markdown 每轮写回后的正文、哈希和修改时间均未变化。
  - 两份文件分别保留 1 个和 8 个`sheet_token`占位；飞书内嵌 Sheet 元数据可读，但单元格值接口均返回`not found sheetId`，所以旧逻辑每轮都会强制重转并再次得到相同占位。
  - 旧逻辑只要本地 Docx Markdown 含`sheet_token`就绕过未变化判断，没有记录该云端版本是否已经回刷，导致正常跳过被错误计为下载。
- 实现方式：
  - 数据库 schema 从 v6 升级到 v7；`sync_links`新增`placeholder_refresh_revision`，只记录 Docx 占位符回刷版本，不改变云端或本地文档内容。
  - 回刷版本固定为`cloud_token@cloud_mtime毫秒#sheet-preview-v1`；云端 token、云端修改时间和转码迁移版本共同决定是否需要重试。
  - 历史映射没有回刷记录时允许首次强制重转；下载落盘后持久化本次回刷版本，即使结果仍包含占位也不会在同一版本重复强制下载。
  - 已记录同一回刷版本时恢复使用云端修改时间、本地修改时间和内容哈希的正常跳过判断；云端更新或后续提升`sheet-preview`迁移版本时重新尝试一次。
- 当前结果：
  - 同一云端版本第一次扫描仍保留历史占位自动修复能力，第二次及后续未变化扫描会记为跳过，不再反复显示两次下载。
  - 云端文档发生修改，或转码迁移版本显式升级后，会生成新的回刷版本并再次尝试转换。
  - 本次实现未修改正式任务、正式数据库或“芯华 - 工作记录”的本地文件；已安装的 v0.8.19 需要升级到本版本后才会执行 schema v7 迁移并生效。
- 验证方式：
  - TDD 回归先在旧实现上复现第二轮仍下载且缺少回刷版本字段，随后验证首次下载、版本持久化、同版本第二轮跳过和转码器只调用一次。
  - 后端全量`python -m pytest -q`：652 项通过；新增用例同时验证云端修改时间增加 10 秒后会重新调用转码器。
  - 前端`npm run lint`、`npm run typecheck`、`npm run test`和`npm run build`：通过；35 个测试文件、112 项测试通过。
  - `python scripts/build_installer.py --nsis`：通过；生成`dist/LarkSync-Setup-v0.8.20-dev.1.exe`，大小`71,725,963 bytes`（`68.40 MiB`）。
  - 安装包 SHA256 为`CDDDEAD722F400C023F55465827F3A7A9F027120EB234AA1E3FC41C99DD0403A`。
- 遗留问题：
  - 两份样本文档的飞书 Sheet 值接口仍返回`not found sheetId`；本修复只消除无效重复下载，不把无法取得的 Sheet 单元格内容臆测成 Markdown 表格。
...
- 开发原因：
  - 2026-07-28 至 2026-07-29 的正式数据中，8 个任务累计出现 76 次下载失败；共同错误为飞书 `code=20026`。
  - 飞书刷新成功会轮换 `refresh_token`，旧值不能再次使用；v0.8.0-dev.41 引入的进程内 Keyring 缓存使长驻进程无法观察到另一进程写入的新值。
  - 重启 v0.8.12 后立即恢复，证明持久化凭据仍有效，失败来自旧进程继续使用缓存副本，而不是任务目录、网络或文档内容。
  - 任务级认证异常只写入 `last_error`，活动记录仅留下`完成: total=0 ok=0 failed=0 skipped=0`；问题中心因此丢失真实 `20026` 证据，并把汇总误归类为下载问题。
- 实现方式：
  - 新增跨平台操作系统文件锁；Windows 使用 `msvcrt.locking`，macOS/Linux 使用 `fcntl.flock`，默认等待上限 20 秒。
  - 同一 App ID 的授权交换、Token 刷新、身份资料回写和退出登录共用跨进程锁；进入临界区后绕过进程缓存，从 Keyring 或当前 TokenStore 重新读取凭据。
  - 对 `20026`、`20064`、`20073` 只执行一次安全恢复：仅当持久化 `refresh_token` 与本次失败值不同，才复用新 access token 或用新 refresh token 重试；相同旧值绝不重放，直接提示重新连接飞书。
  - 日志只写 refresh token 的 SHA256 前 10 位摘要、进程号和锁等待时间，不输出原始 access token、refresh token 或 App Secret。
  - 保持既有 OAuth v1 为默认；仅当用户显式配置 `/authen/v2/oauth/token` 时启用 v2 参数和 `offline_access`，已有正确 v2 地址不再被配置迁移覆盖。
  - 调度下载、调度上传、冲突执行和手动任务的顶层异常先写入独立失败事件，再写计数汇总；所有完成汇总无论 `failed` 数量均不进入问题中心。
  - 更新安装在启动安装器前校验安装包和重启目标，关闭桌面/Vite，并确认自有或复用后端完全退出；安装器未接管时恢复后端服务。
- 当前结果：
  - 多个本机进程不会同时消费同一个 refresh token；等待者读取已轮换凭据后直接复用或安全刷新。
  - 持久化凭据确实失效时，用户得到“请重新连接飞书”及原始错误码，不再形成无限重复失败。
  - 问题中心以真实认证错误建立一条`auth_permission`问题，不再为同一次运行额外生成空的下载问题。
  - 历史空失败汇总会在问题后台再次处理时按现有迁移机制自动结案，原始活动证据继续保留。
  - 版本统一更新为`v0.8.18-dev.1`。
- 验证方式：
  - 跨进程锁覆盖真实多进程互斥、等待成功和超时场景。
  - 认证测试覆盖缓存绕过、并发轮换、旧值禁止重试、明确重授权、身份资料回写不回滚新 Token、v1/v2 请求契约和跨进程锁超时。
  - 同步与问题测试覆盖任务真实错误先于汇总入库、`failed=0`汇总过滤，以及`20026`只生成一条认证问题。
  - 更新测试覆盖自有后端、复用后端、关闭失败、目标校验、安装接管失败恢复和启动顺序。
  - 后端全量`python -m pytest -q`：649 项通过。
  - 前端`npm run lint`、`npm run typecheck`和`npm run build`：通过，无 ESLint 警告或 TypeScript 类型错误，Vite 生产构建成功。
  - 前端全量`npm run test`：35 个测试文件、112 项测试通过。
  - 后端 editable 安装元数据 dry-run：通过，解析版本为`larksync-backend==0.8.18.dev1`。
  - `npm audit --omit=dev`：0 个生产依赖漏洞。
  - `python scripts/update_install_smoke.py`：通过；缺失模拟安装包按预期收敛为`launch_failed`，未触碰当前正式实例。
  - `python scripts/build_installer.py --nsis`：通过；生成`dist/LarkSync-Setup-v0.8.18-dev.1.exe`，大小`71,714,415 bytes`（`68.39 MiB`），SHA256 为`A491FCE037C8EB1C2339919B9370237E0A9330FD7F9C20B68FA58B4C4B04F8AC`。
- 遗留问题：
  - OAuth v2 仍需用户显式成对配置授权与 Token 地址；本版本不主动迁移现有 v1 用户，以避免不必要的重新授权。
...
- 开发原因：
  - v0.8.15 使用满高维护面板和底部锚定，把页面留白转移到卡片内部。
  - 设置页没有统一的“飞书账号与当前设备”上下文标题，默认窗口与窄窗口缺少明确的安全回退契约。
  - v0.8.16-dev.1 已冻结最新版设计，需要将设计规则转为可回归验证的业务页面结构。
- 实现方式：
  - 设置页主工作区使用自然高度网格；逻辑宽度`>=900px`为“主栏 + 380–420px 辅助栏”，逻辑宽度`<900px`回退为单栏。
  - 设置页主栏依次放置“飞书账号与当前设备”、默认同步策略和高级 OAuth；辅助栏放置忽略规则与数据保护。
  - 账号和设备增加统一外层标题，内部仍使用`0.9fr : 1.1fr`平衡双区。
  - 高级 OAuth 默认折叠；说明文字明确为只在更换应用凭证或授权端点时修改。
  - 更新与维护页使用自然高度`3fr : 2fr`双栏；逻辑宽度`>=900px`保持双栏，低于阈值回退单栏。
  - 移除页面根节点和双面板的`h-full / min-h-0 / grid-rows-[auto_minmax(0,1fr)]`强制满高结构。
  - 安装详情和危险操作从`mt-auto`底部锚定改为固定`20px`自然间距，只随本栏内容增长。
  - 增加页面、主辅栏和维护模块的稳定`data-*`结构标记，并为自然高度、模块顺序和响应类补充静态回归测试。
  - 根包、前端包、前端 lockfile 和后端包版本统一更新为`v0.8.16-dev.2`。
- 当前结果：
  - 1280×720 隔离预览中，设置页主栏高度约`491px`、辅助栏约`237px`，两栏顶对齐且没有被强制拉齐。
  - 1280×720 隔离预览中，更新维护页按`772.8px : 515.2px`呈现，安装详情和危险操作直接跟随前一模块。
  - 1080×720 紧凑桌面窗口仍保持双栏；设置页为`632px : 420px`，维护页为`631.2px : 420.8px`。
  - 860×720 安全回退验证为单栏，辅助栏排在主栏之后。
  - 1280×720 与 1080×720 的主滚动区`clientHeight`等于`scrollHeight`，默认内容没有额外滚动。
- 验证方式：
  - 设置与更新维护页定向 Vitest：5 项通过。
  - 前端全量 Vitest：112 项通过。
  - `npm run lint`：通过，无 ESLint 警告。
  - `npm run build`：通过，TypeScript 类型检查和 Vite 生产构建成功。
  - `npm audit --omit=dev`：0 个生产依赖漏洞。
  - 后端全量 pytest：通过。
  - 后端 editable 安装元数据 dry-run：通过，解析版本为`larksync-backend==0.8.16.dev2`。
  - Windows 静默安装 smoke：通过；缺失模拟安装包按预期收敛为`launch_failed`，未触碰当前正式实例。
  - 使用独立 Vite 端口`3669`与只读模拟后端端口`18766`完成隔离浏览器预览；验证结束后关闭两个临时服务并删除模拟脚本。
- 遗留问题：
  - 正式发布前仍需完成 Windows NSIS 构建、稳定版号冻结、Tag 推送与 GitHub Release 资产验证。

## v0.8.16-dev.1 docs: 设置与更新维护页自然高度双栏设计 (2026-07-30)

- 开发原因：
  - v0.8.14 设置图将 OAuth 放在右栏且辅助栏比例偏大，与 v0.8.15 实装模块归属不一致。
  - v0.8.14 维护图要求双面板等高并把安装详情、危险操作锚定底部，会把页面留白转移到卡片内部。
  - 标准图与紧凑图只做等比例缩放，却被当作两套设计维护，增加文档与实装漂移风险。
- 实现方式：
  - 本轮只修改设计文档、图片资产和版本记录，不修改业务页面。
  - 设置页固定为自然高度主辅双栏；主栏承载账号设备、同步策略和折叠 OAuth，辅助栏只承载忽略规则与数据保护。
  - 更新与维护页保留`3:2`双栏，但取消满高、等高和底部锚定。
  - 两页在逻辑宽度`>=900px`时保持模块顺序不变，只连续调整辅助栏宽度、栏间距与内边距。
  - 逻辑宽度`<900px`时提供单栏安全兜底，不作为正常桌面模式。
  - 使用内置 imagegen 的`ui-mockup`模式生成设置页和维护页初稿。
  - 对维护页追加一次`precise-object-edit`，只缩短右面板，明确表现底边无需对齐。
  - 根包、前端包、前端 lockfile 和后端包版本统一更新为`v0.8.16-dev.1`。
- 当前结果：
  - 设置页最新版图为`docs/design/assets/v0.8.16-design-v1-settings-natural-dual.png`。
  - 更新与维护页最新版图为`docs/design/assets/v0.8.16-design-v1-maintenance-natural-dual.png`。
  - 设置页图尺寸为`1539×1022`，维护页图尺寸为`1541×1021`。
  - 设置页辅助栏明显窄于主栏，OAuth 默认折叠且保持在主栏。
  - 更新与维护页右面板明确短于左面板，日志与危险操作之间不存在填充空洞。
  - 旧 v2/v3 图片保留并在图片更新记录中标为已废弃。
- 验证方式：
  - 两张最终图片均从仓库目标路径重新打开并完成原始分辨率视觉复核。
  - 设计文档记录模块归属、尺寸、响应规则、状态交互、验收口径、生成提示约束和图片迭代过程。
  - `npm run typecheck`：通过。
  - `npm run lint`：通过，无 ESLint 警告。
  - Release、Release Notes 和 GitHub Actions 工作流专项测试：20 项通过。
  - `git status -- docs/local_specs`保持干净。
- 遗留问题：
  - 本版本尚未修改`SettingsPage.tsx`或`MaintenancePage.tsx`。
  - 用户确认最新版设计图后，才进入实际开发与正式发布。
...
- 开发原因：
  - 设置页在默认窗口下左栏存在独立滚动条，用户需要额外滚动才能查看少量配置。
  - 更新与维护页内容区底部存在大块未使用空间。
- 实现方式：
  - 设置页取消强制满高外框和左右独立滚动区。
  - 设置页主栏使用剩余宽度，辅助栏限制为`380–420px`。
  - OAuth 凭证与授权端点统一受一个展开状态控制，默认不渲染。
  - 更新与维护页改为`标题区 + 剩余工作区`两行结构。
  - 两张维护主面板占满剩余工作区，安装详情和危险操作继续锚定各自面板底部。
  - 根包、前端包和后端包版本统一更新为`v0.8.15-dev.1`。
- 当前结果：
  - 默认`1280×720`窗口下，设置页桌面主滚动区`clientHeight`与`scrollHeight`均为`844`逻辑像素。
  - 设置页辅助栏物理宽度为`336px`，对应`420px`逻辑宽度；OAuth 折叠时不渲染凭证字段。
  - 更新与维护页左右面板物理高度均为`580.8px`，底边均为`700.8px`。
  - 更新与维护页底边距收敛为主内容区标准边距，未新增页面滚动。
- 验证方式：
  - 定向 Vitest：3 个测试文件、8 项测试通过。
  - `npm run typecheck`：通过。
  - `npm run lint`：通过，无 ESLint 警告。
  - `npm test`：35 个测试文件、112 项测试全部通过。
  - `npm run build`：通过，Vite 生产构建成功。
  - `python scripts/build_installer.py --nsis`：通过。
  - 生成`dist/LarkSync-Setup-v0.8.15-dev.1.exe`，文件大小`71,697,902`字节。
  - 隔离预览使用端口`3669`的 Vite 服务和端口`18766`的内存模拟接口。
  - 隔离预览未连接、停止或修改用户正在运行的正式版。
- 遗留问题：
  - 本版本为开发版本，未创建 GitHub Release，等待用户确认后再进入正式发布流程。
...
- 实现方式：
  ...
  - PowerShell 集成测试不再使用`capture_output=True`创建 stdout/stderr 读取管道。
  - 测试 stdout 重定向到`DEVNULL`，stderr 重定向到临时文件；继续检查`ParserError`且保留 10 秒上限。
- 当前结果：
  - 稳定版源码已冻结为`v0.8.14`。
  - 更新与维护页采用顶底对齐的双主面板。
  - 更新动作按真实状态显示。
...
- 验证方式：
  ...
  - 第三次云端运行在安装包预检已生效后仍超时，堆栈明确停在`subprocess.communicate()`的 stdout 读取线程。
  - 第三次结果证明最终 CI 根因是 GitHub Windows Runner 上继承输出管道未关闭；不是 worker 脚本执行超时。
...
- 开发原因：
  - v0.8.14-dev.3 已确认维护页 v3 设计，但正式页面仍使用页面级检查按钮和左右独立滚动。
  - 自动更新设置仍与日志配置混合保存。
  - 安装详情和危险操作仍是独立卡片，左右栏顶底边界不一致。
  - `已是最新`状态仍渲染不可用的下载、打开目录和安装按钮。
- 实现方式：
  - 页面标题区移除`检查更新`。
  - `检查更新`移动到左主面板的`版本与更新`标题行。
  - 内容区改为`3fr / 2fr`双栏，逻辑宽度低于`900px`时才使用单栏安全兜底。
  - 左侧建立单一`版本与安装`主面板。
  - 右侧建立单一`本机维护`主面板。
  - 两张主面板使用同一 Grid 行和拉伸对齐。
  - 删除维护页左右内部滚动区，统一依赖桌面主内容滚动区。
  - 自动更新和检查间隔移入版本模块，并使用独立`保存更新设置`。
  - 三个日志字段使用独立`保存日志设置`。
  - 更新配置与日志配置分别发送部分配置，不再由一个按钮混合保存。
  - 新增`getUpdateActionVisibility`，按真实更新状态决定下载、打开目录和安装动作是否渲染。
  - 新增`shouldAutoExpandInstallDetails`，根据安装请求和 handoff 阶段判断安装详情是否自动展开。
  - 安装详情允许人工展开或收起；安装活动变化后恢复自动展开能力。
  - 危险操作改为右主面板内部单层语义区；任务列表按需渲染，不建立内部滚动。
- 当前结果：
  - `up_to_date`不显示下载、打开目录或静默安装。
  - `available`显示下载动作。
  - `downloaded`且存在有效安装包路径时显示打开目录和静默安装。
  - 安装请求、安装中、失败或重启未确认会自动展开安装详情。
  - `restart_succeeded`和无安装活动时默认折叠安装详情。
  - 左右主面板在空闲和展开状态下均保持顶底对齐。
  - 设置页和后端接口未修改。
- 验证方式：
  - `npm run typecheck`通过。
  - `npm run lint`通过，无 ESLint 警告。
  - `npm test`通过：35 个测试文件、111 项测试。
  - `npm run build`通过：Vite 生产构建成功。
  - `python scripts/build_installer.py --nsis`通过。
  - 生成`dist/LarkSync-Setup-v0.8.14-dev.4.exe`，安装包约`68.38 MB`。
  - 安装包大小为`71,702,583`字节，SHA256 为`0A764C9CD3808D5E35D196FEBB00B4201775ECD00DA4E270F968E1E38B765299`。
  - 1360×900 隔离预览：桌面缩放`1.000`，左右面板高度均为`566px`，底边均为`720px`。
  - 1080×720 隔离预览：桌面缩放`0.794`，左右面板高度均为`450px`，底边均为`572px`。
  - 两档窗口的文档横向滚动宽度均等于视口宽度，没有横向溢出。
  - 两档窗口的维护页内部滚动区数量均为`0`。
  - 紧凑窗口展开安装详情与任务列表后，两张面板高度均为`620px`，由页面主滚动区统一滚动。
  - 隔离验证使用本地内存模拟接口，不连接正式版后端。
  - 验证结束后已关闭独立 Vite 和模拟接口进程。
  - 安装包只完成本地构建和文件校验，未执行安装、启动或发布。
  - 未启动、停止或操作用户当前运行的正式版。
- 遗留问题：
  - 当前只完成开发分支实现，尚未发布安装包。
  - 后续发布前仍需进行桌面打包体验测试，验证托盘壳中的真实 DPI 缩放和更新流程状态切换。

## v0.8.14-dev.3 docs: 更新与维护页对齐双面板设计 (2026-07-30)

- 开发原因：
  - v0.8.14-dev.2 已确定维护页双栏归属，但左右仍分别堆放独立卡片。
  - 左侧版本卡、安装详情与右侧日志卡、危险操作的顶底边界不一致，标准窗口下呈现瀑布流感。
  - 危险操作使用红色外层容器和内部白色边框，形成重复视觉层级。
  - `已是最新`状态仍展示禁用下载、安装按钮，增加没有操作价值的视觉噪音。
- 实现方式：
  - 本轮继续只修改设计文档、版本记录和设计图，不修改业务页面代码。
  - 设置页继续使用 v2，不重新生成。
  - 维护页左侧只保留一张`版本与安装`外层主面板。
  - `版本与更新`和折叠`安装详情`改为左主面板内部语义区。
  - 维护页右侧只保留一张`本机维护`外层主面板。
  - `日志管理`和`危险操作`改为右主面板内部语义区。
  - 左右主面板处于同一网格行，顶边和底边对齐。
  - 危险操作使用单层风险语义，不再使用红色外框嵌套白色内框。
  - 更新动作按状态显示；`已是最新`时隐藏下载和安装动作。
  - 标准与紧凑窗口保持相同主面板、模块顺序和动作位置。
  - 使用内置 imagegen 的`ui-mockup`模式生成标准与紧凑两张 v3 图。
- 当前结果：
  - 标准图归档为`docs/design/assets/v0.8.14-design-v3-maintenance-standard-aligned-panels.png`。
  - 紧凑图归档为`docs/design/assets/v0.8.14-design-v3-maintenance-compact-aligned-panels.png`。
  - 设计文档将维护页 v2、v2.1 标记为废弃，将两张 v3 图记录为最新。
  - 标准图输出尺寸为`1538×1022`；紧凑图输出尺寸为`1536×1024`。
  - 图片尺寸是 imagegen 规范化结果，目标物理窗口条件分别为`1360×900`和`1080×720`。
  - 当前正式页面仍为 v0.8.13，不包含本设计。
- 验证方式：
  - 执行`python scripts/sync_feishu_docs.py`，4 个飞书开发文档均为最新，清单检查成功。
  - 可视检查标准图：完整保留生产侧栏、顶栏和状态区。
  - 可视检查标准图：左右两张主面板顶底对齐，内部没有四张独立外层卡片。
  - 可视检查紧凑图：模块位置与标准图一致，仅缩小间距和内边距。
  - 可视检查紧凑图：顶栏`立即同步`、`任务管理`、用户区和窗口控制均保留。
  - 两张图均未显示禁用下载、安装按钮。
  - 未运行前端测试、安装包构建或正式版安装，因为本轮没有业务代码变更。
  - 未启动、停止或操作用户当前运行的正式版。
- 遗留问题：
  - 等待用户确认 v3 两张维护页设计图。
  - 用户确认前不修改维护页业务代码。
  - 最终开发时需按真实更新状态驱动动作显隐，不得以设计图中的示例数据代替后端数据。

## v0.8.14-dev.2 docs: 设置与更新维护页稳定双栏设计 (2026-07-30)

- 开发原因：
  - 两页信息量不大，v0.8.14-dev.1 在 1080×720 切换为单栏会制造不必要的模块位移和纵向长度。
  - 桌面壳以 `1360×900` 为设计画布；1080×720 通过约 `0.794` 的统一缩放仍可保留完整逻辑宽度。
  - 设置与维护页没有活动管理、问题中心式的主从关系，不需要在窗口变化时改变信息架构。
- 实现方式：
  - 本轮继续只修改设计文档与图片，不修改业务页面。
  - 正常桌面窗口统一保持双栏；紧凑、标准和宽屏只改变缩放、列宽与留白。
  - 设置页左栏固定账号设备与默认同步策略；右栏固定忽略规则、折叠 OAuth 和数据保护。
  - 维护页左栏固定版本更新与安装详情；右栏固定日志管理和危险操作。
  - 两栏共同使用页面唯一滚动区，不建立独立滚动。
  - 仅在非桌面环境或逻辑内容宽度低于约 `900px` 时提供单栏安全兜底。
  - 使用内置 imagegen 重新生成设置标准、设置紧凑缩放、维护标准、维护紧凑缩放四张 v2 设计图。
  - 维护紧凑初稿遗漏生产顶栏的`立即同步 / 任务管理`；使用标准图作为顶栏参考定向修正为 v2.1。
- 当前结果：
  - v1 单栏相关设计图已在文档图片记录中标记为废弃。
  - 文档`最新设计图`只展示四张稳定双栏 v2 图。
  - 四张图的左右模块归属、主操作位置和桌面壳保持一致。
  - 当前正式页面仍为 v0.8.13，不包含本设计。
- 验证方式：
  - 设置标准与紧凑图均确认：左栏账号策略、右栏规则 OAuth 保护。
  - 维护标准与紧凑图均确认：左栏版本安装、右栏日志危险操作。
  - 四图均确认`检查更新`只位于版本更新卡片，页面标题区无该动作。
  - 维护紧凑修正版确认恢复顶栏`立即同步`与`任务管理`。
  - 图片文件已复制到 `docs/design/assets/v0.8.14-design-v2-*.png` 并完成可视复核。
  - 未运行安装包构建，未安装或启动开发版，未操作用户当前正式版。
- 遗留问题：
  - 等待用户确认 v2 双栏图；确认前不修改业务页面。
  - imagegen 输出画布被规范化为接近 3:2，图片用于确认布局语义，不作为最终像素级实现依据。

## v0.8.14-dev.1 docs: 设置与更新维护页信息架构设计 (2026-07-29)

- 开发原因：
  - v0.8.13 只修复满高与列宽，没有接入现有紧凑/标准/宽屏逻辑。
  - 两页信息量不足以支撑活动管理、问题中心式的三套工作台设计。
  - `检查更新`被放到页面标题右侧，超出版本与安装模块的业务作用域。
  - 设置页的页面保存与任务级忽略规则保存语义混杂。
- 实现方式：
  - 本轮只建立设计方案，不修改设置、更新、日志、OAuth 或重置映射业务代码。
  - 采用一套信息架构和单栏/双栏两种排布；宽屏沿用双栏，不新增第三栏。
  - 页面统一为单一内容滚动区；标准双栏共同随页面滚动。
  - 设置页将忽略规则收敛为摘要和独立管理入口，高级 OAuth 默认折叠，数据保护改为轻量说明。
  - 维护页将`检查更新`放回版本模块；自动更新与检查频率归入更新模块；安装详情在空闲时折叠；危险操作置底。
  - 使用内置 imagegen 和 v0.8.13 真实页面截图生成设置标准、设置紧凑、维护标准、维护紧凑四张高保真设计图。
  - 维护标准初稿误将更新检查间隔放入日志管理；复核后生成 v1.1 修正版，错误初稿未归档为最新版。
- 当前结果：
  - 详细规格、交互状态、动作层级、响应式规则、验收条件和待确认项已写入 `docs/design/v0.8.14-settings-maintenance-information-architecture-design.md`。
  - 四张最新版设计图已存入 `docs/design/assets/`。
  - 当前正式页面保持 v0.8.13，不包含本设计。
- 验证方式：
  - 逐张检查桌面壳、模块顺序、动作位置、单栏/双栏关系和滚动模型。
  - 维护标准修正版确认日志卡片只包含三个日志字段。
  - 维护两图确认`检查更新`只出现于版本与更新模块。
  - 设置两图确认忽略规则与页面保存分离、OAuth 折叠、数据保护降级为说明条。
  - 未运行安装包构建，未安装或启动开发版，未操作用户当前正式版。
- 遗留问题：
  - 等待用户确认六项设计决策；确认前不修改业务页面。
  - imagegen 输出画布被规范化为接近 3:2，图片用于确认布局语义，不作为最终像素级实现依据。
...
- 开发原因：
  - v0.8.12 给设置页和更新维护页增加 `1240px` 最大宽度与水平居中，最大化后形成窄内容带，与总览、任务管理、活动管理和问题中心不一致。
  - 两页仍使用整页纵向滚动，页面标题和主操作会随长内容滚走。
  - 首轮只迁移外层双栏后，设置页内部保留旧三列结构；在 1360×900 的左主列中，账号、设备、同步策略和 OAuth 控件发生挤压、断行或竖排。
- 实现方式：
  - 两页根节点统一为 `h-full` 与 `grid-rows-[auto_minmax(0,1fr)]`；移除页面级 `max-width` 和 `mx-auto`。
  - 设置工作区复用设计基线的 `1.16fr / min 390px / 0.84fr` 双栏；左侧承载账号、设备、同步策略和 OAuth，右侧承载忽略规则与数据保护。
  - 维护工作区使用 `minmax(0,1fr) / 360px`；更新流程与日志维护分别独立滚动。
  - “保存设置”和“检查更新”放在页面标题行；页内不再重复主要操作。
  - 设置页账号与设备改为两块纵向内容；同步策略改为“标题/删除策略 + 三个等宽模式卡”；OAuth 改为 App ID/Secret 双列、Redirect URI 整行。
- 当前结果：
  - 1360×900 下不再出现字段竖排、按钮断行或内容窄条。
  - 1920×1080 下工作区使用全部剩余宽高，不再出现 1240px 居中空白带。
  - 两页的页面骨架、主操作位置和滚动规则与其他生产页面一致。
- 验证方式：
  - 设置页与维护页结构测试覆盖满高根节点、工作区列宽、两个独立滚动区、主操作位置和禁止 1240px 最大宽度。
  - 前端 `109` 项 Vitest、TypeScript 类型检查、ESLint 和 Vite 正式构建通过。
  - 后端全量 `631` 项 pytest 通过。
  - 使用隔离的无头浏览器连接只读正式后端，在 `1360×900` 与 `1920×1080` 分别截图检查设置页和更新维护页。
...
- 开发原因：
  - 正式库 `sync_run_events` 已有 `2,605,791` 条事件；旧查询同时传入 `task_ids` 与 `run_ids`，SQLite 选择任务时间索引后扫描任务全历史，导致旧运行无论成功或失败都可能等待约 `0.9–1.7` 秒。
  - 正式库有 `836,263` 条 `queued`；最近一轮 `1,460` 条“等待周期上传”和 `1,463` 条“云端未更新”在路径和时间上高度对应。
  - Watchdog 的 `opened / closed / closed_no_write` 只读生命周期事件被当成本地修改，文件哈希读取即可制造伪上传排队。
  - 问题中心向文件管理器传入字符串，而文件管理器要求 `Path`，因此“打开文件夹”动作报错。
  - 设置与更新维护页面在最大化窗口中无限拉伸；检查更新按钮脱离更新流程；标准/宽屏事件详情使用 400px 侧浮层遮挡事件区。
- 实现方式：
  - 选中可信运行后，事件请求只发送 `run_ids`；没有运行时才回退到 `task_ids`，复用现有运行时间索引。
  - Watchdog 只转发 `created / modified / moved / deleted`；瞬时上传排队仅保留在内存，不写入持久活动事件。
  - 新增默认“处理结果”筛选，明确排除 `queued`；旧记录不删除、不迁移，可在“全部事件”或“历史排队”中查看。
  - 问题目录动作将任务路径显式转换为 `Path`；前端在动作成功后显示“已打开本地目录”，不再误报等待验证。
  - 设置与更新维护页面统一使用 `1240px` 最大内容宽度；维护侧栏固定为 `340px`；检查更新按钮移动到“更新流程”标题区。
  - 紧凑模式继续使用整页事件详情；标准和宽屏模式改为最大 `720px` 的居中弹窗，支持关闭按钮、Esc 和点击遮罩关闭。
- 当前结果：
  - 运行详情查询不再因任务全历史量级而表现为“失败记录加载慢”。
  - 新产生的只读文件访问不会形成“等待上传”，真实修改仍进入周期上传内存队列。
  - 所有升级用户的旧 `queued` 历史证据完整保留，默认活动视图不再被其淹没。
  - 六项界面与动作问题均已按 `docs/design/v0.8.12-maintenance-activity-runtime-fixes.md` 落地。
- 验证方式：
  - Watchdog 只读事件过滤、四类修改事件放行、上传内存排队不落历史、问题目录 `Path` 转换均有后端回归测试。
  - 运行查询互斥条件、默认事件筛选、历史排队入口、宽屏比例、按钮归属、动作提示和居中弹窗均有前端回归测试。
  - 后端全量 `631` 项 pytest 通过。
  - 前端 TypeScript 类型检查、ESLint、`109` 项 Vitest 与 Vite 正式构建全部通过。
  - 正式库只读复核确认新查询使用 `idx_sync_run_events_run_timestamp (run_id=?)`；旧失败运行读取 30 条事件首次 `0.748ms`，后续 `0.026–0.043ms`。
  - `python scripts/build_installer.py --nsis` 正式构建通过。
...
## v0.8.11 release (2026-07-28)

- 开发原因：
  - 将 v0.8.11-dev.1 的代码同步修复、问题分层与更新下载进度作为正式版本交付，继续由正式版自动升级链路验证用户体验。
- 实现方式：
  - 正式版本号统一更新为 `v0.8.11`。
  - Release 包含 v0.8.11-dev.1 的全部实现、测试与设计文档。
- 当前结果：
  - 正式版将零字节代码标记文件从失败改为明确跳过。
  - 问题中心不再把具体对象失败和任务完成汇总当成同层级问题。
  - 更新页显示真实下载百分比、大小、速度和 SHA256 校验阶段。
- 验证方式：
  - 后端全量 `622` 项 pytest 通过。
  - 前端 TypeScript 类型检查、ESLint、`106` 项 Vitest 与 Vite 正式构建全部通过。
  - `python scripts/build_installer.py --nsis` 正式构建通过。
...
- 开发原因：
  - 正式版只读检查确认，2026-07-28 21:30 的周期上传把两个 ROS `resource/` 零字节标记文件送入飞书文件上传器。
  - 文件上传器拒绝 `file_size <= 0`，两个对象均返回“文件大小不能为空”，本轮结果为 `total=2 / failed=2`。
  - 同一轮的两个对象失败和一个任务完成汇总都被问题中心收录，形成一个根因对应三个未解决问题。
  - LarkSync 更新下载只有阻塞式“下载中”按钮，不能显示百分比、传输大小、速度或 SHA256 校验阶段。
- 实现方式：
  - 普通文件本地签名的 `local_size == 0` 时不调用飞书上传接口，不修改文件内容，不建立虚假映射；运行计为 `skipped`，事件说明为“空文件已跳过：飞书云盘不支持 0 字节文件”。
  - 启动时的未映射文件扫描不把零字节文件加入待上传队列，避免每次启动重复生成无动作上传检查；文件后续写入内容后仍可由 watcher 或下一次扫描发现。
  - 问题分类器升级为 `problem-classifier-v3`。
  - `完成: total=N ok=N failed=N skipped=N` 且 `failed > 0` 的任务汇总继续保留在活动管理，但不再创建问题。
  - 具体同步对象使用 `sync_object`，没有具体对象证据的任务异常使用 `task_run`，冲突继续使用 `conflict`。
  - 错误文本无法判定操作时，使用所属 `sync_runs.trigger_source` 补充分为上传或下载问题。
  - 旧活动问题分批迁移；旧任务汇总以 `workflow_summary_not_problem` 结案，旧“文件大小不能为空”问题以 `zero_byte_file_now_skipped` 结案，原始问题和证据不删除。
  - 更新状态新增 `idle / checking / up_to_date / available / downloading / verifying / downloaded / error`。
  - 下载进度包含 `percent / bytes_per_second / transferred / total`，页面每 750ms 读取状态并显示进度条。
  - 安装包先写入 `.part`；SHA256 通过后原子替换最终文件，失败时删除临时文件并禁用安装入口。
- 当前结果：
  - 零字节代码标记文件不再导致同步任务失败。
  - 文件/对象失败与任务运行异常在问题中心有独立层级标签。
  - 正式库只读模拟共发现 9 条活动问题；升级后预计自动结案 1 条任务汇总和 2 条零字节对象问题，剩余 6 条按对象或任务层级保留。
  - 更新下载过程可观察，下载中断不会留下可被误认为完整安装包的最终文件。
- 验证方式：
  - 新增零字节文件跳过测试。
  - 新增上传对象与任务汇总分离测试。
  - 新增旧任务汇总兼容结案测试。
  - 新增更新进度、原子落盘和校验失败清理测试。
  - 后端全量 `622` 项 pytest 通过。
  - 前端 TypeScript、ESLint、`106` 项 Vitest 和 Vite 生产构建通过。
  - `python scripts/build_installer.py --nsis` 通过；生成 `LarkSync-Setup-v0.8.11-dev.1.exe`，大小 `71,657,616` bytes，SHA256 为 `520C980AA2ABDA395A6AFA95C1C36F2EEF0C8678A0B3E38F33D8954FE2D6A11F`。
  - 安装包仅构建与校验，未安装、未启动、未控制用户当前正式版界面。
- 遗留问题：
  - 零字节文件不会上传到飞书；本版本选择明确跳过并保留本地原文件，避免用填充字节破坏代码语义。

## v0.8.10 release (2026-07-28)

- 开发原因：
  - 活动管理标准窗口长期占用左侧任务栏，但运行记录只能通过下拉框选择，信息主次与用户审计路径相反。
  - 全局正文仍采用 `11/12/13/14px` 四级字阶，非全屏使用时辅助文字和表格内容偏小。
  - 问题中心已有 `ignored` 状态和“已忽略”页签，但没有忽略、填写原因或恢复处理的入口。
  - 正式库只读核对发现 `9` 条未解决问题：`6` 条 system/diagnostic_only、`2` 条 upload/auto_recovering、`1` 条 local_io/auto_recovering；全部有关联任务，但没有同任务、同对象、同操作族的更晚成功事实。
- 实现方式：
  - 标准活动页在统计区下方放置全宽任务选择器，主区改为 `296px` 运行记录与事件工作区左右两栏；在 1280px 物理宽度下仍保留至少 676px 事件区，宽屏继续使用任务/运行/事件三栏，紧凑态继续使用任务/运行双下拉框。
  - 标准态不再渲染任务常驻栏或运行下拉框；任务、运行、筛选、分页和详情选择继续复用原状态，不因窗口模式切换重置。
  - 字阶调整为 `12/13/14/15px`，对应行高为 `18/20/21/22px`；常用 36px 控件提升为 40px，任务主选择器为 42px，活动事件行最小高度为 48px。
  - schema v6 为 `problems` 增加 `ignored_at` 和状态时间索引；迁移不批量改变历史问题状态，适用于所有升级用户。
  - 新增 `POST /problems/{id}/ignore` 和 `POST /problems/{id}/restore`；忽略原因去除首尾空格后必须为 `2–200` 个字符。
  - 人工忽略写入 `ignored_reason / ignored_at`，并在 `problem_actions` 中记录 `ignore_problem`；恢复清除当前忽略字段并记录 `restore_problem`。
  - 同指纹失败再次发生时保持 `ignored` 并累加次数；同对象、同操作的严格成功事实会把已忽略项转为 `resolved`，因为客观恢复证据优先于人工暂不处理。
  - 页面未解决项提供“忽略问题”弹窗，包含预设原因和补充说明；已忽略项显示原因、时间和“恢复处理”，不提供批量忽略。
- 当前结果：
  - 正式库只读审计未修改任何问题或同步数据；升级后 9 条问题仍由用户逐条决定，不做时间阈值批量结案。
  - 旧问题可移入“已忽略”并随时恢复；证据、发生次数和处理历史均保留。
  - 问题忽略不是解决，不会生成伪成功事实，也不计入未解决摘要。
  - 活动标准窗口按“先选任务，再看运行，再看事件”的顺序呈现；非全屏不再重复占用任务列表与运行选择两个位置。
- 验证方式：
  - 后端新增 schema v5→v6 兼容迁移、忽略/恢复审计、重复失败保持忽略、可信成功自动结案和 API 参数/状态冲突测试。
  - 活动页面测试覆盖紧凑双下拉、标准顶部任务+运行/事件两栏、宽屏三栏。
  - 问题页面测试覆盖未解决忽略入口，以及已忽略原因和恢复入口。
...
- 验证方式：
  ...
  - 正式工作流 `30345784898` 全部通过；质量门、Windows、macOS arm64、macOS x86_64 构建与三平台安装启动 smoke 均成功。
  - GitHub Release 发布时间为 `2026-07-28T09:19:44Z`；Latest 为非草稿、非预发布的 `v0.8.9`，六个资产齐全。
  - Windows 正式安装包大小 `57,090,668` bytes；GitHub digest 与 `.sha256` 均为 `e00234a3383deaf3deae709521a0d8babb09539b3277df3acdd8c4633a47b103`。
  - 并行上传后的 Release 正文已恢复 Windows、arm64 和 x86_64 完整校验表。

## v0.8.8 release (2026-07-28)

- 开发原因：
  - `v0.8.7` 活动详情点击后仍长时间无内容；正式日志记录 `/sync/logs/sync` 单次请求耗时 `5.865s` 至 `16.287s`。
  - 正式库有 `2,596,632` 条运行事件；按运行筛选时对索引列使用 `coalesce(run_id, '')`，SQLite 查询计划退化为完整索引或时间索引扫描。
  - 最近七天有 `777` 次运行记录 `1,281` 条 `delete_pending`；历史 cloud 墓碑中 `1,728` 条最终以“云端文件已恢复”取消，属于路径判断先报待删、同轮 token 验证再取消的重复活动。
  - 问题中心仍有 `2,010` 条未解决项；其中 `1,988` 条超过 30 天，精确恢复键没有匹配到任何更晚成功事实。
- 实现方式：
  - `SyncRunEventService` 对已规范化的小写状态和非空运行 ID 使用原列直接 `IN`，保留搜索条件的模糊匹配行为。
  - 下载扫描使用全部非忽略云端候选及文件夹 token 建立存在集合；同步映射 token 仍存在时不创建 cloud missing 墓碑，执行阶段仍保留原有 token 二次确认。
  - 抽取统一同步路径策略；同步运行与问题收敛共用临时文件、隐藏缓存、资源目录、回收目录、Markdown 镜像和任务自定义忽略规则。
  - 新增后台当前状态收敛：任务删除/停用、授权后成功活动、运行中断后成功活动、路径已排除、上传/转换/删除目标已消失均写入明确验证枚举。
  - 上传目标不存在只在同步根目录当前可用时成立；外接盘或同步根目录不可用时保持未解决，避免把临时离线误判为恢复。
  - 每批最多读取 `500` 条未解决问题、单轮最多 `10` 批、至多每 `60` 秒执行一次；使用 `(last_seen_at, id)` 内存游标遍历，同任务后续成功只返回最新一条，GET 查询不执行批量写入。
- 当前结果：
  - 正式库只读查询计划对单运行首屏由扫描改为 `run_id=?` 索引查找；同一查询样本从约 `1332.7ms` 降至约 `0.5ms`。
  - cloud token 存在的对象不会先产生待删活动再在同轮取消；真实云端删除、宽限期、失败重试和回收目录行为不变。
  - 对正式库使用最终规则执行只读 dry-run：扫描 `2,010` 条，预计解决 `2,004` 条并保留 `6` 条；原因分布为运行恢复 `82`、授权恢复 `15`、路径排除 `23`、目标消失 `1,884`。
  - 原问题、出现记录和证据不删除；解决项进入“已解决”，新失败仍可按既有指纹重新打开。
  - 所有规则为通用产品规则，不包含用户任务 ID、目录、文件名或正式数据库特例。
- 验证方式：
  - 新增索引友好 SQL、稳定 token 排除虚假缺失、目标消失、同步根目录不可用、隐藏缓存排除、授权恢复、运行恢复和仍存在错误保留测试。
...
- 遗留问题：
  ...
  - 保留的未解决问题需要通过详情证据确认是否为真实文件错误；本版本不会仅按问题年龄自动结案。

## v0.8.7 release (2026-07-23)

- 开发原因：
  - `v0.8.7-dev.1` 已完成启动调度、无变化上传过滤和空活动口径修复，用户要求继续通过正式版自动升级快速验证。
- 实现方式：
  - 根包、前端、后端及两个锁文件统一从 `v0.8.7-dev.1` 提升到 `v0.8.7`。
  - README 稳定版本和代码基线更新为 `v0.8.7`；CHANGELOG 增加正式发布锚点。
  - 稳定标签触发 Windows NSIS、macOS x86_64 与 macOS arm64 构建、安装启动 smoke、SHA256 生成和 GitHub Release 上传。
- 当前结果：
  - `v0.8.7` 被指定为启动周期继承、无变化上传快速过滤和活动统计修复的稳定版本。
  - 从 `v0.8.6` 自动更新时，Windows 应选择 `LarkSync-Setup-v0.8.7.exe`；正式渠道不会选择开发版安装包。
  - schema v4 在所有升级用户本地执行通用空运行重分类，不依赖正式数据库中的路径、任务 ID 或其他个人值。
  - 发布过程不安装、不启动本地构建，不修改当前正式数据库、同步目录或飞书数据。
- 验证方式：
  - 对应业务提交的后端全量 608 项 pytest 通过。
...
## v0.8.7-dev.1 (2026-07-23)

- 开发原因：
  - `v0.8.6` 后端健康检查已缩短到约 1–2 秒，但自动升级后仍会在启动保护期结束时立即运行全部秒/小时周期任务，用户感知为启动后持续卡顿。
  - 正式数据库只读审计显示，一次定时下载检查 2,471 个对象、耗时约 434.5 秒，仅下载 2 个；一次定时上传处理 2,251 个路径、耗时约 320.8 秒，其中 2,233 个最终为“内容未变化”。
  - 对上述无变化上传抽样 500 个路径，`sync_links.local_size/local_mtime` 与磁盘值 500/500 一致，说明 Watchdog 触碰事件可在云端转换和比对前安全过滤。
  - 运行收尾强制写入 `has_activity=true`；未到期删除墓碑与重复 `delete_pending` 也会每分钟创建运行；活动摘要又遗漏删除统计，共同产生大量“上 0 下 0”记录。
- 实现方式：
  - 新增 `sync_schedule_checkpoint_v1:{task_id}:{direction}` 调度检查点；首次升级优先读取相同任务、相同方向最近一次 `scheduled_upload/scheduled_download` 运行，上传和下载分别计算剩余周期。
  - 新任务没有检查点和历史运行时仍立即首次执行；每次周期调用结束后只更新当前方向检查点，手动同步不改变周期。
  - 周期上传取得静默窗口路径后，一次加载任务映射；`local_size` 一致且 `local_mtime` 误差不超过 1 毫秒时直接丢弃触碰事件，不创建运行、不执行 Markdown 转换或云端请求。
  - `has_pending_tombstones()` 只返回 `expire_at <= now` 的到期项；墓碑创建返回 `created`，同一 pending 项刷新时不再重复写 `delete_pending`。
  - 飞书删除返回 `not found. token=...` 时按幂等成功处理，清理墓碑与映射，不再每 5 分钟重复记录删除失败。
  - 运行收尾恢复使用 `SyncRunService` 的事实推断；schema v4 将所有用户历史中“成功、无错误、七项活动计数均为 0”的运行通用重分类为 `legacy_check`，保留原始运行与事件。
  - 活动运行摘要增加非零“删 / 待删 / 删失败”计数；上传、下载和异常继续固定显示。
  - 逐文件“云端未更新”和 Markdown 资源解析日志降为 DEBUG，保留成功、失败、冲突、删除和运行摘要。
- 当前结果：
  - 自动升级或重启不再立即触发尚未到期的全量下载；60 秒上传周期最多等待剩余 60 秒，2 小时下载周期沿用升级前剩余时间。
  - 正式样本中的 2,233 个无变化路径会在本地签名层结束，不再形成耗时数分钟的空上传处理。
  - 纯空成功运行不进入默认活动列表；历史通用迁移适用于所有升级用户，不依赖用户路径、任务 ID 或正式数据库专用值。
  - 真实删除活动会显示“删 / 待删 / 删失败”，不再被误读为“上 0 下 0”。
  - 发布过程不安装、不启动本地构建，不修改当前正式数据库、同步目录或飞书数据。
- 验证方式：
  - 新增调度方向隔离、升级剩余周期、签名快速过滤、墓碑到期与去重、云端已删除幂等、运行事实推断、schema v4 历史迁移和页面删除摘要回归测试。
  - 后端全量 608 项 pytest 通过。
  - 前端 ESLint、TypeScript、35 个文件共 105 项测试和 Vite 生产构建通过。
  - `python scripts/build_installer.py --nsis` 通过；生成 `LarkSync-Setup-v0.8.7-dev.1.exe`，大小 71,626,775 bytes，SHA256 `13CFE3D47786CE88C1DE8C3702EB10394039C7190E221C4E6BC1F511703720FB`。仅构建，未安装或启动。
  - 正式数据库仅执行 SQLite `mode=ro` 查询；未写入、复制、迁移或停止正式版进程。
- 遗留问题：
  - 冷启动桌面 WebView 从后端健康就绪到首个页面请求仍约需 4 秒；本版本优先消除启动后的数分钟同步工作负载，不改动已经过空白页修复验证的窗口创建顺序。
  - 正式自动升级后需观察：窗口可交互时间、启动 2 分钟内是否出现全量下载、纯“上 0 下 0”历史是否消失，以及到期删除是否显示正确计数。

## v0.8.6 release (2026-07-22)

- 开发原因：
  - `v0.8.6-dev.1` 已完成大历史库启动和后台维护竞争热修复，用户确认继续升级并通过正式自动更新渠道验证。
- 实现方式：
  - 根包、前端、后端及两个锁文件统一从 `v0.8.6-dev.1` 提升到 `v0.8.6`。
  - README 稳定版本和代码基线更新为 `v0.8.6`；CHANGELOG 增加正式发布锚点。
  - 稳定标签触发 Windows NSIS、macOS x86_64 与 macOS arm64 构建、安装启动 smoke、SHA256 生成和 GitHub Release 上传。
- 当前结果：
  - `v0.8.6` 被指定为启动后台维护性能热修复的稳定版本。
  - 从 `v0.8.5` 自动更新时，Windows 应选择 `LarkSync-Setup-v0.8.6.exe`，正式渠道不会选择开发版安装包。
  - 发布过程不安装、不启动本地构建产物，不修改当前正式数据库、同步目录或飞书数据。
  - GitHub Latest 已指向非草稿、非预发布的 `v0.8.6`；Windows、macOS arm64 和 macOS x86_64 安装包及各自 SHA256 文件均已上传。
- 验证方式：
  - 对应业务提交的后端全量 pytest 599 项通过；前端 ESLint、TypeScript、35 个文件共 105 项测试与 Vite 生产构建通过。
  - 本地 NSIS 预构建通过；`v0.8.6-dev.1` 安装包大小 71,617,542 bytes，SHA256 为 `8C08BB4AB302E2AC11EAD8C355B8E69BB2C9DB10DF8ED471D46F66172CA05F1D`。
  - 正式资产必须以 GitHub Actions 对 `v0.8.6` 标签重新构建的文件与 SHA256 为准，不复用开发版文件改名。
  - 正式工作流 `29936630876` 全部通过；质量门、Windows 构建、macOS arm64 与 macOS x86_64 构建以及三平台安装启动 smoke 均成功。
  - GitHub Release 发布时间为 `2026-07-22T16:13:25Z`，Latest 返回 `v0.8.6`；Windows 正式安装包大小 57,079,359 bytes。
  - 使用项目更新选择函数验证 `v0.8.6 > v0.8.5`，Windows 平台正确选择 `LarkSync-Setup-v0.8.6.exe`、对应 `.sha256` 资产和 GitHub digest。
  - 下载三平台正式安装资产并独立计算 SHA256，均与各自 `.sha256` 文件一致：Windows `15d9e544bc9c6f6d8d19f2b3f557dec0a63b0130c6048e9f1b7720e0ba42c1f0`，macOS arm64 `a7bdcbddef669735d67780c53abc16c8a9a812c9b1eb89ce8da95ac097427ae6`，macOS x86_64 `9f31638d15d433cc0466ddb08ca1dafc46a439c7a5518d2527152832fc1d533d`。
  - 三个平台并行上传后 Release 正文一度只保留 Intel 校验值；已恢复三平台完整校验表，安装包与校验资产未受影响。
- 遗留问题：
  - 用户通过 `v0.8.5` 正式版自动升级后，需要验收首次启动耗时、活动/问题页面响应和后续新问题入库；历史 v3 回放保持暂停属于本版设计行为。

## v0.8.6-dev.1 (2026-07-22)

- 开发原因：
  - 用户从 `v0.8.4` 自动升级至 `v0.8.5` 后，桌面应用长时间卡在启动阶段，页面接口也持续等待。
  - 正式数据库只读审计显示 `sync_run_events` 有 2,514,619 条；问题历史游标一度仍需处理 36,035 条相关事件。
  - JSONL 文件大小为 1,162,403,128 bytes，回填断点仅为 6,420,157 bytes；后台连续批次均为 `inserted=0, skipped=200`，说明 SQLite 已有数据仍被逐条重复校验。
  - 两个后台维护任务竞争 SQLite 和事件循环时，活动总览、同步日志及诊断接口等待 44–66 秒，健康检查也曾超过 20 秒。
- 实现方式：
  - 新增 `problem_event_cursor_v4` 作为实时问题游标；首次启动以现有事件表末尾建立基线，并在同步调度启动前提交。
  - `problem_event_cursor_v3` 原值保留，并首次复制到 `problem_history_cursor_v3`；`v0.8.6` 不自动继续历史重放，也不删除历史事件、问题或恢复事实。
  - 实时问题单批从 250 条降至 20 条；满批等待 2 秒，空闲等待 15 秒；失败事件只查询本批指纹对应的问题，不再每批加载全部问题记录。
  - 日志维护启动前检查回填状态；当 SQLite 已存在事件且 JSONL 回填未完成时，将断点对齐到当前文件尾并标记完成，JSONL 原文件保持不变。SQLite 为空时仍保留旧版导入能力。
  - JSONL 后台追赶间隔从 1 秒调整为 5 秒，降低正常运行时的维护唤醒频率。
  - 问题游标初始化或日志维护准备发生异常时记录完整错误并降级到后台增量处理，不阻断 FastAPI 生命周期和桌面首屏启动。
- 当前结果：
  - `v0.8.5 -> v0.8.6` 首次启动不再自动追赶数万条问题历史，也不再对 1.16 GB JSONL 执行无效重复回填。
  - 升级前已有问题记录仍可直接读取；启动基线之后产生的新失败和恢复事件继续进入统一问题生命周期。
  - 全新安装和 SQLite 事件表为空的旧安装仍可从 JSONL 恢复事件，兼容路径不依赖用户专用数据库值。
  - 正式版进程、数据库、同步目录和飞书云端数据均未被修改；本次诊断只使用只读连接和现有日志。
- 验证方式：
  - 飞书开发文档同步成功；4 份本地文档均无需更新。
  - 后端全量 pytest 599 项通过；新增覆盖 v4 游标建立与幂等、v3 历史断点保留、启动后新失败消费、JSONL 有库/空库分支、回填断点对齐和两类启动降级。
  - 前端 ESLint 无警告；Vitest 35 个文件共 105 项通过；TypeScript 类型检查与 Vite 生产构建通过，转换 192 个模块。
  - `python scripts/build_installer.py --nsis` 通过；生成 `LarkSync-Setup-v0.8.6-dev.1.exe`，大小 71,617,542 bytes，SHA256 `8C08BB4AB302E2AC11EAD8C355B8E69BB2C9DB10DF8ED471D46F66172CA05F1D`。仅构建，未安装或启动。
  - 自动升级体验由 `v0.8.6` GitHub Release 验收；正式资产必须以 GitHub Actions 对稳定标签重新构建的产物为准。
- 遗留问题：
  - v3 历史断点保留但默认暂停；如未来需要继续补齐更老的恢复事实，应提供显式空闲维护或手动预演入口，不能重新绑定桌面启动。

## v0.8.5 release (2026-07-22)

- 开发原因：
  - `v0.8.5-dev.1` 已完成正式数据库证据驱动的活动与问题数据可用性修复，用户要求直接发布正式版本，通过稳定渠道继续模拟普通用户自动升级。
- 实现方式：
  - 根包、前端、后端及两个锁文件统一从 `v0.8.5-dev.1` 提升到 `v0.8.5`。
  - README 稳定版本和代码基线更新为 `v0.8.5`；CHANGELOG 增加正式发布锚点。
  - 正式标签触发 Windows NSIS、macOS x86_64 和 macOS arm64 构建、安装启动 smoke、SHA256 生成及 GitHub Release 上传。
- 当前结果：
...
- 开发原因：
  - 标准活动页同时显示左侧任务列表和主栏任务下拉框，形成两个任务选择入口；主栏反而缺少清晰的运行上下文。
  - 正式数据库已有 18,173 条真实活动运行，但活动总览请求超过 30 秒仍超时，导致运行事件无法显示。
  - 正式数据库已有 3,084 条未解决问题和 11 条已解决问题，但问题中心显示三个状态均为 0。
  - 问题中心全局摘要每 10 秒触发一次 `refresh_sources()`；旧刷新会为 3,084 条问题构造规范化路径集合，并扫描 2,514,310 条事件，长期占用 SQLite。
  - 前端为活动、问题列表和问题摘要注入全零占位数据；加载或超时时，界面看起来像真实空数据。
- 实现方式：
  - 标准活动布局保留左侧唯一任务列表；主栏顶部只显示当前运行选择器和“仅显示有实际动作的运行”说明。
  - 活动时间范围默认改为全部保留记录；事件查询切换运行时不复用上一运行的占位数据。
  - 活动总览、任务诊断和运行事件分别增加加载、读取失败、真实为空状态；失败状态提供重试入口。
  - `run_kind=legacy_check` 且 `has_activity=false` 的旧记录继续保存在数据库；默认活动查询和统计仍只使用 `has_activity=true`，不删除历史记录。
  - `/problems` 和 `/problems/summary` 默认 `refresh=false`；前端显式发送 `refresh=false`，页面轮询只读取已持久化问题。
  - 新增 `sync_meta.problem_event_cursor_v3`；后台按 `timestamp + event_id` 顺序每批处理 250 条失败或恢复事件，游标与问题变更在同一事务提交。
  - 后台处理在应用启动 3 秒后开始；批次满时等待 0.25 秒继续，追平后每 10 秒检查一次，异常时等待 30 秒重试。
  - 删除旧版按数千问题路径执行 `LOWER(REPLACE(path)) IN (...)` 的历史恢复扫描；恢复事件直接按解决键匹配同任务、同对象、同操作族的问题。
- 当前结果：
  - 活动页面不再存在两个任务选择入口；标准模式可以直接选择真实运行并查看事件。
  - 问题页面无需等待历史处理完成即可读取现有 3,095 条问题记录；加载和接口错误不会再显示为真实 0 条。
  - 42,991 条旧版无动作检查完整保留，但不会进入活动页面；18,173 条实际活动运行继续可见。
  - 历史恢复事实会在后台逐批补齐；桌面启动和页面读取不再与一次性全量回填绑定。
- 验证方式：
  - 飞书开发文档同步成功；4 份本地文档均无需更新。
  - 后端全量 pytest 通过，共 593 项；新增覆盖持久游标分批推进、同对象后续成功结案和问题 GET 默认不刷新。
  - 前端 ESLint 无警告；Vitest 35 个文件共 105 项通过；TypeScript 类型检查和 Vite 生产构建通过，转换 192 个模块。
  - `python scripts/build_installer.py --nsis` 通过；生成 `LarkSync-Setup-v0.8.5-dev.1.exe`，大小 71,611,075 bytes，SHA256 `8C1317EBC9AF374BC602AAB796791199DC956272F239E5607A8CB242D0FD4D9F`。仅构建，未安装或启动。
  - 对正在运行的正式数据库只读执行新增量 SQL：事件总量 2,514,310 条，相关失败/恢复事件约 44,006 条；三个不同游标位置读取 250 条分别耗时约 0.090、0.072、0.070 秒。
  - 正式数据库审计全程使用 SQLite `mode=ro` 与 `query_only=ON`；未写入正式数据库、未触发同步、未打开或控制桌面窗口。
- 遗留问题：
  - 首次升级需要在后台逐批补齐历史恢复事实；问题总数会随同对象后续成功事实被识别而逐步收敛，这是预期行为。
  - 自动更新后的实际页面响应、增量追平耗时和用户操作体验需要通过 `v0.8.5` 正式安装包继续验收。

## v0.8.4 release (2026-07-22)

- 开发原因：
  - `v0.8.4-dev.1` 已完成活动检测、执行运行和问题生命周期修正，用户决定直接通过正式渠道模拟普通用户的自动升级与安装体验。
- 实现方式：
  - 根包、前端、后端和前端锁文件统一从 `v0.8.4-dev.1` 提升到 `v0.8.4`。
  - README 稳定版本与代码基线更新为 `v0.8.4`；CHANGELOG 增加正式发布锚点。
  - 正式标签触发 Windows NSIS、macOS x86_64 和 macOS arm64 构建、安装启动 smoke、SHA256 生成及 GitHub Release 上传。
- 当前结果：
  - `v0.8.4` 被指定为检测/活动分离和问题自动收敛的首个稳定版本。
  - 从 `v0.8.3` 自动更新时，Windows 应选择 `LarkSync-Setup-v0.8.4.exe`；正式更新渠道不会选择历史 `-dev` 资产。
  - 正式发布不安装、不启动本地构建产物，不修改当前正式数据库、同步目录或飞书数据。
  - GitHub Latest 已指向非预发布 `v0.8.4`；Windows、macOS arm64、macOS x86_64 三个平台安装包及三个 SHA256 文件均为 uploaded。
- 验证方式：
  - 对应业务提交的后端全量 pytest 592 项通过；前端 ESLint、TypeScript、34 个文件共 104 项测试和 Vite 生产构建通过。
  - 本地 NSIS 预构建通过；`v0.8.4-dev.1` 安装包大小 71,608,039 bytes，SHA256 为 `AF01912D9C642296D3166550AB6202A0E5AC77862927006B5134441294CA3D00`。
  - 正式资产必须以 GitHub Actions 对 `v0.8.4` 标签重新构建的产物和 SHA256 为准，不复用开发版文件改名。
  - 正式工作流 `29916213638` 全部通过；Windows NSIS 与两个 macOS DMG 均完成构建、安装启动 smoke、校验生成和 Release 上传。
  - 下载三个正式安装资产并独立计算 SHA256，均与各自 `.sha256` 文件一致：Windows `afec81ac9dde0535b7925294423bcd0f61dbb6e9e4f637c9de5e8404829bf030`，macOS arm64 `d48842ab120097851f6d3c591102b42438a7fb2515dd2c74ca7299facacc1f82`，macOS x86_64 `db7841d4ce335b7d564588e79b8577d0e7e076a9ec7f744fa81b76261a80ba56`。
  - 匿名 GitHub API 命中 `403 rate limit` 时，项目更新服务按设计回退公开 Release 重定向；实测识别 Latest 为 `v0.8.4`，判断其高于 `v0.8.3`，并选择 Windows 安装包及对应校验文件。
- 遗留问题：
  - 自动更新的下载、静默安装、进程退出与重启体验由用户在当前正式版上按普通用户路径继续验收；若失败，需保留“更新与维护”页面状态和更新日志。

## v0.8.4-dev.1 (2026-07-22)

- 开发原因：
  - 正式版最近 400 次运行中有 208 次没有文件动作；检查和执行共用运行记录，活动列表噪声过高。
  - 活动页异步选中任务后把详情模式从 `events` 重置为 `overview`，事件请求随之停用；数据库已有事件但页面可显示为空。
  - 正式版有 3,084 条未解决问题、仅 11 条已解决；旧逻辑只消费失败事件，并用任务级成功运行做手动验证，无法准确收敛文件问题。
  - 删除类未解决问题有 1,073 条，其中 1,069 条最新状态是正常安全等待 `delete_pending`。
- 实现方式：
  - 删除活动页的任务切换页签重置；显式运行选择优先，默认选择最近 `has_activity=true` 的运行；时间过滤使用 `last_event_at ?? finished_at ?? started_at`。
  - schema 升至 v3；`sync_runs` 增加 `run_kind / has_activity`，旧零动作成功运行迁移为 `legacy_check`，运行查询默认排除检查记录。
  - 新增 `sync_task_check_states`，每个任务最多一行；字段记录最近检测状态、来源、开始/结束时间、变化数、连续无变化次数和脱敏错误。
  - 无待上传对象的调度检查不再创建运行；云端扫描和实际运行结束会更新检测状态。完整云端 `ChangePlan` 物理拆分留待可复用一次扫描结果时实施，避免重复调用飞书接口。
  - 问题分类器升级到 `problem-classifier-v2`；解决键固定为 `task_id + object_key + operation_family`，新增解决运行、解决事件、最后成功时间和恢复事实审计。
  - 恢复事件限定为 `uploaded / downloaded / deleted / created / linked / mirrored`；只有时间更晚且解决键相同才自动结案。后续同键失败重新打开问题。
  - `delete_pending` 不再创建问题；旧记录不删除，标记为 `resolved`，验证原因是 `workflow_state_not_problem`。
  - 恢复查询只针对当前未解决问题涉及的任务和对象路径，不全表复制或逐次扫描 250 万事件。
- 当前结果：
  - 活动页面能够持续查询真实事件，且不会被最新无变化检查遮住。
  - 高频无变化检查只覆盖一行任务状态；连续 100 次无变化仍只有一条检查状态记录。
  - 未解决问题表示对象当前最新事实仍异常；自动结案、审计和再次失败重开形成闭环。
  - 页面展示自动解决证据和可操作性；后端只返回当前真实可执行动作。
- 验证方式：
  - 飞书开发文档同步成功，4 份本地文档均无需更新。
  - 后端全量 pytest 592 项通过；覆盖同对象恢复、跨对象隔离、重新打开、`delete_pending` 排除、空检查不解决问题、活动运行默认过滤、100 次检查压缩，以及无变化云端扫描不写运行/事件。
  - pytest 结束时既有媒体上传测试出现一条 aiosqlite 工作线程晚于事件循环关闭的警告；测试结果为通过，本次未扩大范围修改该资源清理问题。
  - 前端 ESLint 无警告；Vitest 34 个文件共 104 项通过；TypeScript 类型检查和 Vite 生产构建通过，转换 192 个模块。
  - `python scripts/build_installer.py --nsis` 通过；生成 `LarkSync-Setup-v0.8.4-dev.1.exe`，大小 71,608,039 bytes，SHA256 `AF01912D9C642296D3166550AB6202A0E5AC77862927006B5134441294CA3D00`。仅构建，未安装或启动。
  - 正式版本机 API 仅做聚合只读审计；未触发同步、未刷新问题源、未修改正式数据库和正式同步目录。
- 遗留问题：
  - 当前正式数据库约 2.12 GB 且正式版正在运行。为避免在线备份和逐行脱敏影响用户工作，本轮未创建全量快照；发布候选阶段按 `docs/REAL_DATA_TESTING.md` 在用户允许的空闲窗口执行。
  - 云端扫描仍由现有执行器完成一次；本轮已完成语义分离、检查状态和活动过滤，完整物理分离需要先保证扫描结果能直接复用为执行计划。

## v0.8.3 release (2026-07-22)

- 开发原因：
  - `v0.8.3-dev.4` 预发布资产和 GitHub 质量门已通过，用户决定将同一功能基线快速提升为正式版，以便通过稳定渠道验证自动升级。
- 实现方式：
  - 根包、前端、后端和锁文件统一从 `v0.8.3-dev.4` 提升到 `v0.8.3`。
  - README 稳定版本和功能列表同步更新；CHANGELOG 增加稳定版发布锚点。
  - 发布说明生成器兼容当前开发日志的“开发原因 / 当前结果 / 验证方式”字段，避免正式 Release 错误显示“未记录详细结果”。
  - 正式标签触发 Windows NSIS、macOS x86_64 和 macOS arm64 构建、安装启动 smoke、SHA256 资产及 Release 说明上传。
- 当前结果：
...
- 开发原因：
  - 旧“活动与问题”只聚焦少量活跃任务，无法稳定审计全部任务、运行和分页事件。
  - 旧冲突页只处理冲突，没有跨运行稳定问题、出现记录、动作历史或解决验证。
  - 正式脱敏快照含 2,278,533 条事件，其中 38,417 条为失败、删除异常、待删除或中断事件；仅回溯最近 5,000 条会漏掉历史问题。
- 实现方式：
  - 将页面拆分为“活动管理”和“问题中心”；侧栏与旧 `#problems` 路由保持兼容。
  - 活动管理使用物理 viewport 与 16px 滞回切换 `compact / standard / wide`；事件接口新增 `since / until` 与稳定 `event_id`，前端按任务、运行、时间、类型、关键字和页码查询。
  - 新增 `problems / problem_occurrences / problem_actions` 三张表和 schema v2 索引；问题指纹固定为来源、任务、分类、阶段、标准化错误码和相对对象键的 SHA256。
  - 分类器版本固定为 `problem-classifier-v1`；Request ID、Token 和冲突云端 Token 在证据入库前脱敏。
  - 同步失败只提供任务重试和打开本地目录；冲突只提供使用云端和使用本地。动作成功不会直接标记解决：冲突检查源记录 `resolved`，任务问题要求发生时间之后存在成功运行。
  - 历史回溯通过 `source_kind + source_id` 唯一索引做增量反连接查询；启动后每批处理 1,000 条并主动让出事件循环。页面刷新也只补一批，已处理来源不重复扫描或计数。
  - 问题中心支持状态、分类、严重级别、任务、时间和关键字筛选；标准/宽屏使用页码，紧凑态最多累加 200 条后按批继续浏览。
  - 活动事件与问题详情使用任务、运行、事件或问题 ID 互相定位；窗口模式切换保留筛选与实体选择，紧凑详情使用明确返回链路。
- 当前结果：
  - 活动页展示全部任务；标准态为任务/事件两栏，宽屏态为任务/运行/事件三栏，紧凑态为选择器、双行列表和整页详情。
  - 问题中心按严重级别和最近发生时间服务端排序；队列、证据、动作历史和后端真实动作形成闭环。
  - 冲突继续复用既有 `ConflictResolutionService`，未新增“保留双方”、单文件重试、忽略或批量处理等不存在的能力。
  - 旧数据库升级时自动创建问题表与索引；历史问题在后台逐批建立，不阻塞数据库初始化和首屏生命周期。
- 验证方式：
  - 飞书文档同步脚本执行成功，4 份本地文档均无需更新。
  - 后端全量 pytest 583 项通过；新增覆盖稳定去重、证据脱敏、动作约束、失败回退、后续成功验证、冲突源终态、schema 迁移和事件时间范围。
  - 前端 ESLint 无警告，Vitest 34 个文件共 103 项通过，TypeScript 类型检查通过，Vite 生产构建转换 192 个模块并成功产出。
  - `python scripts/build_installer.py --nsis` 通过；生成 `LarkSync-Setup-v0.8.3-dev.4.exe`，大小 71,577,821 bytes，SHA256 `A1237AD9D81A9A5F0828AE35BD1374B775837D1741C6AF7491676BF2B7235488`。仅构建未安装或启动。
  - 只读快照配置校验通过：schema v1、8 个任务、57,954 次运行、2,278,533 条事件、14 条冲突、3,587 条 tombstone；任务均停用、无遗留运行、SQLite integrity 为 `ok`。
  - 同规模脱敏合成基准回溯 38,417 条问题事件：全部入库，批大小 1,000，峰值内存 37.6 MB，完成后重复刷新 0.124 秒；测试目录使用临时库并自动删除。
  - 未启动桌面窗口、开发服务器或系统浏览器；未连接飞书、未读取系统 keyring、未修改正式库和正式同步目录。
- 遗留问题：
  - 本版本是开发基线，不生成正式 Release；安装包体验与专用飞书测试目录中的真实写动作需要用户进入开发版验收阶段后执行。
  - 首次历史回溯耗时随历史问题量增长；当前同规模合成库约 18.9 秒，但在后台分批执行，单批峰值内存和重复查询已受控。

## v0.8.3-dev.3 (2026-07-22)

- 开发原因：
  - v2 方案只描述三档布局方向，没有明确进入/退出断点、低高度降级、列宽预算、滚动归属和模式切换时的状态保留规则，开发时仍可能出现临界宽度抖动、隐藏选中项或重复请求。
  - 活动紧凑 v2 仍展示五列表格并叠加约 45% 宽的详情抽屉；在 `1080×720` 物理窗口按现有桌面壳缩放后，事件对象、阶段和结果无法同时保持可读。
  - 问题标准 v2 把长问题标题、状态和操作放在同一横行；长路径或中文标题会挤压动作区，无法形成稳定的响应式边界。
- 实现方式：
  - 将物理 viewport、逻辑 viewport 和应用缩放职责分开，新增 `compact / standard / wide` 带 16px 滞回区间的切换契约，并定义低高度标记、壳层尺寸、字号下限和信息降级顺序。
  - 逐档写明活动管理的任务、运行、事件、详情布局；补充筛选、选中、实时事件插入、归档任务、加载/空/异常、键盘焦点和跨模式状态迁移。
  - 逐档写明问题中心的队列、诊断、证据、动作区布局；补充问题合并与排序、动作生命周期、并发更新、敏感信息、加载/空/异常和跨模式返回逻辑。
  - 活动紧凑态改为两行事件列表与全内容区详情主从切换；活动宽屏态固定为任务、运行、事件三列，不增加第四列。
  - 问题标准态将动作移入详情底部固定操作栏；问题紧凑态补齐独立列表态；问题宽屏态限定为问题队列、诊断证据和动作状态三列。
  - 使用内置 imagegen 生成六张 v3 设计图；保留活动标准 v2 与问题紧凑详情 v2，存在结构问题的旧图继续归档但标记为废弃。
  - 根包、前端、后端和锁文件统一提升到 `v0.8.3-dev.3`；稳定版仍为 `v0.8.2`。
- 当前结果：
  - 三档模式现在都有可直接转成前端验收用例的几何、显示、隐藏和交互规则，不再依赖开发者自行解释设计图。
  - 紧凑态不再通过缩小字号容纳桌面表格；列表与详情使用明确的主从导航，返回后恢复筛选、选择和滚动位置。
  - 标准态优先保证完整工作流；详情动作与长标题解耦，低高度时由正文独立滚动，操作区保持可达。
  - 宽屏态最多三列，新增空间用于运行上下文、证据和动作结果，不引入第四层并排信息。
  - 本版本仍只更新设计文档、图片和版本记录；未修改 `ActivityIssuesPage`、问题数据模型、接口或路由。
- 验证方式：
  - 活动紧凑列表图为 `1536×1024`，SHA256 `E56712ADA0DDE81A17D2F27A04D74D9BA304F05B0835D06AB2625F677F803991`。
  - 活动紧凑详情图为 `1536×1024`，SHA256 `9414B37A64AAAAB6443948E70825B6A53B9D495B7FF31541372EDDF453046317`。
  - 活动宽屏图为 `1586×992`，SHA256 `606ED1866C54166D1E777125C6CC8F6EC495D39053BB476AB17C29BBCBAE2191`。
  - 问题标准图为 `1541×1020`，SHA256 `6E74E647DBE1553B7F3171513425122D2C075EF4E2E00B63A7DA2A9971B305B0`。
  - 问题紧凑列表图为 `1536×1024`，SHA256 `8D347407C9FAF6A321C79415F7C4AF34DBCB8A34F423FFD2398FD6D8C9DD3BC8`。
  - 问题宽屏图为 `1586×992`，SHA256 `681846C575783588B68E43028D37F991149368B8BD87E4467497DCCF467AAB1A`。
  - 文件名中的 `1080×720 / 1360×900 / 1536×960` 表示目标物理窗口和布局档位；PNG 像素尺寸由 imagegen 输出决定。
  - 文档内 10 个图片引用全部可解析；`git diff --check` 和四个包/锁文件 JSON 解析通过。
  - 前端 `npm run typecheck`、`npm run lint`、`npm run build` 通过；生产构建转换 189 个模块。
  - 后端版本与发布相关测试 `test_version.py / test_release.py / test_release_notes.py / test_release_workflow.py` 共 20 项通过。
- 遗留问题：
  - imagegen 设计图用于确认信息结构和视觉关系，不作为逐像素实现证据；实现时必须以文档中的物理 viewport、逻辑宽度和组件尺寸为准。
  - 问题中心完整效果仍依赖统一问题实体、稳定指纹、发生记录、动作记录和服务端 `available_actions`；这些数据能力完成前不得用前端模拟成已实现功能。
  - 页面开发仍需用户确认本轮详细方案后才能开始。

## v0.8.3-dev.2 (2026-07-22)

- 开发原因：
  - 活动管理 v1 在默认 `1360×900` 窗口需要至少 1100px 页面宽度，超过实际扣除侧栏和边距后的约 1068px。
  - v1 两页重新设计了侧栏和顶栏，偏离当前安装版的 228px 侧栏、56px 顶栏和全局命令职责。
  - 问题中心 v1 展示了单文件重试、忽略、批量处理、通用解决状态和处理历史，但当前后端没有统一问题实体、稳定去重、状态机或通用动作历史。
- 实现方式：
  - 只读检查实际安装版在 `1360×900` 和 `1080×720` 下的壳层、内容宽度、字体缩放与活动页列宽；未修改正在运行的正式版。
  - 重写 `docs/design/v0.8.2-activity-problem-center-plan.md`，固定活动管理、问题中心、全局壳层和跨页导航的职责边界。
  - 新增基于物理 viewport 的 `compact / standard / wide` 三档布局规则；窗口切换保留选择、筛选、滚动和打开详情。
  - 规划 `ProblemRecord / ProblemOccurrence / ProblemActionRecord`，补充问题分类、严重级别、稳定指纹、状态验证、接口和现有动作能力矩阵。
  - 使用内置 imagegen 生成活动管理与问题中心的标准窗口和紧凑窗口四张 v2 图片；v1 图片保留并标记为废弃概念稿。
  - 根包、前端、后端和锁文件统一提升到 `v0.8.3-dev.2`；稳定版仍为 `v0.8.2`。
- 当前结果：
  - 活动管理标准模式只保留任务栏和事件工作区；运行改为顶部选择器，详情使用覆盖抽屉。
  - 活动管理紧凑模式移除常驻任务栏和运行栏；使用任务/运行选择器和单事件主区。
  - 问题中心标准模式使用问题队列和诊断工作区两栏；处理记录进入页签，动作区不再常驻占宽。
  - 问题中心紧凑模式使用问题列表与详情主从切换；详情提供返回列表、页签和底部真实动作。
  - 设计图只展示当前已有的“重试任务、打开本地目录、重新授权、冲突决策”等动作；未实现动作必须等待后端通过 `available_actions` 返回后才能显示。
- 验证方式：
  - v2 四张图片均可读取：活动标准 `1540×1021`、活动紧凑 `1536×1024`、问题标准 `1540×1021`、问题紧凑 `1536×1024`。
  - 活动标准 SHA256：`59F1ABD3A981F5CF0806D14EA980316205AD2DBB24181AFDC68144CFDE9C1DFF`。
  - 活动紧凑 SHA256：`286F8D8F6D817D8050E4304A3DA677D2AEA9C15781CC186C23E6D582F434A2F6`。
  - 问题标准 SHA256：`7A6B7C0487B8EF543F61F5B14289CE7FADED0C0EE492FDD06013D330A96575A0`。
  - 问题紧凑 SHA256：`6271DBDC591BAA304E81DDB1B1B62FBDF5310F0F51255C2BC1B5752D90F2F779`。
  - 静态复核确认四图均保留真实全局壳层；页面标题和页面动作未进入顶栏；标准模式没有四栏；紧凑模式没有多栏挤压。
- 遗留问题：
  - 本版只完成方案、设计和图片归档，未修改 `ActivityIssuesPage`、`ConflictResolutionPage`、路由、问题数据库或动作接口。
  - 图片文件的像素尺寸是 imagegen 输出尺寸；文件名中的 `1360×900 / 1080×720` 表示目标物理窗口与布局模式，不表示 PNG 像素尺寸。
  - 实际开发必须先由用户确认本方案，再按“问题数据基线 → 页面实现 → 真实数据验收”推进。

## v0.8.3-dev.1 (2026-07-22)

- 开发原因：
  - 活动管理与问题中心已有文字改版方案，但缺少可评审、可追踪版本的页面设计图。
  - 后续设计图片更新需要保留历史、明确最新版和记录更新原因，避免图片被覆盖后无法判断实现基准。
  - v0.8.1 下载 v0.8.2 时两次发生连接中断；页面已有五步更新流程，但用户观察到流程似乎没有真正实装。
- 实现方式：
  - 使用内置 imagegen，以现有浅色仪表盘为视觉参考、旧活动页和冲突页为信息参考，分别生成活动管理与问题中心 v1 高保真设计图。
  - 图片使用版本化文件名存入 `docs/design/assets/activity-problem-center/`；改版文档嵌入两张图片，并新增最新版判定、禁止覆盖、废弃标记和图片更新记录规则。
  - 只读审计 `MaintenancePage`、`useUpdate`、更新 API、`UpdateService`、`UpdateScheduler`、安装请求和托盘 handoff 链路；结合正式版更新日志与当前状态形成 `docs/design/v0.8.3-auto-update-v2-plan.md`。
  - 源码开发基线提升为 `v0.8.3-dev.1`；稳定版说明继续保持 `v0.8.2`，避免把方案稿误写成已发布能力。
- 当前结果：
  - 活动管理 v1 明确展示全部 `8 / 8` 任务，按任务、运行、事件组织审计，并从失败事件跳转问题中心。
  - 问题中心 v1 统一同步失败、权限/认证、待删除、冲突和系统问题，按问题队列、诊断、处理与处理记录组织；冲突不再独占入口。
  - 实机证据确认两次下载分别在 `21,036,352 / 57,021,601` 和 `4,656,256 / 57,021,601` 字节中断，异常为 `httpx.RemoteProtocolError`；现有下载路由未捕获该异常，更新日志也未记录失败。
  - v0.8.2 已成功安装并自动重启；状态同时存在 `download_path=null`、`install_request=null` 和 `install_handoff.stage=restart_succeeded`，现有前端会推导出“等待下载/等待确认”与“安装完成/重启成功”并存的矛盾流程。
  - 自动更新 v2 方案确定统一 `UpdateOperation`、后台 `.part` 下载、HTTP Range 恢复、最多四次重试、结构化错误、operation/request/handoff 关联和新版本启动对账。
- 验证方式：
  - 两张 PNG 均完成本地读取校验：活动管理 `1672×941`、问题中心 `1671×941`；文件分别为 `1,172,857` 和 `1,172,487` bytes。
  - 活动管理图片 SHA256：`CB726E58F84D7B30F448B050E5B359DE17B3C3519CE3B5E4431FDADFBD0E1FE7`。
  - 问题中心图片 SHA256：`B487D19845605E264894DB2292B69DBB1E4DF7BD8F7F6987D05AAD93C1B66F60`。
  - 正式版只读状态：`current_version=v0.8.2`、`update_available=false`、`restart_succeeded`；未写入正式版更新状态、安装请求或配置。
  - 文档图片引用、版本记录、升级阶段、接口建议、测试场景和验收标准完成静态复核。
- 遗留问题：
  - 本版只完成设计与升级方案，尚未修改 `ActivityIssuesPage`、`ConflictResolutionPage`、`MaintenancePage` 或更新后端状态机。
  - imagegen 图片中的演示日期、路径、计数和错误码只用于布局评审；实际开发必须使用文档的数据口径和真实接口，不得照抄演示值。
  - 自动更新 v2 应按“下载可靠性与统一状态机 → 安装交接 → 页面流程 → 安装包回归”的依赖顺序推进，不与活动管理 / 问题中心页面提交混合。

## v0.8.2 (2026-07-22)

- 开发原因：
  - 托盘双击已有桌面窗口时，旧实现只检查子进程仍在运行并直接返回，没有恢复、置前或路由导航；窗口被关闭或隐藏后表现为长时间无响应。
  - 首次窗口启动固定等待 0.4 秒检查子进程，并通过完整托盘入口加载依赖，增加可见启动延迟。
  - 正式版默认端口 8000 使用频率高，容易与本机开发服务冲突。
  - “活动与问题”默认仅关注筛选会隐藏无近期动作任务；“冲突处理”只覆盖冲突，后续需要拆分完整活动审计与统一问题处理。
- 实现方式：
  - 桌面窗口关闭事件改为取消销毁并异步隐藏，WebView 子进程在托盘存活期间持续常驻。
  - 桌面子进程在 `127.0.0.1` 随机端口创建带随机令牌的控制通道；描述文件使用系统临时目录中的单次随机路径，退出时删除。
  - 托盘复用已有窗口时发送 `show + url` 指令；宿主依次导航、恢复和置前，不创建第二个 WebView。
  - 打包入口为 `--desktop-window` 增加快速分支，避免导入托盘、后端管理和图标依赖；默认启动不再固定阻塞 0.4 秒，显式测试仍可设置提前退出检测窗口。
  - 正式后端默认端口改为 18765；`LARKSYNC_BACKEND_PORT` 覆盖规则不变。Vite、CLI、WSL/OpenClaw、macOS smoke、CORS 和使用文档同步迁移。
  - 配置加载时仅把精确匹配的 `localhost/127.0.0.1:8000/auth/callback` 迁移到 18765；自定义域名、路径和环境变量不改。非生产真实数据测试同时检测新正式端口 18765 和过渡期旧端口 8000。
  - 新增 `docs/design/v0.8.2-activity-problem-center-plan.md`；活动管理默认展示全部任务，问题中心统一失败、权限、待删除、冲突和系统问题。本版不修改两个页面代码。
- 当前结果：
  - 关闭桌面窗口后宿主继续存活；再次双击托盘可复用同一 PID 并恢复到指定页面。
  - 默认生产地址为 `http://127.0.0.1:18765`；当前正在运行的 v0.8.1 正式版 8000 进程未被停止或修改。
  - OAuth 已有 Token 可继续使用；后续需要重新授权时，飞书应用安全设置必须包含 `http://localhost:18765/auth/callback`。
  - 活动与问题、冲突处理页面保持 v0.8.1 行为，改版进入后续独立版本。
- 验证方式：
  - TDD 定向测试覆盖隐藏而非销毁、带令牌回环控制、已有窗口导航/恢复、子进程退出重开、托盘退出清理、桌面快速入口、默认端口、OAuth 回调迁移和运行安全边界。
  - 后端全量 pytest：578 项通过；前端 Vitest：31 个文件、92 项通过；TypeScript typecheck、ESLint 和 Vite production build 通过。
  - `python scripts/build_installer.py --nsis`：通过；安装包 `LarkSync-Setup-v0.8.2.exe` 为 71,503,818 bytes，SHA-256 为 `83240BB453DE177C392F0F3266305DB49EF10EFB38495875F844585A154C6E99`。
  - 目录版隔离 smoke：`snapshot_test`、独立端口 18103、独立数据目录和 file Token Store；`/health=ok`，就绪 1,556.4 ms，版本 `v0.8.2`，`packaged=true`，静态前端 HTTP 200，关闭后进程与端口正常退出。
  - 默认端口 smoke：不设置 `LARKSYNC_BACKEND_PORT`，目录版在 18765 启动；使用临时数据目录，返回版本 `v0.8.2` 和 production Profile，关闭后 18765 释放。
  - 验证结束后旧正式版 8000 仍由原 PID 42188 监听；隔离端口 18102、18103 和新默认端口 18765 均无残留监听。
- 遗留问题：
  - 用户升级后应在飞书开放平台为当前应用增加 18765 OAuth 回调地址；现有授权未失效时不影响自动升级与日常同步。
  - 活动管理与问题中心按方案另行开发，不包含在 v0.8.2。

## v0.8.1 (2026-07-21)

- 开发原因：
  - 用户完成 `v0.8.0-dev.41` 启动、授权与字号复测后，同意将版本提升为稳定版 `v0.8.1`。
  - 字号和间距调整后，总览摘要卡在窄窗口出现提示换行与数值拥挤；同步任务表原 `1120px` 最小宽度会把右侧操作列裁出可视区。
  - 活动页中栏只有约 366px 时，四项摘要横排会把中文标签拆成单字并覆盖事件时间线；最小窗口中的筛选按钮和阶段标签也会被压成竖排。
  - 首次引导页三栏没有独立高度与滚动边界，低高度窗口会直接裁掉右侧 CLI 诊断；任务详情右侧五张检查卡也存在底部危险操作不可达的风险。
- 实现方式：
  - 总览摘要卡收紧图标框、卡片内边距和内容间距，并为卡片与内容容器补充最小宽度和溢出边界；保留 22px 核心数值与当前全局字阶。
  - 同步任务表最小宽度由 `1120px` 调整为 `1000px`；队列、删除、失败、冲突四列减少横向内边距并固定单行，1080×720 下仍可看到完整操作列。
  - 活动页四项问题摘要由单行四列调整为 2×2；筛选按钮、下拉入口和阶段胶囊禁止收缩换行，避免中文标签竖排。
  - 首次引导根布局改为 `64px + minmax(0,1fr)` 两行；左、中、右三栏各自使用纵向滚动，内容超出时不再被页面根节点裁切。
  - 任务详情检查器改为独立纵向滚动，五张检查卡固定自身高度；危险操作不再承担剩余高度，避免内部内容被隐藏。
  - 顶栏右侧命令区由 430px 扩到 460px，并收紧按钮间距，使完整账号名称获得稳定可读宽度。
  - 根包、前端包、锁文件、后端包与维护页演示版本统一到 `v0.8.1` / `0.8.1`。
- 当前结果：
  - 1360×900 默认窗口下，总览、同步任务、活动与问题、冲突处理、设置、更新维护及首次引导内容完整；任务详情右栏在内容超过可用高度时显示独立滚动。
  - 1080×720 最小窗口下，六个主导航页面的 `documentElement.scrollWidth` 与 `body.scrollWidth` 均为 1080px，没有整页横向溢出。
  - 任务表右侧运行、启停、目录和更多操作全部可见；活动摘要不再覆盖时间线，四类筛选和事件阶段均保持横排。
  - 正在运行的正式版、正式端口、正式数据目录、正式 Token 和同步任务均未停止、修改或升级；视觉验收使用隔离的 `synthetic_test` 数据。
- 验证方式：
  - 先写适配契约测试并确认旧实现失败；实现后定向回归 5 个文件、16 项测试通过。
  - 前端全量 Vitest：31 个文件、92 项通过；ESLint、TypeScript typecheck 和 Vite production build 通过。
  - 后端全量 pytest：572 项通过；首次执行因 244 秒命令上限被终止，扩大上限后完整重跑于 223.4 秒通过。
  - Edge headless 使用独立用户目录和隔离端口，在 1360×900 与 1080×720 逐页检查；任务详情最小窗口实测 5 张检查卡均存在，检查器 `overflow-y=auto`，`scrollHeight=772` 大于 `clientHeight=743` 时可以滚动。
  - `python scripts/build_installer.py --nsis`：通过；安装包 `LarkSync-Setup-v0.8.1.exe` 为 71,494,798 bytes，SHA-256 为 `7AB1ACA1816C2189B4AF54A9B960420C9D079086F778F1DC5DCD184162A14C3E`。
  - 最终目录版隔离 smoke：`snapshot_test`、独立端口 18091、独立数据目录和 file Token Store；`/health=ok`，版本为 `v0.8.1`，`packaged=true`，静态前端存在，调度器与文件监控关闭，关闭接口返回 `shutting_down` 且进程和端口均正常退出。
- 遗留问题：
  - 本轮不执行真实飞书写入测试；安装后应由用户在现有正式数据态复核 1360×900 常用窗口和系统缩放显示，再决定是否创建 GitHub Release 供自动升级。
  - 构建仍提示 Browserslist 数据超过 6 个月，以及 PyInstaller 对未安装的可选数据库驱动发出既有 warning；本轮产物、自动化测试与隔离启动未受影响。

## v0.8.0-dev.41 (2026-07-21)

- 开发原因：
  - 正式安装版 `2026-07-21 16:03:25` 初始化后，首个 `/health` 直到 `16:04:12` 才响应，启动可用性等待约 47 秒。
  - 正式数据的 `sync-events.jsonl` 为 1,094,937,001 bytes；配置保留 90 天，日志服务启动后会扫描该文件，旧实现在线程主循环内执行同步文件清理。
  - 8 个启用任务在 1 秒保护期后几乎同时开始；启动负载期间 `/tray/status` 最长约 24.7 秒，`/sync/logs/sync` 最长约 38.3 秒。
  - `/auth/status` 同步执行 Token 读取、身份补齐、Token 刷新和 Drive 探测；连续 4 次分别约 12.7、13.3、13.2、13.3 秒，并持续轮询放大负载。
  - OAuth 回调在 Token 交换后还同步请求用户资料，再写入多条 Windows 凭据；一次成功回调约 3.15 秒，回调后又串行等待授权状态远程探测。
  - `v0.8.0-dev.40` 统一抬升小字号，但 11px 与 12px 层级行高过近，侧栏、顶栏和正文留白没有同步调整，信息层级与密度不协调。
- 实现方式：
  - `/auth/status` 改为本地二态检查：凭据存在即“已授权”，不存在即“未授权”；不刷新 Token、不补用户资料、不请求飞书 Drive，也不再返回 `drive_ok` 三态字段。
  - Windows Keyring 首次读取后使用进程内缓存；Token 更新、登出和用户资料写入转入工作线程，避免同步系统调用阻塞 FastAPI 事件循环。
  - OAuth 回调关键路径只保留授权码换 Token 和安全存储；用户资料请求从 Token 交换中移除，昵称在首屏稳定 2 秒后异步补齐，登录后的更新检查延后 5 秒，并新增 Token 交换与安全存储分段耗时日志。
  - 日志维护默认延后 20 秒；JSONL 清理通过 `asyncio.to_thread()` 执行。适用文件为 `sync-events.jsonl`，保留天数仍沿用用户配置，本次不删除或缩短现有 90 天历史。
  - 自动同步默认延后 12 秒；任务按创建时间排序，每个任务相差 0.75 秒启动；上传与下载调度合计最多 2 路并发。手动同步入口与用户配置的后续同步周期不变。
  - 移除 Google Fonts 运行时网络依赖，使用 Segoe UI Variable / Microsoft YaHei UI 与 Cascadia Code 系统字体栈。
  - 字阶统一为 11px/16px、12px/18px、13px/20px、14px/20px；侧栏由 220px 增至 228px，顶部由 52px 增至 56px，正文留白改为 32px × 24px，运行卡片内边距和行间距同步增加。
- 当前结果：
  - 隔离后端实测从进程启动到 `/health` 可用为 1,204.1 ms；首次本地授权状态为 79.4 ms，进程缓存后的第二次为 1.5 ms。
  - 授权页和设置页只展示“已连接 / 未连接”；网络探测失败不再生成“授权过期、权限不足、检查中”一类启动状态，也不再触发已连接状态轮询。
  - 1360×900 无界面 headless 截图确认：侧栏、56px 顶栏、四张摘要卡、主区与右栏均完整显示；较大字号未造成横向滚动、控件裁切或底部遮挡。
  - 正式安装版及其任务未被停止、修改或升级；性能取证只读取日志与配置中的非敏感统计字段，集成 smoke 使用独立数据目录和 `snapshot_test`。
- 验证方式：
  - 授权、安全存储、日志维护、同步调度和系统状态定向后端回归通过，共 38 项。
  - 前端授权、首次引导、设置和桌面壳定向回归通过，共 12 项；TypeScript typecheck、ESLint 与 Vite production build 通过。
  - 后端全量 pytest：572 项通过；前端 Vitest：31 个文件、92 项通过。
  - 隔离性能 smoke：`snapshot_test`、独立端口 18081、独立数据目录、真实 Keyring 只读；`health_ready_ms=1204.1`、`auth_first_ms=79.4`、`auth_cached_ms=1.5`。
  - Edge headless 1360×900 视觉验收使用独立端口 18082、独立数据目录和快照只读配置；未打开或操作用户前台窗口。
  - `python scripts/build_installer.py --nsis`：通过；安装包 `LarkSync-Setup-v0.8.0-dev.41.exe` 为 71,494,853 bytes，SHA-256 为 `42270585F17FD351026400C6995BD17E982A0934AA22CA029342FC9E2398A10D`。
  - 最终目录版隔离 smoke：`snapshot_test`、独立端口 18041、独立数据目录和 file Token Store；`/health=ok`，就绪 1,212.0 ms，版本为 `v0.8.0-dev.41`，`packaged=true`，静态前端存在，关闭接口退出码 0。首次误用 memory Token Store 被运行安全校验按预期拒绝，改为规定的 file Store 后通过。
- 遗留问题：
  - 当前正式 `sync-events.jsonl` 约 1.09 GB，本版保证其维护不冻结 API，但不会擅自删除 90 天保留期内的历史；如需缩减磁盘，应由用户在维护页调整保留天数后执行清理。
  - 本版仍为开发版；需要用户安装后复测真实窗口的启动时间、扫码回跳和常用页面密度，再决定稳定版发布。

## v0.8.0-dev.40 (2026-07-21)

- 开发原因：
  - 用户在 `v0.8.0-dev.39` 扫码授权后短暂看到“授权过期”，随后又正常进入桌面壳。
  - 正式安装版日志显示，`2026-07-21 15:03:30` 和 `15:04:47` 的 `/auth/status` 分别耗时约 13.6 秒和 13.3 秒，并记录 Drive 权限检查请求异常。
  - 原授权状态把 Drive 探测的网络超时、响应异常和飞书明确返回缺权全部映射为 `drive_ok=false`；前端据此显示“权限不足 / 重新授权飞书”，导致有效令牌被误判。
  - 当次 `/auth/callback` 在 `15:05:21` 返回 307 成功跳转；后续只读状态检查显示账号仍连接且令牌未过期。LarkSync 未输出“授权过期”文案，该瞬时提示来自飞书授权页，不是本地回调的 `state 无效或已过期`。
  - 桌面壳默认窗口为 `1280x820`，但前端以 `1536x1024` 设计画布连续缩放；默认窗口实际缩放约 80%，使 12–13px 文字显示为约 10px。
- 实现方式：
  - 授权状态改为三态：飞书返回 code 0 为 `drive_ok=true`；飞书明确返回非零 code 为 `false`；请求超时、网络异常或响应解析失败为 `null`。
  - `drive_ok=null` 时保留当前登录，不显示缺权或重新授权按钮；桌面壳提示“权限检查暂时不可用”，并每 5 秒自动重试。
  - Drive 权限探测超时由 10 秒缩短为 5 秒，降低启动阶段等待；设置页和首次授权页同步使用三态文案。
  - 参考 Codex Companion 源码中的 `1360x900` 默认窗口和正常尺寸 1:1 策略，将 LarkSync 默认窗口、设计画布及托盘内部参数统一为 `1360x900`。
  - 大于设计画布的窗口不再放大界面；小于设计画布时按宽高较小比例缩放，最低比例为 `1080/1360`。
  - 高频 `10px / 11px / 12px` 辅助字号分别提升为 `11px / 12px / 13px`；14px 及以上层级保持不变。
- 当前结果：
  - 飞书接口暂时不可用时不会再诱导用户重复扫码；只有飞书明确返回缺权才显示重新授权入口。
  - 默认窗口相较上一版宽度增加 80px、高度增加 80px，并从约 80% 缩放恢复为 1:1 渲染。
  - 当前设备主工作区为 `2048x1104`，`1360x900` 默认窗口可完整容纳；未启动或操作任何前台窗口进行验证。
  - 首次打包检查发现当前虚拟环境缺少已声明的 `pywebview`，该次产物未交付；补齐后端 editable 依赖后重新打包，最终目录包含 `webview`、WebView2 DLL、pythonnet 与 clr_loader。
- 验证方式：
  - 授权与窗口定向后端回归：18 项通过；补充打包、授权和窗口最终定向回归共 43 项通过。
  - 后端全量 pytest：571 项通过。
  - 前端 Vitest：31 个文件、94 项测试通过；覆盖授权未知状态、自动重试、1360x900 画布和大窗口不放大。
  - 前端 ESLint、TypeScript typecheck 和 Vite production build：通过。
  - `python scripts/build_installer.py --nsis`：通过；最终安装包 `LarkSync-Setup-v0.8.0-dev.40.exe` 为 59,800,110 bytes，SHA-256 为 `ADC48DD5AA64EE06428C39533D5E05F6DD64651D5F10C8226421E2A5889F0629`。
  - 目录版隔离 smoke：使用临时数据目录、`synthetic_test`、独立端口 `18040`、file Token Store、scheduler/watcher 关闭；`/health=ok`，版本为 `v0.8.0-dev.40`，`packaged=true`，静态启动画面与 WebView 依赖均存在，随后通过关闭接口退出，退出码 0。
- 遗留问题：
  - 飞书授权页自身的瞬时“授权过期”提示无法由本地前端控制；本地通过成功回调和后续授权状态判断最终结果。
  - `v0.8.0-dev.40` 仍为开发版，需用户安装后人工检查默认窗口、字号和真实网络波动下的提示，再决定是否发布稳定版。
...
- 开发原因：
  - 使用用户确认的专用飞书根目录执行首次真实双向回归时，任务虽然报告上传 1、下载 11、失败 0、删除 0，但请求审计显示无删除补齐阶段仍对刚下载的 Markdown 执行了转换与增强镜像刷新。
  - 根因包含两层：双向无删除补齐先执行全量下行再执行全目录上行；同时，增强模式的下行阶段会主动刷新云端 Markdown 镜像。两条路径都会让未修改的下行文件产生资源上传、镜像旧 Token 清理和新 Token 创建。
- 实现方式：
  - 双向无删除补齐在访问云端前冻结当时已存在且未被忽略的本地文件清单。
  - 保持云端优先：先执行 `allow_deletes=False` 下行，再仅通过 `_run_upload_paths()` 处理冻结清单；下行期间新增的 Markdown、附件和资源文件不会进入补齐上行。
  - 下载编排把 `allow_deletes=False` 同时解释为“不允许附带云写”；Docx 下行仍写入本地及同步映射，但不会调用增强模式的 Markdown 云端镜像刷新。
  - `upload_only` 仍执行全目录无删除上行，`download_only` 仍只执行无删除下行；补齐结束后的常规同步顺序不变。
  - 回归测试模拟下行阶段新建文件，明确断言上行路径只包含下行前存在的 `local-only.md`，不包含 `downloaded-during-reconcile.md`；另以 `download_only / bidirectional` 参数化验证无删除下行均不会创建镜像目录。
- 当前结果：
  - 真实数据测试的生产快照、只读和最终专用双向阶段全部通过；前两次双向尝试因审计发现重复回传而未判通过，问题均已由真实日志定位并修复。
  - 最终增强双向任务成功：总文件 30、完成 14、跳过 32、上传 1、下载 13、失败 0、删除 0、冲突 0。
  - 最终云端审计共 187 条：语义只读 183 条，真实写入 4 条；4 条写入仅包含测试前已存在 Markdown 的 2 次 `upload_all`、1 次 `import_tasks` 和 1 次临时导入源文件清理，全部 HTTP 200、飞书码 0。
  - 第二轮常规下载/上传开始前，4 条预期写入已经全部结束；常规阶段扫描 14 个本地文件但未产生额外云端写入，证明刚下载文件没有再次触发回传或增强镜像刷新。
  - 正式版 `v0.7.29` 已恢复，健康检查、授权状态、8 个任务及全部任务 ID 与维护前一致；重启后正式调度器按原配置自行运行，本轮未取消或修改正式任务。
- 验证方式：
  - TDD 失败阶段：原实现调用全目录 `_run_upload()`，新增回归按预期失败。
  - 修复后同步运行器定向回归：通过；新建任务、近期任务和无删除下行墓碑保护共 3 项通过，`test_sync_runner.py` 全文件通过。
  - 脱敏生产快照：数据库约 1.98 GB，8 个任务全部停用，遗留运行 0，SQLite `integrity_check=ok`；表内包含 2,278,533 条运行事件。
  - 快照查询基准：任务列表 p95 0.22 ms、最近运行 p95 83.83 ms、最近事件 p95 22.96 ms，Python 峰值内存约 4.2 MB，低于 250 ms 与 256 MB 门槛。
  - 真实只读回归：下载 11、失败 0；审计 50 条且全部为 GET，写请求 0。
  - 修复前真实双向证据：allowlist 错误根目录返回 403；合法任务上传 1、下载 11、失败 0、冲突 0，但补齐阶段发生重复 Markdown 转换与增强镜像刷新，因此未判通过。
  - 第一版路径冻结复测：任务上传 1、下载 12、失败 0，但审计仍有 130 条写请求；日志确认这些写请求来自增强模式的下行镜像刷新，由第二道“无删除下行禁止附带云写”边界修复。
  - 最终真实增强双向复测：错误根目录返回 403；合法任务上传 1、下载 13、失败 0、删除 0、冲突 0；187 条审计中仅有 4 条预期写入，写入数相较修复前的 20 条降至 4 条。
  - 后端全量 pytest：563 项通过；同步运行器、运行档位、快照、限流、审计和桌面后端回归均通过。
  - 前端 Vitest：30 个文件、89 项测试通过；ESLint、TypeScript typecheck 与 Vite production build 均通过。
  - `python scripts/build_installer.py --nsis`：通过；最终安装包 `LarkSync-Setup-v0.8.0-dev.38.exe` 为 71,478,894 bytes，SHA-256 为 `356CB19B87DC01B9EE1604F767B2563189808D62280104121E460E33E3E7562F`。
  - 最终目录版打包 smoke：在独立数据目录、`synthetic_test`、scheduler/watcher 关闭条件下返回 `v0.8.0-dev.38`，退出码 0。
- 遗留问题：
  - 专用飞书目录保留三轮双向测试产生的文档与 `_LarkSync_MD_Mirror` 测试镜像，便于人工复核；如不再需要，可由用户在该专用目录统一删除。
  - 维护窗口开始前，正式版任务 `7d2a0255-b406-48fb-bb61-995147b81b9c` 自然结束时报告 2 个 `delete_failed`，原因均为云端目标文件夹已不存在；该现象来自正式版原任务，不属于本轮专用目录验收，建议后续独立排查删除幂等性和错误摘要口径。
...
- 开发原因：
  - 删除全局底部状态栏后，总览页仍沿用固定的 `146 / 278 / 310px` 三层高度，内容约在画布 `y=886px` 结束，底部产生明显空白。
  - 总览页“快速操作”重复顶部命令栏的立即同步和任务管理；“实时连接”重复侧栏的后端、飞书连接状态。
  - 最近同步只展示 5 条记录，没有利用释放出的工作区高度。
- 实现方式：
  - 总览根容器使用 `100vh / --desktop-scale - 98px` 计算逻辑高度；基准画布保持 146px 摘要和 300px 运行区，第三层使用 `minmax(0, 1fr)` 吸收不同窗口宽高比产生的剩余空间。
  - 顶部第二张摘要卡改为“今日活动”，按下载与上传事件汇总；右栏改为“优先处理 / 任务状态 / 今日传输”，移除与桌面壳重复的操作和连接信息。
  - 最近同步扩展为 7 条，生产数据和开发演示数据使用同一条目上限；趋势图随第三层高度自适应，但不改变 1536×1024 基准布局密度。
  - 为左右列及三层面板增加明确的最小高度和溢出边界，避免 CSS Grid 自动最小尺寸造成 5–8px 的底部越界。
- 当前结果：
  - 1536×1024、1440×1024、1280×1024 和 1080×720 四档窗口中，总览左右栏都延伸至工作区底部，底边误差为 0.3–0.4px。
  - 运行区与右侧任务状态区纵向误差为 0px；7 条最近同步和底部历史入口完整可见。
  - 页面不存在全局底栏、重复壳层操作、横向溢出或主工作区纵向滚动。
- 验证方式：
  - `python scripts/sync_feishu_docs.py`：通过；4 个飞书文档包已检查。
  - TDD 失败阶段：今日活动、新右栏信息架构、7 条记录和自适应三层高度断言均按预期失败。
  - Edge + Playwright 四档像素审计：左右栏底边误差不超过 0.4px，运行区对齐误差 0px，最近同步均为 7 条，控制台及网络错误为 0。
  - 前端定向测试：通过，2 个测试文件、12 个测试。
  - `npm run dev:test`：通过；隔离前端、后端和桌面窗口正常启动，验证后主动退出。
  - `npm --prefix apps/frontend test`：通过，30 个测试文件、88 个测试。
...
- 开发原因：
  - 用户要求继续完成其他页面与原始设计的一致性优化，本轮按主导航顺序处理“活动与问题”。
  - 当前实现把任务选择做成工作台内 68px 横向卡片条，右侧诊断栏缩为 336px，时间线改成独立卡片列表，与 `07-activity-issues-light.png` 的核心结构差异明显。
  - 当前演示态错误诊断只展示原因、建议和路径，缺少源稿中的基础信息、错误码、原始错误详情和固定处理操作。
- 实现方式：
  - 新增本地规格 `docs/local_specs/activity_issues_design_alignment_v0.8.0-dev.33.md`，以 07 号浅色方向稿为主基线，并叠加后续纯白底部、高对比度和完整可见性规则。
  - 先更新活动页与演示页结构测试，使旧任务卡片条、272/336px 列宽和简化诊断结构按预期失败，再修改实现。
  - 将任务选择移到页头中部，保留唯一“刷新诊断”入口；开发演示态与真实数据态统一使用该结构。
  - 主工作台恢复为 `276px / minmax(0,1fr) / 416px`，1536×1024 下中栏实际宽度为 568px。
  - 运行历史新增运行 ID 搜索和 5 次完整运行记录；中栏改为四类问题摘要、四项筛选与连续时间轴；右栏改为基础信息、错误码、原始错误、原因分析、编号建议和固定双操作。
  - 演示数据继续只在开发环境默认启用；`?ui-data=live` 与生产构建继续读取真实后端数据。
- 当前结果：
  - 1536×1024 下三栏宽度为 276px、568px、416px；1440×1024 下为 258.8px、532.5px、390px；1280×1024 下为 230px、473.3px、346.7px。
  - 三档均完整显示 5 条运行记录、5 条关键事件、4 张问题摘要和底部复制/重试操作。
  - 三档工作台均位于状态栏上方，无页面级横向溢出，控制台错误为 0。
  - 点击不同事件可更新错误诊断；切换任务可更新页头任务选择，三档交互验证均通过。
- 验证方式：
  - `python scripts/sync_feishu_docs.py`：通过；4 个飞书文档包已检查。
  - TDD 失败阶段：活动页与演示页新增的页头任务选择、276/416px 列宽、运行搜索、时间线和错误详情断言按预期失败。
  - 活动页定向测试：通过，2 个测试文件、6 个测试。
  - `npm --prefix apps/frontend test`：通过，30 个测试文件、87 个测试。
  - `npm --prefix apps/frontend run typecheck`：通过。
  ...
  - Edge + Playwright 1536×1024、1440×1024、1280×1024 三档截图、边界测量和交互验证：通过。
  - 截图与报告位于 `docs/local_specs/visual_audit/2026-07-15-activity-design-alignment/`，不进入 Git。
...
- 开发原因：
  - 用户确认同步任务页之后还应存在原始设计中的独立任务详情页；现有版本把该页面改成了不同的连续检查栏结构。
  - 开发环境同步任务表默认展示 `task_001` 等演示任务，但详情页只读取后端真实任务，点击演示任务会显示“未找到任务”。
  - 任务详情与任务行三点打开的“任务设置”弹窗职责混淆，原始稿中的“编辑策略”入口未落到现有设置弹窗。
- 实现方式：
  - 先新增本地规格 `docs/local_specs/task_detail_original_design_restoration_v0.8.0-dev.28.md`，以 `06-task-detail-light-v3.png` 为唯一视觉基线，明确独立详情页与任务设置弹窗的边界。
  - 先改写 `TaskDetailPage` 测试，使旧结构因缺少固定“任务详情”标题、五张右侧卡片和演示任务详情而失败，再完成页面实现。
  - 页面恢复为面包屑与固定标题、`minmax(0, 1fr) + 300px` 双列、任务身份与同步关系、当前/最近运行、5 条运行历史，以及问题摘要、任务操作、策略摘要、忽略目录、危险操作五张独立卡片。
  - 新增强类型任务详情演示数据；`task_001` 使用 256 个总文件、174 个已处理文件的 68% 进度，显示 128 个上传、86 个下载、43 个跳过、1 个冲突和 5 次运行。
  - 开发环境详情页沿用同步任务页演示模式；`?ui-data=live` 读取真实任务和诊断数据，生产构建始终使用真实后端数据。
  - 详情页“编辑策略”复用 `TaskSettingsModal`；演示态保存、启停、运行和重置只更新当前页面状态，不写入后端。
  - 新增 `/sync/tasks/{task_id}/open-local-folder`；接口只读取该任务已配置的 `local_path`，校验目录存在后调用系统文件管理器，不接受客户端任意路径。
  - 将跨平台打开目录逻辑提取到 `src/core/file_manager.py`，更新维护页原接口继续复用同一实现。
  - 视觉脚本除检测卡片边界外，增加每张卡片 `scrollHeight <= clientHeight + 1` 的内容级检测，修复运行历史末行/查看更多重叠和危险操作说明溢出。
- 当前结果：
  - 从同步任务页点击第一行“查看任务详情”可进入完整独立页面，不再显示“未找到任务”。
  - `1536x1024`、`1440x1024`、`1280x1024` 均显示 68% 当前运行、5 条历史和 5 张右侧卡片。
  - 三档窗口均无文档或主工作区横向溢出；左右首卡顶部误差不超过 1px；左列与右列内部无重叠。
  - 三档所有卡片底边均位于状态栏上方，且所有关键卡片 `scrollHeight` 均未超过 `clientHeight`。
  - “编辑策略”可打开与任务表三点按钮相同的居中设置弹窗，`Escape` 可关闭。
  - 冲突策略文案按 SSOT 使用“云端优先（保留副本）”，未照搬原稿中与产品定义冲突的“保留双方文件”。
- 验证方式：
  - `python scripts/sync_feishu_docs.py`：通过；4 个飞书文档包已检查。
  - 任务详情定向测试：通过，1 个测试文件、3 个测试。
  - `npm --prefix apps/frontend test`：通过，30 个测试文件、87 个测试。
  ...
  - `npm --prefix apps/frontend run lint`：通过，无警告。
  - `python -m pytest`（`apps/backend`）：通过，537 个测试。
  - `python scripts/build_installer.py`：通过；Vite 生产构建和 PyInstaller 桌面目录打包成功，生成 `dist/LarkSync/LarkSync.exe`（17.5 MB），本轮未指定 `--nsis`。
  - Edge + Playwright 三档截图与内容边界检查：通过；结构化报告错误为 0。
  - 截图与报告位于 `docs/local_specs/visual_audit/2026-07-15-task-detail-original-restoration/`，不进入 Git。
- 遗留问题：
  - 演示文件数、容量、速度和预计剩余时间仅用于开发视觉基线；真实接口未提供这些统计时显示“尚未统计”或 `--`，不使用演示值填充。
  - 云端“打开目录”使用任务已有 `cloud_folder_token` 构造飞书文件夹地址；未新增或猜测第三方 API 字段。

## v0.8.0-dev.27 (2026-07-15)

- 开发原因：
  - 用户指出当前同步任务页与 `docs/local_specs/design_artifacts/v0.8.0/light/04-sync-tasks-light.png` 原始浅色稿仍有明显差距。
  - 开发测试数据默认被隐藏时，任务表进入空状态，无法呈现原稿中的运行中、错误、冲突、停用和三种同步模式。
  - 旧工具栏同时暴露刷新、测试任务开关和新建任务，唯一主操作不明确。
  - 固定画布缩放到 `1440x1024`、`1280x1024` 时，任务行只占工作台上半区，内部留下大面积空白。
- 实现方式：
  - 先新增本地规格 `docs/local_specs/sync_tasks_design_contract_v0.8.0-dev.27.md`，逐项定义原稿保留项、纠错项、列宽、行高、操作顺序和三档窗口验收口径。
  - 新增 8 条强类型同步任务演示数据，覆盖双向同步、仅上传、仅下载、运行中、空闲、失败、冲突和停用状态。
  - 演示数据仅在开发环境默认启用；查询参数 `?ui-data=live` 返回真实数据；生产构建始终使用后端任务数据。
  - 工具栏只保留“新建任务”主按钮；刷新与测试任务显示开关移动到表格页脚的次级区域。
  - 表格恢复原稿的信息顺序：任务名称、本地目录、云端目录、同步模式、状态与健康、最近运行、四类计数、操作。
  - 本地目录允许最多两行展示；双向同步、仅上传、仅下载分别使用蓝、绿、紫三种模式色。
  - 状态取消大色块胶囊，改为状态文本和健康点两层信息；操作固定为运行、启停、详情、三点设置。
  - 页面改为满高连续工作台；任务行在 `1536px` 保持 `76px`，在 `1440px` 和 `1280px` 分别补偿为 `88px` 和 `100px` 设计高度，抵消桌面画布缩放后的内部空白。
  - 演示模式下运行、启停、删除和任务设置均只修改本地展示状态，不写入真实任务；三点按钮继续打开独立居中设置弹窗。
- 当前结果：
  - `1536x1024`、`1440x1024`、`1280x1024` 均完整显示 8 条任务，文档和主工作区无横向溢出。
  - 三档实测物理行高分别为 `76px`、`82.5px`、`83.3px`；任务表底边均未越过状态栏上边线。
  - 表格底边与状态栏上边线的间距分别为 `23px`、`5px`、`19.2px`，页面底部没有异色带或大面积外部留白。
  - 搜索“设计资源”后任务行从 8 条准确收敛为 1 条。
  - “新建任务”可打开五步向导；每行最右侧三点可打开标题为“任务设置”的居中弹窗。
  - 浏览器控制台错误和页面异常均为 0。
- 验证方式：
  - `python scripts/sync_feishu_docs.py`：通过；4 个飞书文档包已检查。
  - 同步任务页与任务面板定向测试：通过，2 个测试文件、7 个测试。
  - `npm --prefix apps/frontend test`：通过，30 个测试文件、86 个测试。
  - `npm --prefix apps/frontend run typecheck`：通过。
  - `npm --prefix apps/frontend run lint`：通过，无警告。
  - `python -m pytest`（`apps/backend`）：通过，536 个测试。
  - `python scripts/build_installer.py`：通过；Vite 生产构建和 PyInstaller 桌面目录打包成功，生成 `dist/LarkSync/LarkSync.exe`，本轮未指定 `--nsis`。
  - Edge + Playwright 三档截图与交互检查：通过；8 行任务、行高、表格边界、横向溢出、搜索、新建向导、三点设置弹窗和控制台错误均有结构化报告。
  - 截图与报告位于 `docs/local_specs/visual_audit/2026-07-15-sync-tasks-design-pass/`，不进入 Git。
- 遗留问题：
  - 演示数据用于锁定前端设计效果，不代表真实同步结果；需要验证真实任务时使用 `?ui-data=live`。
  - 本轮未新增或猜测飞书 API 字段，未改变同步任务接口契约。

## v0.8.0-dev.26 (2026-07-15)

- 开发原因：
  - 用户复核 `v0.8.0-dev.25` 后发现内部容器仍有卡片被工作台底边遮挡。
  - 活动页的运行与事件列表、设置页的本地目录、维护页的安装前检查和维护记录存在首屏半张卡片。
  - 主工作区冷蓝底色与近白状态栏在底部形成异色横带；旧设计稿仍保留 30–34px 底栏和整块浅蓝工作区的错误描述。
- 实现方式：
  - 先新增本地规格 `docs/local_specs/remaining_pages_visibility_correction_v0.8.0-dev.26.md`，将“首屏无半张卡片”和“底部纯白收口”设为硬性验收指标。
  - 同步修正本地壳层规格：底栏固定为 `78px`；主工作区最后 `56px` 过渡到 `#FFFFFF`；状态栏背景统一为 `#FFFFFF`。
  - 四页根容器移除 `min-h-[760px]`，改为 `h-full min-h-0`，避免页面在桌面壳内被强行撑破。
  - 活动页首屏展示 5 次完整运行和 6 条完整事件；剩余 2 次运行、2 条事件通过固定提示表达。
  - 冲突页将队列规则与统计合并为固定摘要，压缩决策区间距，完整保留 4 条冲突和两种决策动作。
  - 设置页缩短 section 间距，将最近保存并入数据保护区；账号、策略、OAuth、本地目录和 3 条忽略规则均保持完整。
  - 维护页将版本说明与安装前检查改为并列布局；维护设置改为横向标签输入，运行环境、危险操作和最近记录全部落在首屏。
  - 为关键列增加 `data-visibility-region`，为关键卡片增加 `data-visibility-card`；浏览器脚本逐卡片比较边界，不再只测工作台整体高度。
- 当前结果：
  - `1536x1024`、`1440x1024`、`1280x1024` 共 12 组页面组合全部通过边界检测。
  - 活动页每档 `11/11` 张关键卡片完整可见；冲突页 `9/9`；设置页 `10/10`；维护页 `8/8`。
  - 每档文档宽度与主工作区宽度均无横向溢出。
  - 每页工作台底边均未越过状态栏上边线。
  - 主工作区背景在底部收口到 `#FFFFFF`；状态栏实测为 `rgb(255, 255, 255)`。
  - 浏览器控制台错误和页面异常均为 0。
- 验证方式：
  - `python scripts/sync_feishu_docs.py`：通过；4 个飞书文档包已检查。
  - 四页展示组件、桌面壳和 App 定向测试：通过，3 个测试文件、12 个测试。
  - `npm --prefix apps/frontend test`：通过，30 个测试文件、84 个测试。
  - `npm --prefix apps/frontend run typecheck`：通过。
  - `npm --prefix apps/frontend run lint`：通过，无警告。
  - `python -m pytest`（`apps/backend`）：通过，536 个测试。
  - `python scripts/build_installer.py`：通过；前端生产构建和 PyInstaller 桌面目录打包成功，本轮未指定 `--nsis`。
  - Edge + Playwright 三档逐卡片边界检测：通过；12 组页面的 `clippedCards` 均为空。
  - 截图与结构化报告位于 `docs/local_specs/visual_audit/2026-07-15-remaining-pages-visibility/`，不进入 Git。
- 遗留问题：
  - 演示数据仍只用于开发环境视觉验证；生产构建继续使用真实后端数据。
  - 本轮未新增或猜测飞书 API 字段，未改变同步、冲突、设置或更新接口契约。

## v0.8.0-dev.25 (2026-07-14)

- 开发原因：
  - 用户指出 `v0.8.0-dev.24` 四页下半区仍存在大面积留白，页面虽然没有横向溢出，但没有形成完整桌面工作台。
  - 本地真实数据不足时，活动与冲突页频繁进入空状态，无法验证多事件、多冲突和复杂详情下的视觉密度。
  - 设置与维护页按内容自然高度堆叠，工作区剩余高度没有承载配置状态、目录信息或维护记录。
- 实现方式：
  - 先新增本地规格 `docs/local_specs/remaining_pages_full_canvas_showcase_v0.8.0-dev.25.md`，将工作台占满、底边距和演示数据量设为硬性验收指标。
  - 新增 `shouldUseRemainingPagesShowcase`：开发环境默认使用演示数据；查询参数 `?ui-data=live` 显式回到真实数据；生产环境始终返回真实数据模式。
  - 新增四个类型化展示组件，并在页面入口与既有真实数据组件之间做模式分流；真实 hooks、API 和生产行为继续保留。
  - 四页标题旁统一展示“前端演示数据”，避免把样例任务、冲突、版本和运行状态误认为真实数据。
  - 活动页使用完整高度三栏工作台，展示 4 个任务、7 次运行、8 条问题事件、问题统计、事件诊断和关联运行。
  - 冲突页展示 4 条非空冲突队列、本地/云端版本、段落差异、队列统计、覆盖影响和冲突安全机制。
  - 设置页展示账号设备、默认策略、计划、OAuth、3 条忽略规则、4 项配置状态、本地目录缓存和最近保存结果。
  - 维护页展示新版本、安装包、五步交接、版本说明、安装前检查、运行环境和维护记录；危险任务仍默认收起。
  - 四页根布局改为“标题栏 + `minmax(0, 1fr)` 主工作台”，长列表和次级内容在栏内滚动。
- 当前结果：
  - `1536x1024` 下四页工作台高度均为 `744px`，与内容区底部距离均为 `23px`。
  - `1440x1024` 下底边距为 `21.6px`；`1280x1024` 下底边距为 `19.2px`。
  - 三档窗口的文档宽度均等于 viewport 宽度；桌面主区 `scrollWidth` 与 `clientWidth` 相等。
  - 活动事件选择、冲突选择和维护危险任务展开均通过浏览器交互检查。
  - 浏览器控制台和页面异常监听均无错误。
  - 正式构建不会启用演示数据；本轮没有改变真实同步、冲突解决、设置保存和更新安装 API 契约。
- 验证方式：
  - `python scripts/sync_feishu_docs.py`：通过；4 个飞书文档包已检查。
  - 演示模式与四页组件测试：通过，1 个测试文件、5 个测试。
  - `npm --prefix apps/frontend test`：通过，30 个测试文件、84 个测试。
  - `npm --prefix apps/frontend run typecheck`：通过。
  - `npm --prefix apps/frontend run lint`：通过，无警告。
  - `python -m pytest tests/test_version.py -q`（`apps/backend`）：通过，确认版本为 `v0.8.0-dev.25`。
  - `python scripts/build_installer.py`：通过；前端生产构建和 PyInstaller 桌面目录打包成功，本轮未指定 `--nsis`。
  - Edge + Playwright 完成三档四页截图、底边距、溢出、控制台和关键交互检查：通过。
  - 截图证据位于 `docs/local_specs/visual_audit/2026-07-14-remaining-pages-full-canvas/`；该目录属于本地规格资料，不进入 Git。
- 遗留问题：
  - 当前演示组件用于先锁定视觉和交互结构；后续应按同一展示模型逐项接回真实数据，不再直接用后端字段驱动布局。
  - 未新增或猜测任何飞书 API 字段。

## v0.8.0-dev.24 (2026-07-14)

- 开发原因：
  - 活动与问题页由多个独立卡片拼接，任务、运行、问题和操作之间缺少连续上下文，空数据时留白零散。
  - 冲突处理页在未检测到冲突时仍保留三栏操作骨架，健康状态与待处理状态的视觉语义不明确。
  - 设置页的默认策略、OAuth 和页面级按钮分别提供保存入口，用户无法确认一次修改是否已完整保存。
  - 更新与维护页同时提供两个“检查更新”；重置同步映射的任务和危险按钮长期暴露，主次关系失衡。
- 实现方式：
  - 先新增本地规格 `docs/local_specs/remaining_desktop_pages_redesign_v0.8.0-dev.24.md`，逐页定义信息架构、唯一主操作和三档窗口验收口径。
  - 活动与问题页新增任务上下文条；主体改为 `272px / 自适应 / 336px` 连续工作台，统一运行历史、问题概览、时间线、事件诊断和处理操作。
  - 冲突数为 0 时改用完整健康空状态；存在冲突时使用 `260px / 自适应 / 300px` 连续工作台，并为“使用云端 / 使用本地”补充覆盖影响说明。
  - 设置页合并飞书账号与当前设备；默认策略和 OAuth 面板移除局部保存按钮，统一通过页面右上角“保存设置”提交。
  - 更新与维护页将更新流程和维护设置合并为连续双栏工作台；移除流程区重复更新检查，仅保留页面主操作。
  - 重置同步映射默认只展示影响说明和“选择任务重置”，用户主动展开后才渲染任务列表；实际重置仍沿用应用内二次确认。
  - 为四页补充结构、唯一操作和危险内容默认隐藏的渲染测试；版本统一推进至 `v0.8.0-dev.24`。
- 当前结果：
  - 四个剩余主导航页面与总览、任务页共享相同的连续面板语言、边框对比度和操作层级。
  - 设置页可见“保存设置”数量为 1；维护页可见“检查更新”数量为 1。
  - 维护页初始不渲染具体任务；点击“选择任务重置”后按钮切换为“收起任务列表”，`aria-expanded=true`。
  - `1536px`、`1440px`、`1280px` 三档窗口中，文档 `scrollWidth` 均等于 viewport 宽度；浏览器控制台和页面异常监听均无错误。
  - 条件式首次授权页继续沿用既有业务流程，本轮未修改授权规则或飞书 API 字段。
- 验证方式：
  - `python scripts/sync_feishu_docs.py`：通过；4 个飞书文档包已检查。
  - 四页与设置面板定向测试：通过，5 个测试文件、7 个测试。
  - `npm --prefix apps/frontend test`：通过，29 个测试文件、79 个测试。
  - `npm --prefix apps/frontend run typecheck`：通过。
  - `npm --prefix apps/frontend run lint`：通过，无警告。
  - `python -m pytest tests/test_version.py -q`（`apps/backend`）：通过，确认开发态版本读取为 `v0.8.0-dev.24`。
  - `python scripts/build_installer.py`：通过；前端生产构建、托盘图标生成和 PyInstaller 桌面目录打包成功，本轮未指定 `--nsis`。
  - Playwright + Edge 完成三档真实页面截图、唯一主操作、危险任务展开、页面溢出和浏览器错误检查：通过。
  - 截图证据位于 `docs/local_specs/visual_audit/2026-07-14-remaining-pages-redesign/`；该目录属于本地规格资料，不进入 Git。
- 遗留问题：
  - 本轮仅重构现有页面的信息架构和交互层级，不改变同步、冲突解决、配置保存和更新安装 API 契约。
  - 未新增或猜测任何第三方 API 字段。
