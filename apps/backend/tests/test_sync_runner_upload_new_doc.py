...
@pytest.mark.asyncio
async def test_upload_markdown_repairs_previous_table_marker_large_table_link_once(
    tmp_path: Path,
) -> None:
    ...
    link_service.links[str(markdown_path)] = SyncLinkItem(
        ...
        updated_at=0.0,
        local_hash=file_hash,
        cloud_revision="doc-existing@1#md-table-render-v4",
    )
    docx_service = FakeDocxService()
    runner = SyncTaskRunner(
        docx_service=docx_service,
