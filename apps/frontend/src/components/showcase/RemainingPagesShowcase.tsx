      ...
      <div data-workspace-fill="true" className="grid min-h-0 grid-cols-[minmax(0,1fr)_360px] overflow-hidden rounded-xl border border-[#cdddf0] bg-white shadow-[0_16px_40px_rgba(51,112,255,0.08)]">
        <main data-visibility-region="maintenance-main" className="showcase-scroll-region min-h-0 overflow-y-auto border-r border-[#dce7f4] p-4">
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            <div data-visibility-card="maintenance-version" className="rounded-lg border border-[#d7e4f5] bg-[#fbfdff] p-3"><p className="text-xs text-[#6b7f96]">当前版本</p><p className="mt-1.5 text-xl font-semibold text-[#102033]">v0.8.0-dev.25</p><p className="mt-1.5 text-xs text-[#52657a]">上次检查：今天 17:42</p></div>
            <div data-visibility-card="maintenance-version" className="rounded-lg border border-[#f5d28f] bg-[#fffbeb] p-3"><p className="text-xs text-[#92400e]">可用版本</p><p className="mt-1.5 text-xl font-semibold text-[#78350f]">v0.8.0-dev.27</p><p className="mt-1.5 text-xs text-[#92400e]">LarkSync-Setup-v0.8.0-dev.27.exe</p></div>
          </div>
          <div data-visibility-card="maintenance-package" className="mt-3 rounded-lg border border-[#d7e4f5] p-3"><div className="grid grid-cols-[110px_minmax(0,1fr)_120px] gap-4 text-xs"><div><p className="text-[#6b7f96]">安装包大小</p><p className="mt-1 font-semibold text-[#102033]">18.2 MB</p></div><div><p className="text-[#6b7f96]">下载位置</p><p className="mt-1 truncate font-mono text-[#102033]">D:/Downloads/LarkSync-Setup-v0.8.0-dev.27.exe</p></div><div><p className="text-[#6b7f96]">校验状态</p><p className="mt-1 font-semibold text-[#047857]">SHA256 通过</p></div></div><div className="mt-3 flex gap-2"><button type="button" className="h-8 rounded-lg bg-[#3370ff] px-4 text-xs font-semibold text-white">下载更新</button><button type="button" className="h-8 rounded-lg border border-[#bfd3ee] px-4 text-xs font-semibold text-[#3370ff]">打开安装包目录</button><button type="button" className="h-8 rounded-lg border border-[#8fd7bf] bg-[#f2fbf8] px-4 text-xs font-semibold text-[#047857]">静默安装</button></div></div>
          ...
          <section data-maintenance-summary-grid="true" data-visibility-card="maintenance-summary" className="mt-4 grid grid-cols-2 gap-4 border-t border-[#dce7f4] pt-4">
            <div><PanelHeading title="版本说明" hint="v0.8.0-dev.27 · 同步任务页设计复原" /><div className="mt-3 grid grid-cols-2 gap-2">{[['完整首屏', '四页卡片不再被底边裁切。'], ['统一底色', '工作区与状态栏无异色横带。'], ['滚动边界', '列表滚动范围清晰可见。'], ['信息密度', '保留关键数据并减少拥挤。']].map(([title,desc]) => <div key={title} className="rounded-lg border border-[#d7e4f5] bg-[#fbfdff] p-2.5"><p className="text-xs font-semibold text-[#102033]">{title}</p><p className="mt-1 text-[10px] leading-4 text-[#52657a]">{desc}</p></div>)}</div></div>
