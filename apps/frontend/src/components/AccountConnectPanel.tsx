...
type ConnectionResult = "new" | "restored" | "existing" | "reauthorized";
...
export function AccountConnectPanel({
  ...
  const [createdProfileDisplayName, setCreatedProfileDisplayName] = useState<string | null>(null);
  const [appDisplayName, setAppDisplayName] = useState("LarkSync 应用 1");
  const [manualDisplayName, setManualDisplayName] = useState("");
  const [autoGeneratedAppName, setAutoGeneratedAppName] = useState(false);
  ...
  const [connectionResult, setConnectionResult] = useState<ConnectionResult>(mode === "reauthorize" ? "reauthorized" : "new");
  ...
  useEffect(() => {
    setAppDisplayName((current) => (
      /^LarkSync 应用 \d+$/.test(current)
        ? `LarkSync 应用 ${(profiles.data?.length ?? 0) + 1}`
        : current
    ));
    ...
    const poll = async () => {
      try {
        ...
        if (status === "registered") {
          ...
          setCreatedProfileDisplayName(profile.display_name || appDisplayName);
          setSession(null);
          setSessionKind(null);
        ...
        if (status === "authorized") {
          ...
          setConnectionResult((result.connection_result as ConnectionResult | undefined) || (mode === "reauthorize" ? "reauthorized" : "new"));
  ...
  }, [appDisplayName, mode, session, sessionKind]);
  ...
  const beginRegistration = () => {
    const displayName = appDisplayName.trim();
    if (!displayName) {
      setError("请填写用于区分应用配置的应用名称。");
      return Promise.resolve(null);
    }
    setConnectPath("automatic");
    setCreatedProfileId(null);
    setCreatedProfileAppId(null);
    setCreatedProfileDisplayName(displayName);
    setAutoGeneratedAppName(/^LarkSync 应用 \d+$/.test(displayName));
    return startSession(
      "/app-profiles/registration-sessions",
      { brand: "feishu", display_name: displayName },
      "registration",
      "registering_app",
    );
  };
  ...
  const finishAuthorized = async () => {
    ...
    try {
      ...
      if (
        mode === "add"
        && connectPath === "automatic"
        && autoGeneratedAppName
        && createdProfileId
      ) {
        const nextDisplayName = `LarkSync · ${organizationName}`;
        await apiFetch(`/app-profiles/${createdProfileId}/display`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ display_name: nextDisplayName }),
        });
        setCreatedProfileDisplayName(nextDisplayName);
        await completionRef.current.refetchProfiles();
      }
    ...
    try {
      const profile = await apiFetch<AppProfile>("/app-profiles/manual", {
        ...
        body: JSON.stringify({
          ...
          brand: "feishu",
          display_name: manualDisplayName.trim() || null,
        }),
      });
      setAppSecret("");
      setCreatedProfileId(profile.id);
      setCreatedProfileAppId(profile.app_id);
      setCreatedProfileDisplayName(profile.display_name || profile.app_id);
      setAutoGeneratedAppName(false);
    ...
    return (
      <div data-account-connect-root="true" data-connect-phase="authorized">
        <div data-testid="authorization-success" className="rounded-2xl border border-[#a7e2cf] bg-[linear-gradient(135deg,#f2fbf8,#f7fbff)] p-6 text-center shadow-[0_16px_40px_rgba(16,185,129,0.10)]">
          ...
          <h2 className="mt-2 text-2xl font-semibold text-[#102033]">{connectionResult === "restored" ? "已恢复之前移除的账号" : "授权成功"}</h2>
          <p className="mt-2 text-sm leading-6 text-[#52657a]">
            {connectionResult === "restored"
              ? `${organizationName ? `${organizationName} · ` : ""}${accountName} 已恢复，原任务、状态和历史数据已重新关联。`
              : mode === "reauthorize"
    ...
    return (
      <div data-account-connect-root="true" data-connect-phase="app_registered">
        <div className="rounded-2xl border border-[#a7e2cf] bg-[linear-gradient(135deg,#f2fbf8,#f7fbff)] p-6 text-center shadow-[0_16px_40px_rgba(16,185,129,0.10)]">
          ...
          {createdProfileDisplayName ? <p className="mt-2 text-xs font-semibold text-[#34516f]">LarkSync 内名称：{createdProfileDisplayName}</p> : null}
  ...
  return (
    <div data-account-connect-root="true" data-connect-phase={phase}>
      <div className="rounded-2xl border border-[#cfe0f5] bg-[linear-gradient(135deg,#f7fbff,#edf5ff)] p-5">
        ...
        <label className="mt-4 block max-w-md text-xs font-semibold text-[#294662]">
          应用名称
          <input
            aria-label="应用名称"
            ...
            className="mt-2 h-10 w-full rounded-xl border border-[#b9cfee] bg-white px-3 text-sm font-medium text-[#102033] outline-none focus:border-[#3370ff] focus:ring-2 focus:ring-[#3370ff]/15"
          />
          <span className="mt-2 block font-normal leading-5 text-[#71869d]">仅用于在 LarkSync 中区分不同应用，不会修改飞书后台显示的名称。</span>
        </label>
          ...
          <div className="mt-3 grid gap-2">
            {profiles.data?.map((profile) => (
              <button key={profile.id} type="button" disabled={busy} onClick={() => void beginExistingDevice(profile.id)} className="flex items-center justify-between rounded-xl border border-[#d6e3f3] px-4 py-3 text-left hover:border-[#3370ff] hover:bg-[#f7faff] disabled:opacity-50">
                <span><span className="block text-sm font-semibold text-[#102033]">{profile.display_name || profile.app_id}</span><span className="mt-1 block text-xs text-[#71869d]">{profile.source === "official_registration" ? "自动创建" : profile.source === "legacy" ? "升级迁移" : "手动配置"} · {profile.brand === "lark" ? "Lark" : "飞书"} · …{profile.app_id.slice(-4).toUpperCase()} · {new Date(profile.created_at * 1000).toLocaleDateString()}</span>{profile.recoverable_account_count ? <span className="mt-1 block text-xs font-semibold text-[#b45309]">含 {profile.recoverable_account_count} 个可恢复账号</span> : null}</span>
      ...
      {manualOpen ? (
        <div className="mt-3 grid gap-3 rounded-2xl border border-[#d6e3f3] bg-white p-5">
          <input aria-label="手动应用名称" value={manualDisplayName} maxLength={120} onChange={(event) => setManualDisplayName(event.target.value)} placeholder="应用名称（仅 LarkSync 内，可选）" className="rounded-xl border border-[#c7d7ea] px-4 py-2.5 text-sm outline-none focus:border-[#3370ff]" />
